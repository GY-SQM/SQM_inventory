...
def _dbg_log(tag: str = "", location: str = "", message: str = "", data=None) -> None:
    # v8.7.0: 프로덕션 debug 파일 쓰기 제거 → logger.debug
...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def _create_dialog(self) -> None:
        ...
        if not getattr(self, 'compact_mode', False):
            ...
            # v8.7.0: 파싱 결과 수동 편집 활성화 (더블클릭 + Ctrl+C/V)
    ...
    def _parse_thread(self) -> None:  # v8.7.0 [SRP] refactored — sub-methods below
        """백그라운드 파싱"""
        try:
    ...
    # ───────────────────────────────────────────────────────────────
    # _parse_thread sub-methods  (v8.7.0 [SRP])
    # ───────────────────────────────────────────────────────────────

    def _pt_init_parser(self, ParserClass):  # v8.7.0 [SRP]
    ...
    def _pt_extract_template_hints(self) -> dict:  # v8.7.0 [SRP]
    ...
    def _pt_parse_documents(self, parser, ctx: dict):  # v8.7.0 [SRP]
    ...
    def _pt_parse_bl(self, parser, file_path: str, ctx: dict):  # v8.7.0 [SRP]
    ...
    def _pt_handle_bl_carrier_detection(self, bl_result, ctx: dict):  # v8.7.0 [SRP]
    ...
    def _pt_handle_missing_dates(self, do_result):  # v8.7.0 [SRP]
    ...
    def _pt_collect_warnings_and_crosscheck(self, pl_result, inv_result, bl_result, do_result):  # v8.7.0 [SRP]
    ...
    def _pt_show_error_recovery_dialog(self, bl_result, pl_result, inv_result, do_result):  # v8.7.0 [SRP]
    ...
    def _pt_finalize_preview(self):  # v8.7.0 [SRP]
    ...
    def _pt_parse_confirm(self, pl_result, elapsed_str: str):  # v8.7.0 [SRP]
    ...
    def _sync_tree_edit_to_preview_data(self) -> None:
        """v8.7.0: GlobalEditableTree 편집 후 preview_data 동기화.
        ...
        try:
            ...
            logger.debug(f"[v8.7.0] preview_data 동기화 완료: {len(self.preview_data)}행")
        except Exception as e:
            logger.debug(f"[v8.7.0] preview_data 동기화 스킵: {e}")

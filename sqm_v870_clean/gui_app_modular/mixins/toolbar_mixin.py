...
class ToolbarMixin:
    ...
    def _setup_toolbar(self) -> None:
        ...
        # v8.7.0: 버전 배지 → 타이틀바로 이동 (우측 공간 확보)
        ...
        # v8.7.0: 컬러 액션 버튼 바 (v8.3.4 스타일 복원)
        try:
    ...
    def _build_action_button_bar(self) -> None:
        """v8.7.0: v8.3.4 스타일 컬러 버튼 바 — 다크/라이트 자동 대응.
    ...
    def _build_quick_theme_buttons(self, parent) -> None:
        ...
        try:
            ...
            for icon, label, theme in [("🌙", "Dark", "darkly"), ("☀", "Light", "litera")]:  # v8.7.0 수정
                btn = tk.Label(
    ...
    def _refresh_toolbar_colors(self) -> None:
        """v8.7.0: 테마 전환 시 툴바 전체 배경·전경색 동기화"""
        try:
            self._load_toolbar_colors()
    ...
    def _highlight_active_tab(self) -> None:
        """v8.7.0 [SIDEBAR]: 활성 탭 = 밝은 흰색, 비활성 = 탭 고유 컬러."""
        SIDEBAR_BG   = tc('bg_secondary')
        ACTIVE_BG    = tc('bg_secondary')
        ACTIVE_BAR   = tc('border_focus')
    ...
    def _on_dn_cross_check(self) -> None:
        """🔍 DN 교차검증 — 고객 Sales Order / DN vs SQM DB (v8.7.0).

...
class SQMInventoryApp:
    ...
    def __init__(self, root=None, db_path: Optional[str] = None):
        ...
        # v8.7.0: 메인 윈도우 숨기고 스플래시(모래시계+타이머) 표시
        self.root.withdraw()
        ...
        # v8.7.0: 초기화 완료 → 스플래시 제거 + 메인 윈도우 표시
    ...
    def _setup_ui(self) -> None:
        ...
        # v8.7.0 PERF-2: dirty flag 시스템 초기화
        ...
        def _on_notebook_tab_changed(event):
            try:
                ...
                # v8.7.0 PERF-2: dirty 탭 전환 시 새로고침
    ...
    def _show_loading_splash(self) -> None:
        ...
        try:
            ...
            logger.debug(f"[v8.7.0] 스플래시 표시 실패 (무시): {e}")
    ...
    def _hide_loading_splash(self) -> None:
        ...
        try:
            ...
            # 메인 윈도우 표시
            self.root.deiconify()
            logger.info(f"[v8.7.0] 스플래시 제거 → 메인 윈도우 표시 ({elapsed:.1f}s)")
        except Exception as e:
            # 실패해도 메인 윈도우는 반드시 표시
            try:
                self.root.deiconify()
            ...
            logger.debug(f"[v8.7.0] 스플래시 제거 실패 (무시): {e}")
    ...
    def _force_initial_theme_refresh(self) -> None:
        ...
        try:
            ...
            if hasattr(self, '_highlight_active_tab'):
                self._highlight_active_tab()
            # v8.7.0: 메뉴바 컬러 복원 (apply_tc_theme_to_all 이후)

...
(function () {
  ...
  var _onestopState = {
    ...
    carrier: '',
    step: 1,
    /* v8.7.0 Hybrid: 사용자가 사전 결정하는 packing_type 기본값 (선적 전체 적용) */
    /*   'C'      = 기본 (500kg × 2pack, 가장 흔한 패턴) */
    /*   'A'/'B'  = 강제 적용 (드문 경우) */
    /*   'auto'   = LOT별 net÷mxbg 자동 판별 (v8.7.0 이전 방식) */
  ...
  function _ensureParseResultWindow() {
    ...
    /* v8.7.0: 팔레트 구성 탭 — 자동/사용자 확인 + 셀 점유 사전 계산 */
  ...
  function showOneStopInboundModal() {
    ...
    var html = [
      ...
      '  <h2 style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">📥 입고 — SQM v8.7.0 (OneStop)'
      ...
      '  </div>',
      /* v8.7.0: 팔레트 구성 사전 결정 (Hybrid 설계) — 사용자가 선적 전체의 기본 packing_type 을 미리 지정 */
      '  <div class="onestop-row">',
      '    <label>📦 팔레트:</label>',
  ...
  /* v8.7.0 Hybrid: 사용자가 선적 전체의 기본 packing_type 사전 결정 */
  ...
  window.onestopParseStart = function() {
    ...
    xhr.onload = function(){
      ...
      if (xhr.status >= 200 && xhr.status < 300 && body && body.ok) {
        ...
        /* v8.7.0 fix: open window FIRST so onestop-preview-body exists in DOM */
        var _cEl2 = document.getElementById('onestop-carrier');
        _openParseResultWindow(_cEl2 ? _cEl2.value : '', rows.length);
        _onestopRenderPreview(_onestopState.previewRows);
        /* v8.7.0: 팔레트 구성 자동 판별 + 500kg 강제확인 게이트 초기화 */

# -*- coding: utf-8 -*-
"""
★★★ SQM v6.3.2 — Navy Blue Theme v5.1 ★★★
====================================================
날짜: 2026-03-04
작성: Ruby
...
def fix_main_app(sqm_root):
    ...
    if 'STANDARD_THEMES' not in content:
        ...
        if match:
            ...
            NEW_BLOCK = (
                ...
                f"{indent}# ★ v5.1-NAVY: ttkbootstrap 색상 소스 직접 수정\n"
                f"{indent}try:\n"
        ...
        # ★ 이미 STANDARD_THEMES 코드가 있으면 _gy_colors 블록 전체 교체
        gy_block_pat = re.compile(
            r'_gy_colors\s*=\s*\{[^}]+\}',
            re.DOTALL)
        match_gy = gy_block_pat.search(content)
        if match_gy:
            NEW_GY_BLOCK = """_gy_colors = {
                        'primary':   '#2563eb',   # 로열 블루
                        'secondary': '#64748b',   # 슬레이트
                        'success':   '#10b981',   # 에메랄드
                        'info':      '#0ea5e9',   # 스카이 블루
            ...
            if '#2563eb' not in match_gy.group():
                content = content[:match_gy.start()] + NEW_GY_BLOCK + content[match_gy.end():]
                print("  ✅ _gy_colors → Navy Blue 색상 교체")
            else:
                print("  ℹ️ Navy Blue 이미 적용됨")
        else:
            print("  ⚠️ _gy_colors 블록 미발견")

    # ─── (B) current_theme 동기화 ───
    # 패턴1: flatly/darkly/cosmo 등 어떤 테마든 하드코딩
    # 패턴2: _ttk_detect (v4 패치) — 이미 자동 감지
    CT_REPLACEMENT = (
        "# ★ v5.1-NAVY: 실제 테마 자동 감지\n"
        "        try:\n"
    ...
    if '_ttk_detect' in content and 'v5.1-NAVY' in content:
...
def main():
    print()
    print("=" * 64)
    print("  🎨 SQM v6.3.2 — Navy Blue Theme v5.1")
    ...
    print()
    print("  적용된 GY Navy Blue 테마:")
    print("   🔵 Primary:  #2563eb (로열 블루) → 헤더/탭/버튼/스크롤바 전체")
    print("   🟦 Info:     #0ea5e9 (스카이 블루)")
    print("   🟢 Success:  #10b981 (에메랄드)")
    print("   🟡 Warning:  #f59e0b (앰버)")
    print("   🔴 Danger:   #ef4444 (레드)")
    print("   🌊 BG:       #0b1120 (딥 네이비)")
    print()
    print("  상태별 행 색상:")

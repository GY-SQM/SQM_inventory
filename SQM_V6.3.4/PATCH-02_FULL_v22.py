...
from tkinter import ttk, filedialog, BOTH, YES, X, Y, LEFT, RIGHT, TOP, BOTTOM, END, W, E, N, S as TK_S
...
class S1OneStopOutboundDialog:
    ...
    def show(self):
        ...
        self.win.grab_set()
        center_dialog(self.win, 960, 720)
        ...
        for i, (k, v) in enumerate(BATCH_STATUS.items()):
            ttk.Label(sm_fr, text=v, font=("맑은 고딕", 8),
                      foreground=STATUS_COLOR[k]).pack(side=LEFT, padx=2)
    ...
    def _build_tab1(self):
        ...
        # 텍스트 입력
        self.paste_text = tk.Text(fr, height=14, font=("Consolas", 10), wrap='none')
        ...
        ttk.Button(btn_fr, text="🔄 파싱 → DRAFT   ▶",
                   command=self._do_parse, style='Accent.TButton').pack(side=RIGHT, padx=4)
    ...
    def _do_parse(self):
        ...
        if not batches:
            ...
            if errors:
                err_msg += "\n" + "\n".join(errors[:5])
            self.parse_status_var.set(f"❌ 파싱 실패")
    ...
    def _build_tab2(self):
        ...
        ttk.Button(top, text="DRAFT → WAIT_SCAN  ▶",
                   command=self._confirm_draft, style='Accent.TButton').pack(side=RIGHT, padx=4)
    ...
    def _refresh_tab2(self):
        ...
        for batch in self.batches:
            ...
            if not tonbags:
                ttk.Label(bf, text="⚠️ 가용 톤백 없음", foreground="red").pack(padx=8, pady=4)
                continue
            ...
            grid_fr = ttk.Frame(bf)
            grid_fr.pack(fill=X, padx=4, pady=2)

            batch['_tonbags'] = tonbags

            for i, tb in enumerate(tonbags):
                var = tk.BooleanVar(value=tb['id'] in self.tonbag_selections.get(bid, set()))
                self._tb_check_vars[(bid, tb['id'])] = var

                cb_text = f"#{tb['sub_lt']}  {tb['weight']:.0f}kg"
                if tb.get('location'):
                    cb_text += f"  [{tb['location']}]"

                cb = ttk.Checkbutton(grid_fr, text=cb_text, variable=var,
                                     command=lambda b=bid, t=tb['id'], v=var: self._toggle_tonbag(b, t, v))
    ...
    def _random_select(self, batch_id):
        ...
        for tb in available:
            ...
            remaining -= tb['weight']
        if remaining > 0:
            logger.warning(f"[S1] 랜덤선택: 충분한 톤백 부족 (잔여 {remaining:,.0f} kg, 타 배치 점유 {len(others)}건)")

        self.tonbag_selections[batch_id] = picked
        for tb in tonbags:
            key = (batch_id, tb['id'])
            if key in self._tb_check_vars:
                self._tb_check_vars[key].set(tb['id'] in picked)
        self._update_sel_label(batch_id)
    ...
    def _clear_selection(self, batch_id):
        ...
        for tb in batch.get('_tonbags', []):
            key = (batch_id, tb['id'])
            if key in self._tb_check_vars:
                self._tb_check_vars[key].set(False)
        self._update_sel_label(batch_id)
    ...
    def _confirm_draft(self):
        ...
        CustomMessageBox.showinfo(self.win, "✅ WAIT_SCAN 전환",
                                  f"{wait_count}건 → WAIT_SCAN\n톤백 PICKED 처리 완료")
    ...
    def _build_tab3(self):
        ...
        ttk.Button(top, text="⚡ 전체 검증 실행",
                   command=self._finalize_all, style='Accent.TButton').pack(side=RIGHT, padx=4)
        ...
        self.unmatched_label = ttk.Label(out_fr, textvariable=self.unmatched_var,
                                         font=("맑은 고딕", 9), foreground='#dc2626')
        ...
        for c, w, h in [
            ...
            self.tab3_tree.heading(c, text=h)
        ...
        ttk.Button(bot, text="📦 확정건 출고 완료  ▶",
                   command=self._move_to_completed).pack(side=RIGHT, padx=4)
    ...
    def _load_out_scan_file(self):
        ...
        # ── 배치별 actual_qty 매칭 ──
        all_selected = {}  # tonbag_id → batch_id
        for batch in self.batches:
            for tb_id in batch.get('selected_tonbags', []):
                all_selected[tb_id] = batch['id']
        ...
        for rec in records:
            ...
            if tb_id in all_selected:
                bid = all_selected[tb_id]
    ...
    def _build_tab4(self):
        ...
        for c, w, h in [
            ...
            self.tab4_tree.heading(c, text=h)
        ...
        ttk.Label(fr, textvariable=self.tab4_summary_var,
                  font=("맑은 고딕", 10, "bold")).pack(padx=8, pady=5)
    ...
    def _show_audit_log_viewer(self):
        ...
        ttk.Label(row1, text="시작일:").pack(side=LEFT, padx=(12, 2))
    ...
    def _build_footer(self):
        ...
        ttk.Label(fr, textvariable=self.footer_var, font=("Consolas", 9)).pack(padx=8, pady=3)
        self._update_footer()

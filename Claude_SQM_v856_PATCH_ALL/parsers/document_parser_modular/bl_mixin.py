...
class BLMixin:
    ...
    def parse_bl(self, pdf_path: str, **kwargs) -> "Optional[BLData]":
        ...
        # v8.5.6 [PATCH3-BUG3]: total_containers / gross_weight 보강
        # BL 전체 텍스트에서 컨테이너 수 / 총중량 추출
        import re as _bl_re
        ...
        try:
            ...
            source = f"coord_table({carrier_id})" if ship_date else "coord"
            page_no = None
            if not ship_date:
                ship_date = self._extract_ship_date_by_coord(words)
    ...
    def _detect_carrier_from_words(self, words: List[Dict], full_text: str = "", explicit: str = "") -> str:
        ...
        # MAERSK: WAYBILL 단독 제외 — MSC "SEA WAYBILL" 문서에 WAYBILL이 있어 MAERSK 동점 오인 방지
        if any(k in page0_text for k in ("MAERSK", "SCAC")):
            score["MAERSK"] += 2
        if "NON-NEGOTIABLE WAYBILL" in page0_text:
            score["MAERSK"] += 3
        # MSC: 업로드 힌트 ②(MSC 헤더/브랜딩) — msc.com, MEDITERRANEAN, MEDUFP 등
        ...
        if msc_s > msk_s:
            ...
            chosen = "MAERSK"
        elif msc_s == 0 and msk_s == 0:
            chosen = ""
        # 동점: 명시적 머스크 표기 → MAERSK, 그 외 SEA WAYBILL·MSC 브랜딩이면 MSC (힌트 ②)
        elif "NON-NEGOTIABLE WAYBILL" in page0_text or re.search(r"\bMAEU\b", page0_text):
            chosen = "MAERSK"
        else:
            chosen = "MSC"
        tail = chosen if chosen else "(none)"
        logger.info(f"[BL] carrier score: {score} -> {tail}")
        return chosen

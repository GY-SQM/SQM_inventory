...
def _dbg_log(hypothesis_id: str, location: str, message: str, data: dict) -> None:
    try:
        ...
        with open("debug-934e53.log", "a", encoding="utf-8") as f:
            f.write(json.dumps(payload, ensure_ascii=False) + "\n")
    except Exception as e:
        logger.warning(f"[UI] debug log write failed: {e}")
...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def _create_dialog(self) -> None:
        ...
        # v8.4.5: 📁 폴더 선택 버튼 — 폴더 1회 선택으로 BL/PL/FA/DO 자동 감지
        ...
        self._attach_doc_tooltip(
            self.btn_folder,
            "폴더를 선택하면 BL·PL·FA·DO 파일을 자동으로 찾아 등록합니다.\n"
            "파일명에 _BL / _PL / _FA / _DO 가 포함된 PDF를 자동 감지합니다.",
        )
        ...
        self._attach_doc_tooltip(
            ...
            "🚢 선사 뱃지로 템플릿을 바꾼 뒤, PL/Invoice를 해당 선사 힌트로 다시 파싱합니다.\n"
        ...
        # v8.1.5: 선사 뱃지 클릭 → 파싱 템플릿 표(선사·단가 포함) 바로 열기 (수동 콤보 제거)
    ...
    def _select_folder(self) -> None:
        """v8.5.5 [PATCH1]: Ctrl 다중선택 1회 다이얼로그 → 드롭다운 서류 지정.

        ① 파일 탐색기 1회 오픈 (Ctrl로 복수선택 가능)
        ② 선택된 파일 목록을 드롭다운으로 서류 유형(BL/PL/FA/DO) 사용자 직접 지정
           (자동 분류 금지)
        ③ DO 미선택 시: 기존 달력 위젯으로 입항일 직접 입력 안내
        """
        _dlg = self.dialog
        _DOC_LABELS = {
            'BL':           '① BL (Bill of Loading)',
            'PACKING_LIST': '② PL (Packing List)',
            'INVOICE':      '③ FA (Invoice)',
            'DO':           '④ DO (Delivery Order)',
        }
        _DOC_KEYS = ['BL', 'PACKING_LIST', 'INVOICE', 'DO']
        ...
        paths = filedialog.askopenfilenames(
            parent=self.parent,
            title="📂 서류 선택 (Ctrl 키로 복수 선택 가능: BL · PL · FA · DO)",
            initialdir=initial_dir if initial_dir else None,
            filetypes=[("PDF files", "*.pdf *.PDF"), ("Image files", "*.png *.jpg *.jpeg"),
                       ("All files", "*.*")],
        )
        ...
        # ── ② 드롭다운 서류 지정 다이얼로그 ────────────────────
        assign_result = {}   # {doc_key: file_path}

        def _show_assign_dialog():
            """선택된 파일마다 드롭다운으로 서류 유형 지정."""
            top = tk.Toplevel(_dlg if (_dlg and _dlg.winfo_exists()) else self.parent)
            top.title("📋 서류 유형 지정")
            ...
            _is_dark = is_dark()
            _bg  = ThemeColors.get('bg_primary',    _is_dark)
            _fg  = ThemeColors.get('text_primary',  _is_dark)
            _bg2 = ThemeColors.get('bg_secondary',  _is_dark)

            top.configure(bg=_bg)

            tk.Label(top, text="각 파일의 서류 유형을 지정하세요.",
                     bg=_bg, fg=_fg,
                     font=('맑은 고딕', 10, 'bold')).pack(pady=(12, 6), padx=16)

            _combo_opts = ['(미지정)'] + [_DOC_LABELS[k] for k in _DOC_KEYS]
            _vars = []  # [(fname, StringVar)]
            ...
            for i, p in enumerate(paths):
                ...
                tk.Label(frame, text=fname, bg=_bg, fg=_fg,
                         anchor='w', width=36,
                         font=('맑은 고딕', 9)).grid(row=i, column=0, sticky='w', pady=3)
                v = tk.StringVar(value='(미지정)')
                cb = ttk.Combobox(frame, textvariable=v,
                                  values=_combo_opts,
                                  state='readonly', width=26)
                cb.grid(row=i, column=1, sticky='w', padx=(8, 0))
                _vars.append((p, v))

            # DO 안내 레이블
            _lbl_do_hint = tk.Label(
                top,
                text="💡 DO를 선택하지 않으면 입항일 직접 입력 창이 열립니다.",
                bg=_bg, fg=ThemeColors.get('accent_orange', _is_dark),
                font=('맑은 고딕', 9),
            )
            _lbl_do_hint.pack(pady=(4, 2))

            def _on_confirm():
                _label_to_key = {v: k for k, v in _DOC_LABELS.items()}
                _used_keys = set()
                _errors = []
                for fpath, sv in _vars:
                    sel = sv.get()
                    if sel == '(미지정)':
                        continue
                    doc_key = _label_to_key.get(sel)
                    if not doc_key:
                        continue
                    if doc_key in _used_keys:
                        _errors.append(f"'{sel}' 중복 지정됨 — 파일을 다시 확인하세요.")
                        continue
                    assign_result[doc_key] = fpath
                    _used_keys.add(doc_key)

                if _errors:
                    from tkinter import messagebox as _mb
                    _mb.showwarning("지정 오류", "\n".join(_errors), parent=top)
                    return
                top.destroy()
            ...
            ttk.Button(btn_frame, text="✅ 확인", command=_on_confirm).pack(side=LEFT, padx=6)
            ttk.Button(btn_frame, text="✖ 취소", command=_on_cancel).pack(side=LEFT, padx=6)
        ...
        if not assign_result:
            self._log("⚠️ 서류 지정이 취소되었습니다.")
            return

        # ── ③ 등록 + DO 미선택 시 달력 입항일 입력 ─────────────
        registered = []
        for doc_key, fpath in assign_result.items():
            fname = os.path.basename(fpath)
            self.file_paths[doc_key] = fpath
            try:
                self.file_labels[doc_key].config(
                    text=fname,
                    foreground=ThemeColors.get('text_primary', is_dark())
                )
                self.check_labels[doc_key].config(text="✅")
            except Exception:
                pass
            self._log(f"📂 {doc_key}: {fname}")
            registered.append(doc_key)

        # DO 미선택 → 달력으로 입항일 직접 입력
        if 'DO' not in assign_result:
            self._log("📋 D/O 미선택 — 입항일을 직접 입력합니다.")
            self._prompt_arrival_date_without_do()

        if registered:
            self._log(f"📁 파일 선택 완료: {len(registered)}종 ({', '.join(registered)})")
            if any(v for v in self.file_paths.values()):
                self._activate_step(0)
            self._update_parse_hint()
        else:
            self._log("⚠️ 등록된 파일이 없습니다.")

    def _prompt_arrival_date_without_do(self) -> None:
        """v8.5.5 [PATCH1]: DO 미선택 시 기존 달력 위젯으로 입항일 직접 입력."""
        _dlg = self.dialog
        _is_dark = is_dark()
        _bg = ThemeColors.get('bg_primary', _is_dark)
        _fg = ThemeColors.get('text_primary', _is_dark)
        ...
        tk.Label(
            top,
            text="D/O가 없습니다. 입항일을 직접 입력하세요.\n(CON RETURN / FREE TIME은 입항 후 설정 가능)",
            bg=_bg, fg=_fg,
            font=('맑은 고딕', 10),
            justify='center',
        ).pack(pady=(14, 8), padx=16)

        # 기존 달력 위젯 (DateEntry 또는 ttkbootstrap DateEntry)
        _date_var = tk.StringVar()
        try:
            from ttkbootstrap.widgets import DateEntry as _DE
            date_picker = _DE(top, dateformat='%Y-%m-%d', width=14)
            date_picker.pack(pady=6)
            def _get_date():
                return date_picker.entry.get()
        except Exception:
            # 폴백: ttk.Entry + 형식 안내
            tk.Label(top, text="YYYY-MM-DD 형식으로 입력",
                     bg=_bg, fg=_fg, font=('맑은 고딕', 9)).pack()
            entry = ttk.Entry(top, textvariable=_date_var, width=14,
                              font=('맑은 고딕', 11), justify='center')
            entry.pack(pady=6)
            entry.focus_set()
            def _get_date():
                return _date_var.get().strip()
        ...
        def _on_ok():
            ...
            if val:
                ...
                if _re.match(r'^\d{4}-\d{2}-\d{2}$', val):
                    # 선택된 arrival_date를 임시 속성에 보관 → 파싱 시 참조
                    self._manual_arrival_date = val
                    self._log(f"📅 입항일 수동 입력: {val}")
                    top.destroy()
                else:
                    from tkinter import messagebox as _mb
                    _mb.showwarning("형식 오류", "YYYY-MM-DD 형식으로 입력하세요.", parent=top)
            else:
                top.destroy()

        def _on_skip():
            self._log("📅 입항일 입력 건너뜀 — 나중에 수동 수정 가능")
            top.destroy()

        btn_f = tk.Frame(top, bg=_bg)
        btn_f.pack(pady=(6, 14))
        ttk.Button(btn_f, text="✅ 확인", command=_on_ok).pack(side=LEFT, padx=8)
        ttk.Button(btn_f, text="건너뜀", command=_on_skip).pack(side=LEFT, padx=8)
    ...
    def _parse_thread(self) -> None:
        ...
        try:
            ...
            # DB carrier_id가 잘못 저장된 경우 template_id에서 선사 추론
            _CARRIER_FROM_TPL = {
            ...
            # BL 먼저 파싱 → 선사 감지 → PL/INV 힌트 동적 교체
                    ...
                    if doc_type == 'PACKING_LIST':
                        ...
                        if _HAS_CANDIDATE_ENGINE:
                            ...
                            bl_result = parse_bl_with_candidate(
                                ...
                                db_carrier_id=_tpl_carrier_id,  # v8.4.5: DB 템플릿 선사 우선
                            )
                        else:
                        ...
                        if bl_result:
                            ...
                            # v7.3.9: 선사 감지 → PL/INV 힌트 동적 교체
                            # 템플릿 힌트 우선, 없으면 선사 레지스트리 힌트 적용
                            if _carrier_id and _carrier_id != 'UNKNOWN':
                                try:
                                    from features.ai.bl_carrier_registry import CARRIER_TEMPLATES
                                    ...
                                    if _ctpl:
                                        ...
                                        # 힌트 교체 (템플릿 힌트 없을 때만)
                                        if not _hint_packing and hasattr(_ctpl, 'bl_no_prompt_hint'):
                                            _hint_packing = (
                                                f"이 서류는 {_ctpl.carrier_name} 선사의 Packing List입니다. "
                                                f"BL번호 형식: {_ctpl.bl_format_hint}"
                                            )
                                        if not _hint_invoice and hasattr(_ctpl, 'carrier_name'):
                                            _hint_invoice = (
                                                f"이 서류는 {_ctpl.carrier_name} 선사의 Invoice/FA입니다."
                                            )
                                        self._log_safe(
                                            f"  🔄 선사 힌트 동적 교체: {_ctpl.carrier_name} "
                                            f"(PL힌트={'ON' if _hint_packing else 'OFF'}, "
                                            f"INV힌트={'ON' if _hint_invoice else 'OFF'})"
                                        )
            ...
            # v7.9.9 [PARSE-CONFIRM]: 파싱 완료 후 체크박스 확인 다이얼로그

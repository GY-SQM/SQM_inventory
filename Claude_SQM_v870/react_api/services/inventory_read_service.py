...
def search_inventory(
    ...
    with get_db() as db:
        ...
        rows = db.fetchall(
            ...
            f"COALESCE(i.initial_weight, 0) AS initial_weight, "
            f"COALESCE(i.mxbg_pallet, 0) AS mxbg_pallet, "
            ...
            f"COALESCE(i.customs, '') AS customs, "
            f"COALESCE(i.picked_weight, 0) AS picked_weight "
        ...
        normalized_rows = [
            InventoryRow(
                ...
                weight_mt=round(_safe_float(row.get("weight_kg")) / 1000.0, 3),
                ...
                initial_weight=_safe_float(row.get("initial_weight")),
                mxbg_pallet=_safe_int(row.get("mxbg_pallet")),
                salar_invoice_no=row.get("salar_invoice_no") or "",
                ship_date=row.get("ship_date"),
                arrival_date=row.get("arrival_date"),
                con_return=row.get("con_return"),
                free_time=_safe_int(row.get("free_time")),
                warehouse=row.get("warehouse") or "",
                customs=row.get("customs") or "",
                picked_weight=_safe_float(row.get("picked_weight")),
            )
            for row in rows
        ]
...
def get_lot_detail(lot_no: str):
    with get_db() as db:
        inventory_row = db.fetchone(
            ...
            "COALESCE(container_no, '') AS container_no, ship_date, arrival_date, "
            "con_return, COALESCE(free_time, 0) AS free_time, "
            "COALESCE(warehouse, '') AS warehouse, "
            "COALESCE(net_weight, 0) AS net_weight, "
            "COALESCE(current_weight, 0) AS current_weight, "
            "COALESCE(initial_weight, 0) AS initial_weight "
        ...
        return LotDetailResponse(
            ...
            container_no=inventory_row.get("container_no") or "",
            ship_date=inventory_row.get("ship_date"),
            arrival_date=inventory_row.get("arrival_date"),
            con_return=inventory_row.get("con_return"),
            free_time=int(inventory_row.get("free_time") or 0),
            warehouse=inventory_row.get("warehouse") or "",
            net_weight=round(float(inventory_row.get("net_weight") or 0), 3),
            current_weight=round(float(inventory_row.get("current_weight") or 0), 3),
            initial_weight=round(float(inventory_row.get("initial_weight") or 0), 3),
            status_summary=[
                LotStatusItem(

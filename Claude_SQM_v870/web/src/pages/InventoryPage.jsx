...
function StatusBadge({ status }) {
  // v864 ui_constants.py ThemeColors 기준 완전 동일 색상
  const colors = {
    AVAILABLE:         { bg: '#e8fff1', fg: '#127a3a' },  // 초록
    RESERVED:          { bg: '#fff7df', fg: '#946200' },  // 노랑
    PICKED:            { bg: '#eef4ff', fg: '#1f57b0' },  // 파랑
    OUTBOUND:          { bg: '#f3ecff', fg: '#6a35c1' },  // 보라
    SOLD:              { bg: '#f3ecff', fg: '#6a35c1' },  // 보라 (OUTBOUND 동일)
    RETURN:            { bg: '#fff0f0', fg: '#b91c1c' },  // 빨강
    DEPLETED:          { bg: '#f3f4f6', fg: '#6b7280' },  // 회색
    STAGED:            { bg: '#fef3c7', fg: '#92400e' },  // 주황
    PENDING_APPROVAL:  { bg: '#fef9c3', fg: '#854d0e' },  // 연노랑
  };
...
export default function InventoryPage({ onLotClick }) {
  ...
  return (
    <div style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 12 }}>
              <button
                onClick={() => {
                  ...
                  if (search.product_name) params.set('product_name', search.product_name);
                  if (search.keyword) params.set('keyword', search.keyword);
                  window.location.href = `/api/tools/export-lot-list?${params.toString()}`;
                }}
                style={{ padding: '4px 10px', fontSize: 11, background: '#16a34a', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
              >📥 LOT 리스트 Excel</button>
      ...
      {ctxMenu && (
        <>
          ...
          <div style={{
            ...
            <button onClick={() => { setSelectedRow(ctxMenu.row); setOutboundOpen(true); closeCtx(); }}
              style={{
      ...
      <OutboundModal
        open={outboundOpen}
        onClose={() => { setOutboundOpen(false); setSelectedRow(null); }}

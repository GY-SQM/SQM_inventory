...
export default function SettingsPage() {
  ...
  useEffect(() => {
    // Gemini 정보
    api(`${BASE_TOOLS}/gemini/config`).then(d => setGeminiInfo(d)).catch(() => {});
    // Email 설정
    api(`${BASE_TOOLS}/email/config`).then(d => { if (d.config) setEmailCfg(prev => ({ ...prev, ...d.config })); }).catch(() => {});
    // 백업 스케줄
    api(`${BASE_TOOLS}/backup/schedule`).then(d => { if (d.config) setBackupCfg(d.config); }).catch(() => {});
    // 백업 목록
    api(`${BASE_TOOLS}/backup/list`).then(d => setBackupList(d.backups || [])).catch(() => {});
    // 선사 패턴
    api(`${BASE_TOOLS}/bl-carrier/list`).then(d => setCarrierSummary(d.summary || '')).catch(() => {});
  }, []);

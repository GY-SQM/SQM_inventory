...
class BLMixin:
    ...
    CARRIER_COORD_TABLE = {
        ...
        "MSC": {
            ...
            # v8.7.0: x2 25→17 to exclude separator "-" (x=17.7%) and voyage (x=18.4%)
            "vessel": (3.0, 17.0, 29.5, 31.0),
            # 0037W/2551W 위치: x=18.4%, y=29.9% → x=18.2~22%, y=29.5~31.0%
            # v8.7.0: x1 14→18.2 to exclude separator "-" (x=17.7%)
    ...
    def parse_bl(self, pdf_path: str, **kwargs) -> "Optional[BLData]":
        ...
        import re as _bl_re
        _ct_m = _bl_re.search(r'\b([1-9]\d{0,2})\s+(?:cntrs|containers?|CNTRS)', full_text, _bl_re.IGNORECASE)  # v8.7.0: max 3 digits, exclude 4-digit years
        if not _ct_m:

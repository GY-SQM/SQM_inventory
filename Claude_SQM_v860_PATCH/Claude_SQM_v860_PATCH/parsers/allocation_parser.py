...
지원 양식:
A) Song 양식 (v2.7.0 개정): 단일 시트, 1행 타이틀, 2행 컬럼 헤더
   컬럼: Product | SAP NO | Date in stock | QTY (MT) | Lot No | WH | Customs | SOLD TO | SALE REF | GW
   구조: LOT당 본품 1행 + 샘플 1행 쌍 (Product=MIC9000 / MIC9000 sample)
B) Woo  양식 (v2.7.0 개정): 단일 시트, 1~5행 헤더, 6행 컬럼 헤더
   컬럼: Product | SAP NO | Date in stock | QTY (MT) | Lot No | WH | Customs | Export | SOLD TO | SALE REF | Balance | GW | Remark
   구조: LOT당 본품 1행 + 샘플 1행 쌍, Export=반송/일반수출
...
class AllocationParser:
    ...
    def _select_best_sheet(self, excel_path: str) -> Optional[str]:
        ...
        v2.7.0: Song 개정 양식은 단일 시트 ('Lot No', 'Product', 'Date in stock' 컬럼)
                → 다중 시트 우선 탐색 후 단일 시트 fallback
    ...
    def _extract_rows(self, df: pd.DataFrame, header: AllocationHeader) -> List[AllocationRow]:
        ...
        # v2.7.0: 'Lot No'(소문자), 'Date in stock' 등 개정 양식 컬럼명도 인식
        header_row_idx = None
    ...
    def _map_columns(self, headers: List[str]) -> Dict[str, int]:
        ...
        alias_patterns = {
            # v2.7.0: Song/Woo 개정 양식 대응 — 'Lot No', 'Date in stock', 'MIC9000 sample' 등
            ...
            'date_in_stock': ['DATE_IN_STOCK', 'DATE IN STOCK', 'INBOUND_DATE', 'INBOUND DATE',
                             ...
                             'DATE IN STOCK'],   # v2.7.0: Song/Woo 개정 컬럼명
            ...
            # v2.6.1: Export 컬럼 → export_type (반송 / 일반수출 구분)
            # v2.7.0: Woo 개정 양식 Export 컬럼 포함
            'export_type': ['EXPORT', '수출유형', '반송', 'EXPORT_TYPE', 'EXPORT TYPE'],
            # v2.7.0: Balance 컬럼 별도 저장 (Woo 개정: QTY와 별도로 Balance 컬럼 존재)
            'balance': ['BALANCE'],
            # Remark 컬럼 저장 (Song/Woo 모두)
            'remark': ['REMARK', '비고', 'NOTE', 'NOTES'],
        }

...
class ExportMixin:
    ...
    def _export_outbound_report(self, output_path: str, sale_ref: str = None,
        ...
        """
        v8.6.2: 출고 보고서 생성 (Excel + PDF).

        Detail_of_Outbound 로직:
          ① 출고될 때마다 일별로 작성 — delivery_date 기준 날짜별 시트 분리
             (날짜 필터 없으면 전체 날짜 → 날짜별 시트 생성)
          ② Sales order No별 완료 여부는 이 보고서에 표시만 (집계용)

        Args:
            output_path: 저장 경로 (.xlsx — PDF는 같은 이름으로 .pdf 자동 생성)
            sale_ref: 필터 (None이면 전체)
            outbound_date: 날짜 필터 'YYYY-MM-DD' (None이면 전체 날짜 → 날짜별 시트)
            lot_no: 특정 LOT 필터
        ...
        query += " ORDER BY s.delivery_date, s.lot_no, s.is_sample"
        ...
        # ── v8.6.2: 날짜별 그룹핑 ──
        # 출고될 때마다 일별로 작성 — delivery_date 기준
        ...
        # 출고될 때마다 일별로 작성 — delivery_date별 별도 시트
        ...
        def _write_sheet(ws_obj, day_data, sheet_date):
            """날짜별 시트 작성 공통 함수"""
            day_nw = sum(float(r.get('sold_qty_mt') or 0) for r in day_data if not r.get('is_sample'))
            day_gw = sum(float(r.get('gross_weight_kg') or 0) / 1000.0 for r in day_data if not r.get('is_sample'))
            day_ct = sum(int(r.get('ct_plt') or 0) for r in day_data if not r.get('is_sample'))

            # 헤더
            ws_obj.merge_cells('B2:F2')
            ws_obj['B2'] = '■ Outbound report'
            ws_obj['B2'].font = Font(bold=True, size=14)
            ws_obj.merge_cells('B3:C3')
            ws_obj['B3'] = f'■ Date : {sheet_date}'
            ws_obj['B3'].font = Font(bold=True, size=11)

            # 합계 (헤더 우측)
            ws_obj['K4'] = round(day_nw, 3)
            ws_obj['K4'].number_format = num_fmt_mt
            ws_obj['K4'].font = total_font
            ws_obj['L4'] = round(day_gw, 3)
            ws_obj['L4'].number_format = num_fmt_gw
            ws_obj['L4'].font = total_font
            ws_obj['M4'] = day_ct
            ws_obj['M4'].font = total_font

            # 컬럼 헤더 (행 5)
            for col_letter, title, width in columns:
                cell = ws_obj[f'{col_letter}5']
                cell.value = title
                cell.font = col_header_font
                cell.fill = col_header_fill
                cell.alignment = center
                cell.border = thin_border
                ws_obj.column_dimensions[col_letter].width = width
            ws_obj.row_dimensions[5].height = 20
            ...
            for r in day_data:
                ...
                for idx, (col_letter, _, _) in enumerate(columns):
                    cell = ws_obj[f'{col_letter}{row_num}']
            ...
            # 합계 행 (하단)
            ws_obj.merge_cells(f'B{row_num}:J{row_num}')
            cell = ws_obj[f'B{row_num}']
            cell.value = f'TOTAL  ({len([r for r in day_data if not r.get("is_sample")])}건)'
            cell.font = total_font; cell.fill = total_fill; cell.border = thin_border
            ws_obj[f'K{row_num}'] = round(day_nw, 3)
            ws_obj[f'K{row_num}'].number_format = num_fmt_mt
            ws_obj[f'K{row_num}'].font = total_font; ws_obj[f'K{row_num}'].fill = total_fill
            ws_obj[f'K{row_num}'].border = thin_border; ws_obj[f'K{row_num}'].alignment = right_align
            ws_obj[f'L{row_num}'] = round(day_gw, 3)
            ws_obj[f'L{row_num}'].number_format = num_fmt_gw
            ws_obj[f'L{row_num}'].font = total_font; ws_obj[f'L{row_num}'].fill = total_fill
            ws_obj[f'L{row_num}'].border = thin_border; ws_obj[f'L{row_num}'].alignment = right_align
            ws_obj[f'M{row_num}'] = day_ct
            ws_obj[f'M{row_num}'].font = total_font; ws_obj[f'M{row_num}'].fill = total_fill
            ws_obj[f'M{row_num}'].border = thin_border; ws_obj[f'M{row_num}'].alignment = right_align
            ws_obj.row_dimensions[row_num].height = 18
            ws_obj.freeze_panes = 'B6'
        ...
        for sheet_date in sorted_dates:
            ...
            _write_sheet(ws, day_data, sheet_date)

        # ── 전체 요약 시트 (2일 이상인 경우) ──
        if len(sorted_dates) > 1:
            ws_all = wb.create_sheet(title="ALL", index=0)
            _write_sheet(ws_all, data, f"{sorted_dates[0]} ~ {sorted_dates[-1]}")
    ...
    def _export_sales_order_dn_report(self, output_path: str, sale_ref: str = None,
                                       outbound_date: str = None, lot_no: str = None) -> str:
        """
        v8.6.2: Sales order DN 보고서 생성 (Excel + PDF).

        핵심 로직:
          ① sale_ref에 해당하는 allocation_plan 전체 LOT을 파악
          ② sold_table에서 해당 LOT이 모두 OUTBOUND로 기록됐는지 확인
          ③ 완료(is_complete=True)일 때만 DN 보고서 1회 작성
          ④ 미완료 시 → 진행 현황 경고 반환 (파일 미생성)
        """
        import openpyxl
        from openpyxl.styles import Font, Alignment, PatternFill, Border, Side

        # ── ① Sales Order 완료 여부 체크 ──
        if sale_ref:
        ...
        wb.save(output_path)
        logger.info(f"[DN보고서] Excel 저장: {output_path} ({len(data)}행)")

    ...
    def reserve_from_allocation(self, allocation_rows: list, source_file: str = '', reservation_mode: str = '') -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                # v8.6.0 [G5-N+1-FIX]: 루프 전 IN절 일괄 조회 (N+1 → 2회 쿼리로 개선)
                # Before: LOT N개 → fetchone 2N회
                # After:  LOT N개 → fetchall 2회 (IN절 배치)
                _g5_lot_list = [k for k, v in _batch_lot_qty.items() if v > 0]
                _g5_total_map: dict = {}
                _g5_already_map: dict = {}
                if _g5_lot_list:
                    _g5_ph = ",".join("?" * len(_g5_lot_list))
                    # 총 cargo 일괄 조회 (톤백 + 샘플 포함)
                    ...
                    # v8.6.0 [COUNT*500→SUM]: COUNT*500 하드코딩 제거
                    # → qty_mt*1000 직접 합산으로 500/1000kg 톤백 모두 정확히 처리
                    # v8.6.0 [STAGED포함]: STAGED/PENDING_APPROVAL도 예약 점유량에 포함

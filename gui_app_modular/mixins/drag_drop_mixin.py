...
class DragDropMixin:
    ...
    def _setup_drag_drop_alt(self) -> None:
        """v3.9.9: 드래그앤드롭 지원 (드롭 오버레이 포함)"""
        try:
            ...
            self._log("OK Drag & drop enabled (tkinterdnd2)")
            self._has_drag_drop = True
        except ImportError:
            self._has_drag_drop = False
            logger.info("Drag & drop not available (tkinterdnd2 not installed)")
    ...
    def _show_excel_import_options(self, file_path: str) -> None:
        ...
        from ..utils.ui_constants import DialogSize, ThemeColors, center_dialog

...
class AllocationDialog:
    ...
    def _on_reserve(self):
        ...
        if warnings:
            warn_msg = (
                ...
                + "\n".join(warnings[:10])
                + "\n\n현재 기본 설정(Strict ON)에서는 계속 진행해도 전체 예약이 롤백될 수 있습니다.\n계속 진행하시겠습니까?"
            )
        ...
        try:
            ...
            if result.get('success'):
                ...
                if pending_approval > 0:
                    msg += f"\n\n승인대기 적재: {pending_approval}건\n(출고 > ✅ Allocation 승인 대기에서 승인 후, 📌 예약 반영 실행)"
    ...
    def _show_reservation_status(self):
        ...
        try:
            ...
            ttk.Entry(filter_frm, textvariable=customer_var, width=18).grid(row=0, column=1, padx=4, pady=4, sticky="w")
            ...
            def _render_rows(rows):
                ...
                lot_mode_open = sum(
                    ...
                    for r in rows
                    if str(r.get('status', '')).strip().upper() == 'RESERVED'
                )
                summary_var.set(

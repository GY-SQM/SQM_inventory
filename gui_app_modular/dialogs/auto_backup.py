                    ...
                    if validation['valid']:
                        ...
                        if result.get('success'):
                            ...
                            if retry_validation['valid']:
                                self._log(f"✅ 백업 재시도 성공")
                            else:
                                self._log(f"❌ 백업 재시도도 실패")
...
class AutoBackupSettingsDialog:
    ...
    def _update_status(self) -> None:
        ...
        if self.enabled_var.get():
            self.status_label.config(text="✅ 자동 백업이 활성화됩니다", foreground='green')
        else:
            self.status_label.config(text="⛔ 자동 백업이 비활성화됩니다", foreground='gray')

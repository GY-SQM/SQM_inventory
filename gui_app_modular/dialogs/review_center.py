# -*- coding: utf-8 -*-
"""review_center.py - Phase B 검수센터(Review Center)

목표(MVP+B2):
- 원본 PDF 보기(PyMuPDF)
- 드래그(ROI)로 텍스트 추출(텍스트 PDF 우선)
- 캡처(클립보드 이미지) 붙여넣기 + OCR 폴백
- 사용자가 정답 값 입력 → 미리보기 데이터에 즉시 반영
- 적용 후 QC 재검증 콜백(있으면 호출)
- ROI 규칙 저장(SQLite) : review_rules

주의:
- OCR은 pytesseract를 우선 시도하며, 시스템에 tesseract 엔진이 없으면 실패할 수 있음.
"""
...
class ReviewCenterDialog:
    ...
    def _build_ui(self):
        ...
        ttk.Label(left, textvariable=self.var_status, foreground="#555").pack(anchor="w", pady=(10, 0))

...
class OutboundHandlersMixin:
    ...
    def _on_simple_outbound(self) -> None:
        """Simple outbound dialog - enter LOT and quantity (v4.0.3: UI 분리, v8.7.0: SRP 분해)"""
    ...
    # ------------------------------------------------------------------
    # _on_simple_outbound sub-methods (v8.7.0 [SRP])
    # ------------------------------------------------------------------

    @staticmethod
    def _sob_normalize_tonbag_key(raw):  # v8.7.0 [SRP]
        """톤백 키 정규화."""
        if raw is None:
            return ""
    ...
    @staticmethod
    def _sob_hash_file(path):  # v8.7.0 [SRP]
        """SHA-256 해시 계산."""
        h = hashlib.sha256()
    ...
    def _sob_attach_proof_doc(self, st):  # v8.7.0 [SRP]
        """근거문서 첨부 처리."""
    ...
    def _sob_preview_proof_doc(self, st):  # v8.7.0 [SRP]
        """근거문서 미리보기."""
    ...
    def _sob_parse_out_scan_file(self, path):  # v8.7.0 [SRP]
    ...
    def _sob_load_out_scan_file(self, st):  # v8.7.0 [SRP]
        """OUT 스캔 파일 불러오기 및 매칭."""
    ...
    def _sob_on_preview(self, st):  # v8.7.0 [SRP]
    ...
    def _sob_prefetch_lot_data(self, lot_requests):  # v8.7.0 [SRP]
    ...
    def _sob_preview_single_lot(self, st, lot_no, qty_kg, inv_map, tb_map, warnings):  # v8.7.0 [SRP]
    ...
    def _sob_on_execute(self, st):  # v8.7.0 [SRP]
    ...
    def _sob_collect_allocation_items(self, st, customer, sale_ref):  # v8.7.0 [SRP]
    ...
    def _sob_run_outbound_engine(self, st, allocation_items, customer, sale_ref):  # v8.7.0 [SRP]
    ...
    def _sob_build_aux_widgets(self, st, attach_cmd, preview_cmd, load_scan_cmd):  # v8.7.0 [SRP]
    ...
    def _sob_build_buttons(self, st, on_preview, on_execute, on_get_selected_lot, HAS_TTKBOOTSTRAP):  # v8.7.0 [SRP]
        """메인 버튼 및 키바인딩 생성."""
    ...
    def _sob_on_get_selected_lot(self, st):  # v8.7.0 [SRP]

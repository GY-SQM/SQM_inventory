...
class LogTabMixin:
    ...
    def _setup_log_tab(self) -> None:
        ...
        from ..utils.constants import tk, ttk, VERTICAL, BOTH, LEFT, RIGHT, X, Y
        from ..utils.ui_constants import Spacing, ThemeColors, FontScale
    ...
    def _configure_log_tags(self) -> None:
        ...
        if is_dark:
            ...
            self.log_text.tag_configure('success', foreground='#50C878')  # 밝은 녹색
            self.log_text.tag_configure('warning', foreground='#FFB347')  # 밝은 주황
            self.log_text.tag_configure('error', foreground='#FF6B6B')    # 밝은 빨강
            self.log_text.tag_configure('timestamp', foreground='#A0A0A0')
        else:
            # 라이트 모드 색상

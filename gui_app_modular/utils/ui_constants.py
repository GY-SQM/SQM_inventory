...
class Spacing:
    ...
    # 용도별 권장값
    class Padding:
        DIALOG = 20         # v8.7.0: 16→20 다이얼로그 내부
        FRAME = 12          # v8.7.0: 8→12 일반 프레임
        LABELFRAME = 20     # v8.7.0: 16→20 LabelFrame 내부
        BUTTON_GROUP = 10   # v8.7.0: 8→10 버튼 사이
        SECTION = 28        # v8.7.0: 24→28 섹션 사이

    # ── v8.1.8: 탭 레이아웃 표준 상수 ──────────────────────────────
    # 모든 탭에 동일하게 적용하여 시각적 일관성 확보
    class Tab:
        """탭 내부 레이아웃 표준값 (Grid 기반). v8.7.0: 여백 대폭 확대."""
        # 탭 전체 가로 패딩
        OUTER_PADX   = 24       # v8.7.0: 6→24 (넉넉한 좌우 여백)
        # 헤더 (make_tab_header)
        HEADER_H     = 44       # v8.7.0: 38→44
        HEADER_PADY  = (8, 12)  # v8.7.0: (0,4)→(8,12) 헤더 상하 여백
        # 버튼바
        BTN_BAR_PADX = (24, 24) # v8.7.0: (6,6)→(24,24)
        BTN_BAR_PADY = (6, 10)  # v8.7.0: (0,4)→(6,10)
        BTN_GAP      = 8        # v8.7.0: 4→8 버튼 사이 간격
        # 필터/검색바 (버튼바 아래)
        FILTER_PADY  = (6, 10)  # v8.7.0: (0,4)→(6,10)
        # 트리뷰 컨테이너
        TREE_PADX    = (24, 24) # v8.7.0: (6,6)→(24,24)
        TREE_PADY    = (6, 16)  # v8.7.0: (0,6)→(6,16)
...
class ThemeColors:
    ...
    LIGHT = {
    # ── v8.7.0 Pro Light — 딥 네이비 × 아이스 화이트 ──────────────────
    ...
    'text_muted':     '#8098b8',   # 스틸 블루 (v8.7.0: 대비 강화 #364e6e→#8098b8)
    ...
    'search_placeholder': '#6888a8',  # v8.7.0: 대비 강화 (#364e6e→#6888a8)
    ...
    def is_dark_theme(cls, theme_name: str) -> bool:
        """다크 테마 여부 확인"""
        # v8.7.0: 다크 테마 목록 확장 (PRO DARK 계열 포함)
        _DARK_SET = {
    ...
    def configure_tags(cls, tree, is_dark: bool = False):
        ...
        tree.tag_configure('sold',
                          background=ob['bg_dark' if is_dark else 'bg_light'],
                          foreground=ob['fg_dark' if is_dark else 'fg_light'])
        # v8.7.0: depleted 대비 강화 (기존 #aaa/#f0f0f0 → 읽기 불가)
...
def set_global_theme(theme_name: str) -> None:
    ...
    _GLOBAL_THEME   = str(theme_name or 'darkly')  # v8.7.0: 기본 darkly

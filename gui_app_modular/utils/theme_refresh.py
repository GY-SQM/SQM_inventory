...
def _refresh_native_widget(widget, colors: dict) -> None:
    ...
    if not current_fg:
        return
    fg_is_dark = _is_dark_color(current_fg) if current_fg.startswith('#') else (current_fg.lower() in ('black', '#000000', ''))
    needs_fix = False
    if is_dark and fg_is_dark:
        needs_fix = True
    elif not is_dark and not fg_is_dark and current_fg.lower() in ('white', '#ffffff', '#fff', '#f0f0f0', '#e0e0e0', '#ecf0f1'):
        needs_fix = True
    if needs_fix:
        try:

...
class SQMInventoryApp:
    ...
    def _setup_ui(self) -> None:
        ...
        # v8.6.3: 시작 시 대시보드 탭 표시 — 재고 현황 한눈에
        try:
            self.notebook.select(self.tab_dashboard)
            self._active_tab_key = 'dashboard'
            if hasattr(self, '_highlight_active_tab'):
                self._highlight_active_tab()
            # 대시보드 데이터 로드 (UI 렌더링 후 실행)
            self.root.after(800, lambda: self._refresh_dashboard()
                            if hasattr(self, '_refresh_dashboard') else None)
        except (ValueError, TypeError, AttributeError) as _e:
            logger.debug(f"초기 탭 선택: {_e}")

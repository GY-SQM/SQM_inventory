...
class DashboardTabMixin:
    ...
    def _setup_dashboard_tab(self) -> None:
        ...
        try:
            ...
            logger.debug(f"[UI] dashboard product footer: {e}")
    ...
    def _create_dashboard_card(self, parent, title: str, value: str, color: str, fonts=None) -> 'ttk.Frame':
        ...
        # hover 효과: 카드 배경 밝아짐
        _hover_bg = ThemeColors.get('bg_hover', is_dark())

        def _on_card_enter(e):
            for w in (inner, content, title_label, value_label, sub_label):
                try:
        ...
        for w in (outer, inner, content, title_label, value_label, sub_label):

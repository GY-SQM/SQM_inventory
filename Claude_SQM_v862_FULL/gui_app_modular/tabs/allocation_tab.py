...
ALLOCATION_LOT_COLUMNS = [
    ('expand', '☐', 56, 'center'),   # v8.1.8: 열 너비 44→56, 헤더도 ☐ 아이콘으로 직관화
...
class AllocationTabMixin:
    ...
    def _setup_allocation_tab(self) -> None:
        ...
        try:
            ...
            _alloc_style.configure('Alloc.Treeview', rowheight=36,
                                   font=('맑은 고딕', 10))
        ...
        try:
            ...
            self.tree_allocation.tag_configure('alloc_subtotal', font=('맑은 고딕', 9, 'bold'))
        except Exception as e:
    ...
    def _on_allocation_cancel_to_available(self) -> None:
        ...
        if sel:
            ...
            if not CustomMessageBox.askyesno(
                ...
                return
            # v8.4.1 [BUG-D]: 인자 없이 cancel_reservation() 호출 시 HARD-STOP 방지
            # RESERVED 상태의 모든 plan_id를 가져와서 전달
            all_reserved = engine.db.fetchall(
                "SELECT id FROM allocation_plan WHERE status = 'RESERVED'"
            ) or []
            all_plan_ids = [row['id'] if isinstance(row, dict) else row[0] for row in all_reserved]
            if not all_plan_ids:
                CustomMessageBox.showinfo(root, "취소 완료", "취소할 배정이 없습니다.")
                self._safe_refresh()
                return
            r = engine.cancel_reservation(plan_ids=all_plan_ids)
            total = r.get('cancelled', 0)
            CustomMessageBox.showinfo(root, "취소 완료", f"{total}건 취소되었습니다. (→ 판매가능)")
        self._safe_refresh()

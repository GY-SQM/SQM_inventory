...
def get_engine():
    ...
    if _engine is None:
        try:
            ...
            raise HTTPException(status_code=500, detail="엔진 초기화 실패 — 서버 로그를 확인하세요")
    return _engine
...
def get_dashboard(user: dict = Depends(require_role('viewer'))):
    ...
    try:
        ...
        logger.error(f"[API] 대시보드 조회 오류: {e}")
        raise HTTPException(500, "대시보드 데이터 조회 실패")
...
def get_alerts(user: dict = Depends(require_role('viewer'))):
    ...
    try:
        ...
        rows = engine.db.fetchall("""
            ...
            GROUP BY lot_no HAVING COUNT(*) >= ?
            ORDER BY cnt DESC LIMIT 10
        """, (RETURN_ALERT_THRESHOLD,))
        for r in (rows or []):
            rd = dict(r) if not isinstance(r, dict) else r
            alerts.append(AlertItem(
                icon='🔄', message=f"{rd['lot_no']}: 반품 {rd['cnt']}회 — 품질 점검 필요",
                severity='warning', lot_no=rd['lot_no']))
    except Exception as e:
...
def get_inventory(
    ...
    rows = engine.db.fetchall(f"""
        ...
        ORDER BY i.lot_no LIMIT ?
    """, tuple(params))
    # [H3] isinstance 제거 — dict 변환 통일
    return [InventoryItem(**(dict(r) if not isinstance(r, dict) else r))
            for r in (rows or [])]
...
def get_lot_detail(lot_no: str, user: dict = Depends(require_role('viewer'))):
    ...
    # [H3] isinstance 제거 — dict 변환 통일
    lot_d = dict(lot_row) if not isinstance(lot_row, dict) else lot_row
    ...
    tb_list = [TonbagItem(**(dict(t) if not isinstance(t, dict) else t))
               for t in (tonbags or [])]
    ...
    mv_list = [dict(m) if not isinstance(m, dict) else m
...
def get_return_history(
    ...
    rows = engine.db.fetchall(f"""
        ...
        FROM return_history r WHERE {where_sql}
...
def api_inbound(req: InboundRequest, user: dict = Depends(require_role('operator'))):
    ...
    try:
        ...
        logger.error(f"[API] 입고 처리 오류: {e}")
        raise HTTPException(500, "입고 처리 중 오류가 발생했습니다")
...
def api_outbound(req: OutboundRequest, user: dict = Depends(require_role('operator'))):
    ...
    try:
        ...
        logger.error(f"[API] 출고 처리 오류: {e}")
        raise HTTPException(500, "출고 처리 중 오류가 발생했습니다")
...
def api_return(req: ReturnRequest, user: dict = Depends(require_role('operator'))):
    ...
    try:
        ...
        logger.error(f"[API] 반품 처리 오류: {e}")
        raise HTTPException(500, "반품 처리 중 오류가 발생했습니다")
...
async def websocket_dashboard(ws: WebSocket):
    ...
    [C4] 인증: query parameter ?token=<JWT> 필수.
    예: ws://host/ws/dashboard?token=eyJ...
    """
    # [C4] WebSocket 토큰 검증
    token = ws.query_params.get("token")
    if not token:
        await ws.close(code=4001, reason="인증 필요: ?token=<JWT>")
        return
    try:
        ...
        payload = None
    if not payload:
        await ws.close(code=4001, reason="유효하지 않거나 만료된 토큰")
        return
...
def api_get_audit_logs(
    ...
    try:
        ...
        logger.error(f"[API] 감사 로그 조회 오류: {e}")

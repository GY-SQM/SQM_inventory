...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def _select_folder(self) -> None:
        """v8.4.5 Rev2: 폴더 지정 후 BL→PL→FA→DO 순서로 파일 직접 선택.

        ① 안내 팝업 표시
        ② 폴더 선택 (초기 경로로 사용)
        ③ 같은 폴더에서 BL → PL → FA → DO 순서로 각각 파일 선택
           (DO는 선택 안 해도 됨 — 선택사항)
        """
        initial_dir = ""
        try:
        ...
        try:
            ...
            msg = (
                "📁 폴더에서 입고 서류를 순서대로 선택합니다.\n\n"
                "  ① Bill of Loading (BL)  — 필수\n"
                "  ② Packing List (PL)     — 필수\n"
                "  ③ Invoice / FA          — 필수\n"
                "  ④ Delivery Order (DO)   — 선택 (없으면 취소)\n\n"
                "[확인]을 누르면 파일 선택 창이 순서대로 열립니다."
            )
            ok = messagebox.askokcancel(
                "입고 서류 선택 순서 안내",
                msg,
                parent=_dlg if (_dlg and _dlg.winfo_exists()) else None,
            )
            if not ok:
                return
        except Exception as e:
        ...
        def _show():
            try:
                ...
                logger.warning(f'[UI] onestop_inbound: {e}')
        _filetypes = [("PDF files", "*.pdf *.PDF"), ("All files", "*.*")]

        _select_order = [
            ('BL',           '① Bill of Loading (BL) 선택',     True),
            ('PACKING_LIST', '② Packing List (PL) 선택',         True),
            ('INVOICE',      '③ Invoice / FA 선택',              True),
            ('DO',           '④ Delivery Order (DO) 선택 (선택사항)', False),
        ]

        registered = []

        for doc_type, title, required in _select_order:
            _hide()
            ...
            if not path:
                if required:
                    # 필수 서류 취소 → 지금까지 선택된 것만 유지하고 종료
                    self._log(f"⚠️ {title} 취소 — 선택된 파일까지만 등록합니다.")
                    break
                else:
                    # DO는 선택사항 — 건너뜀
                    self._log("📋 D/O 선택 건너뜀")
                    continue

            # 선택한 폴더를 다음 파일 열기 위치로 유지
            try:
                selected_dir = os.path.dirname(path)
                if selected_dir and os.path.isdir(selected_dir):
                    initial_dir = selected_dir

...
# ── 싱글톤 DB: 서버 수명 동안 1회만 생성 ──────────────────────────
_shared_db: SQMDatabase | None = None


def _get_shared_db() -> SQMDatabase:
    """싱글톤 DB 반환. 최초 호출 시 1회만 생성."""
    global _shared_db
    if _shared_db is None:
        _shared_db = SQMDatabase()
        logger.info("DB singleton created")
    return _shared_db


@contextmanager
def get_db() -> Generator[SQMDatabase, None, None]:
    """DB 연결 (싱글톤). 매번 새로 만들지 않고 공유 인스턴스 재사용."""
    yield _get_shared_db()

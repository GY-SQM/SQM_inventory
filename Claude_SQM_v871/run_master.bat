...
set "CLAUDE=C:\Users\kidon\AppData\Roaming\npm\claude.cmd"
...
start "ClaudeCode" cmd /c "chcp 65001 > nul && cd /d "%PROJECT%" && "%CLAUDE%" --dangerously-skip-permissions -p "MASTER.md 파일을 읽고 S1부터 S6까지 중단 없이 전부 실행하라. 이미 완료된 단계는 건너뛰어라."
timeout /t 3 > nul
...
:direct
echo.
"%CLAUDE%" --dangerously-skip-permissions -p "MASTER.md 파일을 읽고 S1부터 S6까지 중단 없이 전부 실행하라. 이미 완료된 단계는 건너뛰어라."
goto end

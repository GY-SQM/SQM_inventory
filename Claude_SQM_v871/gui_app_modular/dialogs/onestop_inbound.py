...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def _select_file(self, doc_type: str):
        ...
        try:
            ...
            logger.warning(f"초기 폴더 계산 무시: {e}")
        ...
        try:
            ...
            logger.warning("[UI] dialog withdraw before file picker failed: %s", e)
            _dlg = None
        ...
        try:
            ...
            logger.warning(f"선택 폴더 저장 무시: {e}")
    ...
    def _parse_thread(self) -> None:  # v8.6.4 [SRP] refactored — P2: InboundParser 위임
        """백그라운드 파싱 — InboundParser에 파싱/병합 위임"""
        try:
            self._cross_check_result = None

            # P2: InboundParser 인스턴스 생성 (UI 콜백 연결)
            _inbound_parser = InboundParser(
            ...
            # UI: BL 선사 뱃지 + 재파싱 버튼 (UI 의존 부분은 다이얼로그에 유지)
            self._pt_handle_bl_ui_updates(bl_result, ctx)

            # 서류 하나 파싱 직후 미리보기 실시간 반영
    ...
    def _pt_handle_bl_ui_updates(self, bl_result, ctx: dict) -> None:
        """BL 파싱 후 UI 업데이트 (선사 뱃지, 재파싱 버튼, 템플릿 자동 매칭)."""
        if not bl_result:
            return
    ...
    def _merge_results(self, invoice, pl, bl, do) -> list:
        """4종 파싱 결과를 18열 미리보기 데이터로 병합 — P2: InboundParser 위임"""
        self._edited_rows = set()
        self._undo_stack = []
        self._redo_stack = []
        self._update_undo_redo_buttons()

        _engine_db = getattr(self.engine, 'db', None)
        _inbound_parser = InboundParser(log_fn=self._log_safe)
        self.preview_data = _inbound_parser.merge_results(
            invoice, pl, bl, do, engine_db=_engine_db
        )
        return
    ...
    def _has_required_docs(self) -> bool:
        """필수 서류 3종 확인 — P2: InboundValidator 위임"""

...
# v6.12 Addon-G: 하드코딩 제거
from engine_modules.constants import DEFAULT_TONBAG_WEIGHT
TONBAG_WEIGHT_KG = DEFAULT_TONBAG_WEIGHT  # fallback (DB weight NULL 시)
...
class PickingEngine:
    ...
    def process(self, parsed: dict, source_file: str = "") -> dict:
        ...
        try:
            ...
            is_large = total_items >= 60  # 60LOT 이상 = 대용량
            if is_large:
                logger.info(f"[PickingEngine] 대용량 처리 시작: {total_items}개 아이템")
            ...
            for idx, item in enumerate(items):
                ...
                # 대용량 진행률 로깅 (20건 단위)
                if is_large and (idx + 1) % 20 == 0:
                    logger.info(
                        f"[PickingEngine] 진행: {idx+1}/{total_items} "
                        f"(PICKED:{result['picked']}, 샘플:{result['sample_picked']})"
                    )
            ...
            logger.info(
                ...
                f"부분:{len(result['partial_lots'])}개"
            )
    ...
    def _process_normal(
        ...
        for tb in tonbags:
            ...
            self.db.execute(
                ...
                SET status = 'PICKED', picked_date = datetime('now'),
                    pick_ref = ?, picking_no = ?, updated_at = datetime('now')
                WHERE id = ?
                """,

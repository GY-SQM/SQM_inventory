...
# Treeview Heading (다크/라이트 분기) — 통일 팔레트
HEAD_DARK = {'bg': '#1e293b', 'fg': '#e2e8f0', 'hover': '#334155'}
HEAD_LIGHT = {'bg': '#f1f5f9', 'fg': '#1e293b', 'hover': '#e2e8f0'}

# 상태별 색상 (다크/라이트) — 통일 팔레트
S_DARK = {
    'available': ('#065f46', '#6ee7b7'),   # 에메랄드
    'reserved':  ('#92400e', '#fcd34d'),   # 골드
    'picked':    ('#5b21b6', '#c4b5fd'),   # 라벤더
    'shipped':   ('#1e3a5f', '#93c5fd'),   # 스카이블루
    'depleted':  ('#111827', '#64748b'),   # 뮤트 (통일)
    'stripe':    ('#0f1a2e', '#e2e8f0'),   # 통일 텍스트
    'odd':       ('#0f1a2e', '#e2e8f0'),
    'even':      ('#0b1120', '#e2e8f0'),
}
S_LIGHT = {
    'available': ('#ecfdf5', '#065f46'),
    'reserved':  ('#fffbeb', '#78350f'),
    'picked':    ('#fef2f2', '#991b1b'),
    'shipped':   ('#eff6ff', '#1e3a5f'),
    'depleted':  ('#f1f5f9', '#94a3b8'),
    'stripe':    ('#f8fafc', '#1e293b'),
    'odd':       ('#f8fafc', '#1e293b'),
    'even':      ('#ffffff', '#1e293b'),
}
...
def apply_colorful_overrides(app):
    ...
    try:
        ...
        # 2. Heading 이중 보험 (다크/라이트 분기)
        HEAD = HEAD_DARK if dark else HEAD_LIGHT
        ...
        if dark:
            ...
            style.configure('Treeview',
                ...
                foreground='#1e293b', rowheight=30,
                font=('맑은 고딕', 11))
            style.map('Treeview',
                background=[('selected', '#dbeafe')],
                foreground=[('selected', '#1e3a5f'), ('!selected', '#1e293b')])

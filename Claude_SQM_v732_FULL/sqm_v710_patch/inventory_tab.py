...
INVENTORY_COLUMNS = [
    ...
    # v7.1.0 [UI-COUNT-1]: TONBAG 개수 컬럼 기본 표시 ON + 직관적 헤더
    # 운영자가 "지금 몇 개가 어느 상태인지" 즉시 파악 가능
    ('tb_avail',           '↓Avail개',       62, 'center', True),
    ('tb_reserved',        '↓Resv개',        62, 'center', True),
    ('tb_picked',          '↓Pick개',        62, 'center', True),
    ('tb_sold',            '↓Sold개',        62, 'center', True),
...
class InventoryTabMixin:
    ...
    def _update_inv_filter_values(self, inventory) -> None:
        ...
        try:
            ...
            cnt_sold = sum(1 for i in inventory if (i.get('status') or '') == STATUS_SOLD)
            status_values = [

...
class OutboundMixin(InventoryBaseMixin):
    ...
    def cleanup_expired_staged_allocations(self, days_old: int = 7) -> Dict:
        ...
        try:
            ...
            with self.db.transaction('IMMEDIATE'):
                cur = self.db.execute("""
                    UPDATE allocation_plan
                       SET workflow_status = ALLOC_WF_REJECTED,
                           rejected_reason = 'AUTO_EXPIRE_AFTER_7DAYS',
                           approved_by     = 'system_cleanup',
                           approved_at     = ?
                     WHERE status          = ALLOC_STAGED
                       AND workflow_status = ALLOC_WF_PENDING
                       AND created_at      < ?
                """, (now, cutoff))
                cnt = cur.rowcount if hasattr(cur, 'rowcount') else 0
            result['success']  = True
    ...
    def cancel_outbound_tonbag(self, lot_no: str, sub_lt: int) -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                # v6.9.3 [RT-FIX]: SOLD 상태 반품 시 sold_table / picking_table 정리
                was_sold = (tonbag['status'] == STATUS_SOLD)
                if was_sold:
                    try:
                        self.db.execute(
    ...
    def _recalc_lot_status(self, lot_no: str) -> None:
        """LOT status 재계산 — v6.8.6: 중복 쿼리 제거 (2→1회)
        ★ v6.8.5 설계 원칙 (기동님 확정)
        판정 우선순위:
         1) AVAILABLE 톤백 1개라도 있으면 → LOT = AVAILABLE
         2) AVAILABLE 없고 PICKED 있으면  → LOT = PICKED
         3) AVAILABLE 없고 RESERVED 있으면→ LOT = RESERVED
         4) 전체 톤백이 모두 SOLD일 때만  → LOT = SOLD
         5) 톤백 없거나 무게 0             → DEPLETED
        """
        lot = self.db.fetchone(
        ...
        _sold_cnt     = _cnt_map.get(STATUS_SOLD,      0)

        # ★ v6.8.7: PARTIAL 도입 (B안 — SOLD+AVAILABLE 혼재 시)
        # 판정 우선순위:
        #  1) 전량 AVAILABLE              → AVAILABLE
        #  2) SOLD + AVAILABLE 혼재       → PARTIAL  ← 신규
        #  3) 전량 SOLD                   → SOLD
        #  4) AVAILABLE 없고 PICKED 있음  → PICKED
        #  5) AVAILABLE 없고 RESERVED     → RESERVED
        #  6) 톤백 없거나 무게 0           → DEPLETED
        if _avail_cnt > 0 and _sold_cnt == 0:
            # 전량 AVAILABLE (RESERVED/PICKED 포함 가능)
            new_status = STATUS_AVAILABLE
        elif _avail_cnt > 0 and _sold_cnt > 0:
            # SOLD + AVAILABLE 혼재 → PARTIAL
            new_status = STATUS_PARTIAL
        elif _total_cnt > 0 and _sold_cnt >= _total_cnt:
            # 전량 SOLD
            new_status = STATUS_SOLD
        elif 'SHIPPED' in _cnt_map:
            new_status = 'SHIPPED'
        elif _picked_cnt > 0:
                ...
                for d in gate1.get('lot_details', []):
                    ...
                    if lot and pk_cnt > 0 and rv_cnt > pk_cnt:
                        ...
                        for ep in (excess_plans or []):
                            try:
                                self.db.execute(
                                    "UPDATE allocation_plan SET status='ALLOC_CANCELLED', "

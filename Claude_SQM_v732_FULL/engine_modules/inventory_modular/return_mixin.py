...
from engine_modules.constants import (
    STATUS_RESERVED, STATUS_SOLD,
    # STATUS_OUTBOUND, STATUS_RETURN: 문자열 리터럴로 직접 사용 (하위호환 명시)
)
import json
import logging
import sqlite3
    ...
    def process_return(self, return_data: list,
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                # v6.9.6 [RT-09]: 반품 후 재입고 위치(location) 없음 WARNING
                # 반품된 톤백이 창고 어느 위치로 돌아가는지 기록 필요
                for _ret_d in result['details']:
                    _loc_chk = self.db.fetchone(
                        "SELECT location FROM inventory_tonbag "
                        "WHERE lot_no=? AND sub_lt=?",
                        (_ret_d['lot_no'], _ret_d['sub_lt'])
                    ) if 'sub_lt' in _ret_d else None
                    _loc_val = str(_loc_chk.get('location') or '').strip() if _loc_chk else ''
                    if not _loc_val:
                        _rt09_warn = (

    ...
    def fix_lot_status_integrity(self) -> Dict:
        ...
        try:
            ...
            for _r in needs_avail:
                ...
                # v6.8.7: SOLD 잔존 여부에 따라 PARTIAL or AVAILABLE
                _sold_cnt_fix = self.db.fetchone(
                    "SELECT COUNT(*) AS cnt FROM inventory_tonbag "
                    "WHERE lot_no=? AND status='SOLD'", (_lot,)
                )
                _sc_fix = int(_sold_cnt_fix.get('cnt',0) if isinstance(_sold_cnt_fix,dict) else (_sold_cnt_fix[0] if _sold_cnt_fix else 0))
            ...
            for _r in needs_sold:
                ...
                detail = f"{_lot}: AVAILABLE→SOLD (전체 {_tc}개 출고)"
                result['details'].append(detail)
                result['fixed'] += 1
                logger.info(f"[fix_integrity] {detail}")
    ...
    def reserve_from_allocation(self, allocation_rows: list, source_file: str = '', reservation_mode: str = '') -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                # v7.1.2 [G5-BATCH-SUM]: 배치 내 동일 LOT 합산 사전 차단
                # 같은 파일에 LOT-A 5t + LOT-A 6t = 11t > 10t 케이스를
                # 루프 진입 전에 미리 감지하여 명확한 에러 메시지 제공
                _batch_lot_qty: dict = {}  # lot_no → 배치 내 누적 qty_mt
                for _ba in (allocation_rows or []):
                    _bln = (normalize_lot(_alloc_val(_ba, 'lot_no')) or '').strip()
                    _bqt = float(_alloc_val(_ba, 'qty_mt') or 0)
                    if _bln:
                ...
                for _bln, _bqt_sum in _batch_lot_qty.items():
                    ...
                    # 재고 cargo 가용량 조회 (샘플 제외)
                    _bcargo_row = self.db.fetchone(
                        "SELECT COALESCE(SUM(weight),0) AS cargo_kg "
                        "FROM inventory_tonbag "
                    ...
                    _bcargo_kg = float(
                        (_bcargo_row.get('cargo_kg') if isinstance(_bcargo_row, dict) else _bcargo_row[0])
                        if _bcargo_row else 0
                    )
                    if _bcargo_kg > 0 and _bqt_kg > _bcargo_kg + 0.5:
                        _g5_msg = (
                            f"[G5-BATCH-SUM] 배치 내 동일 LOT 초과: {_bln} "
                            f"배치합계={_bqt_sum:.3f}t({_bqt_kg:.0f}kg) > "
                            f"가용cargo={_bcargo_kg:.0f}kg — "
                            f"동일 LOT 중복 행 합계가 재고를 초과합니다"
                        )
                ...
                if _g5_errors:
                    # G5 오류 발견 시 전체 배치 즉시 Hard Stop
                    result['success'] = False
                    result['errors'].insert(0, f"[G5-HARD-STOP] 배치 내 LOT 중복 초과 {len(_g5_errors)}건 — 전체 배치 차단")
                ...
                for line_no, alloc in enumerate(allocation_rows, start=1):
                    ...
                    _raw_customer = str(_alloc_val(alloc, 'sold_to') or _alloc_val(alloc, 'customer') or '').strip()
                    # [E] v6.8.3: 고객사명 정규화 — CATL/CATL KOREA 혼용 등 oOTAL MATCH
                    try:
# [v6.8.6 top-level import로 이동]                         from engine_modules.constants import normalize_customer
                        customer = normalize_customer(_raw_customer)
                        if customer != _raw_customer and _raw_customer:
                            logger.debug(
                                f"[E normalize_customer] '{_raw_customer}' → '{customer}'"
                            )
                    except Exception:
                    ...
                    if not is_sample_req and qty_mt > 0:
                        ...
                        _ = qty_mt <= 0.01 + 1e-9  # is_sample_req: 향후 샘플 검증용
                    ...
                    # ── v6.9.3 [AL-FIX-1~4] + v6.9.8 [AL-09]: 입력 무효 HARD-STOP ──────
                    # AL-09: qty_mt = 0 (정확히 0) → 빈 행/오입력 명확 에러
                    if qty_mt == 0:
                    ...
                    if qty_mt < 0:
                        ...
                        continue
                    # AL-FIX-3: customer(sold_to) 공란 → 출고 귀속 불명
                    if not customer:
                        ...
                        continue
                    # AL-FIX-4: sale_ref 공란 → 출고 추적 불가
                    if not sale_ref:
                        msg = (f"[INVALID_SALE_REF] LOT {lot_no}: sale_ref가 비어 있음 "
                               f"(판매참조번호 지정 필수)")
                        logger.warning(msg)
                        result['errors'].append(msg)
                        _build_error_detail(line_no, "INVALID_SALE_REF", msg, lot_no, customer, qty_mt)
                        continue
                    # ─────────────────────────────────────────────────────────────────

                    # v6.7.5 [A]: Allocation Row 충돌 차단
                    # 동일 (lot_no, customer, sale_ref, outbound_date) 조합이
                    # 활성 상태(STAGED/RESERVED/PENDING_APPROVAL)로 이미 존재하면 ALLOC_CONFLICT
                    try:
                        ...
                        if _conflict_row:
                            ...
                            continue  # 해당 행 스킵 — 기존 레코드 보존
                    except Exception as _ce:
                        logger.debug(f"[v6.7.5] 충돌 체크 스킵 (DB 오류): {_ce}")

                    # v6.9.8 [AL-06]: sale_ref 전체 중복 HARD STOP (LOT/모드 무관)
                    # 동일 sale_ref가 RESERVED/PENDING_APPROVAL 상태로 이미 존재하면 즉시 차단
                    # 기존 LOT_MODE_DUP(tonbag_id IS NULL만)보다 강화 → 전체 적용
                    if sale_ref:
                        try:
                            ...
                            if _al06_dup:
                                _dup_id  = _al06_dup.get('id','?')  if isinstance(_al06_dup, dict) else _al06_dup[0]
                                _dup_lot = _al06_dup.get('lot_no','?') if isinstance(_al06_dup, dict) else _al06_dup[1]
                                _dup_cust= _al06_dup.get('customer','?') if isinstance(_al06_dup, dict) else ''
                                msg = (
                                    f"[AL-06][SALE_REF_CONFLICT] 판매참조번호 중복 HARD STOP: "
                                    f"sale_ref={sale_ref} — 이미 LOT {_dup_lot}({_dup_cust}) "
                                    f"plan_id={_dup_id}에서 사용 중 "
                                    f"(동일 sale_ref를 다른 LOT에 재사용 불가)"
                                )
                                logger.error(msg)
                                result['errors'].append(msg)
                                _build_error_detail(line_no, "SALE_REF_CONFLICT", msg, lot_no, customer, qty_mt)
                                continue
                            # 동일 LOT + 동일 sale_ref 중복 (LOT_MODE_DUP 유지)
                            ...
                            if _lot_dup:
                                ...
                                msg = (
                                    ...
                                    f"lot={lot_no} plan_id={_dup_id} — 엑셀 재업로드 확인"
                                )
                                logger.warning(msg)
                                result['errors'].append(msg)
                            ...
                            logger.debug(f"[AL-06/LOT_MODE_DUP 중복체크] 스킵: {_ge}")

                    # v6.7.6 [⑦] UNIT_MISMATCH — KG 이외 단위 차단
                    # allocation 엑셀 복붙 시 MT/TON 단위가 섞이면
                    # kg 계산 전체가 틀어지므로(예: 20MT→20kg 오인) 사전 차단
                    _unit_val = str(_alloc_val(alloc, 'unit') or '').strip().upper()
                    if _unit_val and _unit_val not in ('', 'KG'):
                        msg = (f"[UNIT_MISMATCH] 허용되지 않은 단위: '{_unit_val}' "
                    ...
                    # v7.1.2 [G2-CARGO-CHECK]: cargo_weight_sum 기준 Allocation 총량 초과 검증
                    # 샘플(1kg)은 Allocation 대상 아님 → is_sample=0 기준 가용 cargo 합계와 비교
                    if qty_mt > 0:
                        _g2_cargo_row = self.db.fetchone(
                            "SELECT COALESCE(SUM(weight),0) AS cargo_kg "
                            "FROM inventory_tonbag "
                            "WHERE lot_no=? AND COALESCE(is_sample,0)=0 "
                            "AND status NOT IN ('SOLD','RETURNED','DEPLETED')",
                        ...
                        _g2_cargo_kg = float(
                            (_g2_cargo_row.get('cargo_kg') if isinstance(_g2_cargo_row, dict) else _g2_cargo_row[0])
                            if _g2_cargo_row else 0
                        )
                        _g2_req_kg = qty_mt * 1000.0
                        if _g2_cargo_kg > 0 and _g2_req_kg > _g2_cargo_kg + 0.5:
                            _g2_msg = (
                                f"[G2-CARGO-EXCEED] {lot_no}: "
                                f"Allocation {qty_mt:.3f}t({_g2_req_kg:.0f}kg) > "
                                f"가용cargo {_g2_cargo_kg:.0f}kg "
                                f"(샘플 제외 기준) — Allocation 총량 초과"
                            )
...
# [v6.8.6 top-level import로 이동]                     from engine_modules.constants import STATUS_PICKED, STATUS_RESERVED, STATUS_SOLD, get_tonbag_unit_weight
                    ...
                    # [B] v6.8.3: qty_mt → 톤백 개수 변환 검증
                    # _unit_w(DB 조회 실제 단가) 기반으로 계산,
                    # sublot_count 제공 시에도 qty_mt와 교차 검증
                    if sublot_count > 0:
                        pick_count = sublot_count
                        # [B] sublot_count와 qty_mt 교차 검증
                        if not is_sample_req and _unit_w > 0 and qty_mt > 0:
                            _calc_count = max(1, math.ceil((qty_mt * 1000) / _unit_w))
                            if abs(_calc_count - sublot_count) > 1:  # 1개 오차 허용
                                _b_warn = (
                        ...
                        if is_sample_req:
                            ...
                            # [B] _unit_w 이상 — 기본값 500kg로 fallback + 경고
                            _b_warn = (
                    ...
                    # v7.1.2 [G7-RANDOM-LOG]: 랜덤 선택 이력 audit_log 저장
                    # candidate(후보 전체) / selected(선택) / excluded(제외) + 이유
                    try:
                        import json as _json_g7
                        _all_uids   = [str(tb.get('tonbag_uid') or tb.get('sub_lt','')) for tb in tonbags]
                        _sel_uids   = [str(tb.get('tonbag_uid') or tb.get('sub_lt','')) for tb in (selected if 'selected' in dir() else [])]
                        _excl_uids  = [u for u in _all_uids if u not in _sel_uids]
                        _g7_payload = _json_g7.dumps({
    ...
    def execute_reserved(self, lot_no: str = None, target_date: str = None) -> Dict:
        ...
        # v6.9.4 [LOT-MODE-ONLY]: tonbag_id IS NOT NULL 조건 제거
        # 설계 원칙: 출고 전까지 tonbag 특정 불가 → tonbag_id=NULL 예약도 처리
        # 실 tonbag 확정은 바코드 스캔(scan_and_match_lot_reserved)에서만 수행
        ...
        try:
            ...
            # [H] v6.8.4: outbound_date 30일 초과 만료 예약 경고
            # target_date 없이 호출 시 3개월 전 예약도 함께 실행됨 방지
            if plans:
                try:
                    from datetime import timedelta
            ...
            with self.db.transaction("IMMEDIATE"):
                ...
                for plan in plans:
                    ...
                    # v6.9.4 [LOT-MODE-ONLY]: tonbag_id=NULL 예약은 스캔 대기 상태로 기록
                    # execute_reserved()에서 PICKED로 바꾸지 않음
                    # → scan_and_match_lot_reserved()가 스캔 시 확정
                    if tb_id is None:
                        logger.debug(
                            f"[LOT-MODE] {p_lot} plan_id={plan['id']} "
                            f"tonbag_id=NULL → 바코드 스캔 대기 (execute_reserved 스킵)"
                        )
                        result['executed'] += 1  # 개수 집계는 포함 (정합성 유지)
                    ...
                    # [A] v6.7.8: 샘플 톤백 무게 재확인
                    # RESERVED → PICKED 전환 시 is_sample=1 이면 1kg 이어야 함
                    # 입고 오류로 1kg 이상 저장된 경우 경고 후 스킵 (무게 오차 방지)
                    _is_sample_tb = int(tb.get('is_sample') or 0)
                    tb_weight = tb['weight'] or 0
                    # [K] v6.8.4: tb_weight=0 차단
                    # 입고 오류로 weight=0인 톤백이 PICKED되면
                    # picked_weight += 0 누적 → LOT_TOTAL_MISMATCH 오탐
                    if tb_weight <= 0:
                        _k_warn = (
                    ...
                    # v6.9.0 [M1]: OUTBOUND → PICKED_MOVE (이동 이력)
                    # 실제 매출 집계용 'SOLD'는 confirm_outbound에서만 기록
                    self.db.execute(
                        """INSERT INTO stock_movement
                        (lot_no, movement_type, qty_kg, remarks, created_at)
                        VALUES (?, 'PICKED_MOVE', ?, ?, ?)""",
                    ...
                    try:
                        ...
                        if "no such table" in _oe_msg:
                            ...
                            # v6.9.0 [H3]: UNIQUE 위반 = 중복 피킹 → errors에 추가 + 스킵
                            _h3_msg = (
                                f"[PICKING_DUPLICATE] 중복 피킹 차단: "
                                f"tonbag_id={tb_id}, lot={p_lot} — "
                            ...
                            logger.warning(f"[picking_table] INSERT 실패: {e}")
    ...
    def confirm_outbound(self, lot_no: str = None, force_all: bool = False) -> Dict:
        ...
        # v6.9.0 [H1]: lot_no=None 전체 확정 — force_all=True 없으면 hard-stop
        ...
        try:
            ...
            # [F] v6.8.4: 이중 SOLD 차단
            # sold_table에 이미 동일 tonbag_id 존재 여부 사전 확인
            # confirm_outbound는 PICKED 체크만 있어 중복 호출 시 2중 SOLD 가능
            try:
                ...
                if _tb_ids:
                    ...
                    if _already_sold:
                        ...
                        _f_msg = (
                            f"[DOUBLE_SOLD_BLOCKED] 이미 출고된 톤백 {len(_dup_ids)}개 중복 확정 시도: "
                ...
                logger.debug(f"[F 이중SOLD체크] sold_table 조회 스킵: {_fe}")

            # v6.9.0 [H2]: sale_ref/customer 혼재 preflight — N+1 → 단일 벌크 쿼리
            _sale_ref_set = set()
            _customer_set = set()
            try:
                _tb_ids = [tb['id'] for tb in tonbags]
                ...
                logger.debug(f"[H2 혼재체크] 스킵: {_h2e}")
            ...
            with self.db.transaction("IMMEDIATE"):
                ...
                for tb in tonbags:
                    ...
                    # v7.2.0: stock_movement OUTBOUND (구 SOLD → 신규 OUTBOUND)
                    self.db.execute(
                        "INSERT INTO stock_movement (lot_no, movement_type, qty_kg, remarks, created_at) "
                        "VALUES (?, 'OUTBOUND', ?, ?, ?)",
            ...
            # ② v6.8.0: LOT_TOTAL_MISMATCH 자동 사후검증
            # 출고 확정 직후 initial_weight = current_weight + picked_weight 자동 점검
            try:
                _post_errors = []
                ...
                # ③ v6.8.0: SAMPLE_POLICY_BROKEN — LOT 당 샘플 1개 정책 자동 점검
    ...
    def gate1_verify_picking(
        ...
        try:
            ...
            for lot_no in sorted(matched):
                ...
                if _is_lot_mode:
                    # LOT 모드: allocation_plan.qty_mt 합산 (1MT = 1000kg)
    ...
    def gate1_apply_picking_result(
        ...
        try:
            ...
            with self.db.transaction('IMMEDIATE'):
                ...
                reverted_total = 0
                picking_qty_map = {}  # {lot_no: tonbag_count}  gate1에서 집계됨
    ...
    def cancel_reservation(
        ...
        if plan_ids is None and plan_id is None and lot_no is None:
            result['message'] = "취소할 예약 없음"
            result['errors'].append("[NO_CANCEL_TARGET] lot_no/plan_id/plan_ids 중 하나는 반드시 지정 필요")
            return result
    ...
    def revert_sold_to_picked(self, lot_no: str = None) -> Dict:
        ...
        """
        result = {'success': False, 'reverted': 0, 'errors': []}
        query = """SELECT id, lot_no, weight FROM inventory_tonbag WHERE status = ?"""
        params = [STATUS_SOLD]
        if lot_no:
            query += " AND lot_no = ?"
            params.append(lot_no)
        try:
            ...
            with self.db.transaction("IMMEDIATE"):
                ...
                for tb in tonbags:
                    ...
                    try:
                        _has_cancelled_at = 'cancelled_at' in (alloc_plan_cols if hasattr(self, '_alloc_plan_cols_cache') else set())
                        try:
...
# [v6.8.6 top-level import로 이동]         from engine_modules.constants import QUICK_OUTBOUND_MAX_TONBAGS
    ...
    def _preflight_alloc_cols(self) -> dict:
        """
        allocation_plan 테이블 컬럼 존재 여부 프리플라이트 검사.

        Returns:
            {
                'cols': set[str],         실제 컬럼명 집합
                'has_source': bool,       source 컬럼 존재
                'has_line_no': bool,      line_no 컬럼 존재
                'has_export_type': bool,  export_type 컬럼 존재
        ...
        try:
            ...
            return {
                ...
                'has_workflow_status': 'workflow_status' in cols or 'status' in cols,
    ...
    # =========================================================================
    # v7.0.0: _rfa_build_error_detail — 오류 상세 항목 누적
    # =========================================================================
    @staticmethod
    def _rfa_build_error_detail(
        result: dict,
        line_no: int,
        fail_code: str,
        reason: str,
        ...
        """
        출고 오류 상세 항목을 result['error_details']에 누적.

        Args:
            result:    결과 dict (error_details 키 포함)
            line_no:   엑셀/CSV 행 번호
            fail_code: 오류 코드
            reason:    오류 사유
            lot_no:    LOT 번호
    ...
    # ═══════════════════════════════════════════════════════════════════
    # v7.3.2 Phase3 포팅: FIFO 배정 프리뷰/스테이징 엔진
    # ═══════════════════════════════════════════════════════════════════

    def _get_allocation_plan_columns(self) -> set:
        """allocation_plan 컬럼 목록 조회."""
        try:
            rows = self.db.fetchall("PRAGMA table_info(allocation_plan)") or []
            return {str(r.get("name", "")).strip().lower() for r in rows if isinstance(r, dict)}
        except Exception:
            return set()

    def _fetch_fifo_allocation_candidates(
        self,
        lot_no: str = '',
        product_code: str = '',
        ...
        """FIFO 후보 톤백 조회 (AVAILABLE 기준, inbound_date ASC)."""
        where = ["status = ?"]
        params: list = [STATUS_AVAILABLE]
        ...
        if not include_sample:
            where.append("COALESCE(is_sample, 0) = 0")

        sql = f"""
            SELECT
                id,
                lot_no,
                COALESCE(sap_no, '') AS sap_no,
                COALESCE(bl_no, '') AS bl_no,
                COALESCE(product_code, '') AS product_code,
                COALESCE(product, '') AS product,
                COALESCE(sub_lt, 0) AS sub_lt,
                COALESCE(tonbag_no, '') AS tonbag_no,
                COALESCE(weight, 0) AS weight,
                COALESCE(location, '') AS location,
                COALESCE(is_sample, 0) AS is_sample,
                COALESCE(inbound_date, '') AS inbound_date,
                COALESCE(created_at, '') AS created_at
            FROM inventory_tonbag
            WHERE {' AND '.join(where)}
            ORDER BY
                CASE WHEN COALESCE(inbound_date, '') = '' THEN 1 ELSE 0 END,
                inbound_date ASC,
                CASE WHEN COALESCE(created_at, '') = '' THEN 1 ELSE 0 END,
                created_at ASC,
                lot_no ASC,
                CAST(COALESCE(sub_lt, 0) AS INTEGER) ASC,
                tonbag_no ASC
        """
        if limit and int(limit) > 0:
            sql += f" LIMIT {int(limit)}"

        rows = self.db.fetchall(sql, tuple(params)) or []
        result = []
        for r in rows:
            row = dict(r) if isinstance(r, dict) else {}
            row["weight"] = float(row.get("weight", 0) or 0)
            row["is_sample"] = int(row.get("is_sample", 0) or 0)
            result.append(row)
        return result

    def preview_fifo_allocation_plan(
        self,
        request: Dict,
        strict_mode: bool = True,
    ) -> Dict:
        """FIFO Allocation 후보 계산 (DB 상태 미변경, preview only)."""
        request = dict(request or {})
        lot_no = str(request.get("lot_no", "") or "").strip()
        product_code = str(request.get("product_code", "") or "").strip()
        customer = str(request.get("customer", "") or "").strip()
        sale_ref = str(request.get("sale_ref", "") or "").strip()
        outbound_date = str(request.get("outbound_date", "") or "").strip()
        include_sample = bool(request.get("include_sample", False))

        qty_mt = float(request.get("qty_mt", 0) or 0)
        qty_kg = float(request.get("qty_kg", 0) or 0)
        requested_kg = qty_kg if qty_kg > 0 else qty_mt * 1000.0
        ...
        if requested_kg <= 0:
            result["errors"].append("요청 수량이 0 이하입니다.")
            return result

        candidates = self._fetch_fifo_allocation_candidates(
            lot_no=lot_no,
            product_code=product_code,
            include_sample=include_sample,
        )
        if not candidates:
            result["errors"].append("배정 가능한 AVAILABLE 톤백이 없습니다.")
            return result
        ...
        for row in candidates:
            selected.append(row)
            allocated += float(row.get("weight", 0) or 0)
            if allocated + 1e-9 >= requested_kg:
                break

        short_kg = max(0.0, requested_kg - allocated)
        result["allocated_kg"] = allocated
        result["short_kg"] = short_kg
        result["selected_count"] = len(selected)
        result["selected_rows"] = selected

        if short_kg > 0:
            msg = f"FIFO 후보 부족: 요청 {requested_kg:,.1f}kg / 배정 {allocated:,.1f}kg / 부족 {short_kg:,.1f}kg"
            if strict_mode:
                result["errors"].append(msg)
                return result
        ...
        if include_sample:
            sample_cnt = sum(1 for r in selected if int(r.get("is_sample", 0) or 0) == 1)
            if sample_cnt > 0:
                result["warnings"].append(f"샘플 톤백 {sample_cnt}개가 후보에 포함되었습니다.")

        result["success"] = True
        return result

    def stage_fifo_allocation_plan(
        self,
        request: Dict,
        strict_mode: bool = True,
        source_file: str = '',
    ) -> Dict:
        """FIFO preview 결과를 allocation_plan에 STAGED로 기록."""
        preview = self.preview_fifo_allocation_plan(request, strict_mode=strict_mode)
        if not preview.get("success"):
            return preview

        if not self._table_exists("allocation_plan"):
            preview["success"] = False
            preview["errors"].append("allocation_plan 테이블이 없습니다.")
            return preview

        cols = self._get_allocation_plan_columns()
        if not cols:
            preview["success"] = False
            preview["errors"].append("allocation_plan 컬럼 정보를 읽을 수 없습니다.")
            return preview

        plan_ids = []
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        source_name = os.path.basename(source_file or '') or 'P3_FIFO_PREVIEW'

        with self.db.transaction("IMMEDIATE"):
            for line_no, row in enumerate(preview.get("selected_rows", []), start=1):
                payload = {
                    "lot_no": preview.get("lot_no", ""),
                    "tonbag_id": row.get("id"),
                    "sub_lt": row.get("sub_lt"),
                    "customer": preview.get("customer", ""),
                    "sale_ref": preview.get("sale_ref", ""),
                    "qty_mt": float(row.get("weight", 0) or 0) / 1000.0,
                    "outbound_date": preview.get("outbound_date") or None,
                    "status": "STAGED",
                    "source": "P3_FIFO",
                    "source_file": source_name,
                    "created_at": now,
                    "workflow_status": "PENDING_APPROVAL",
                    "risk_flags": "SAMPLE_INCLUDED" if int(row.get("is_sample", 0) or 0) == 1 else "",
                    "line_no": line_no,
                }
                ...
                sql = f"INSERT INTO allocation_plan ({', '.join(insert_cols)}) VALUES ({placeholders})"
                vals = [payload[k] for k in insert_cols]
                self.db.execute(sql, tuple(vals))

                rid_row = self.db.fetchone("SELECT last_insert_rowid() AS rid")
                rid = int(rid_row.get("rid", 0) if isinstance(rid_row, dict) else (rid_row[0] if rid_row else 0))
                if rid:
                    plan_ids.append(rid)

            try:
                self.db.execute(
                    "INSERT INTO outbound_event_log (outbound_no, event_type, message) VALUES (?, ?, ?)",
                    (
                        preview.get("sale_ref", "") or "",
                        "P3_FIFO_STAGED",
                        f"lot={preview.get('lot_no','')}, selected={preview.get('selected_count',0)}, requested_kg={preview.get('requested_kg',0):.1f}",
                    ),
                )
            except Exception as e:
                logger.debug(f"outbound_event_log 기록 스킵: {e}")

        preview["plan_ids"] = plan_ids
        preview["staged"] = len(plan_ids)
        preview["message"] = f"FIFO allocation staged: {len(plan_ids)}건"
        return preview

    # ═══════════════════════════════════════════════════════════════════
    # v7.3.2 Phase3 포팅: 스캔 활동/알림 이벤트 시스템
    # ═══════════════════════════════════════════════════════════════════

    def get_outbound_final_status_summary(self, days: int = 3) -> Dict:
        """최근 PICKED/SOLD/OUTBOUND 최종 상태 요약."""
        result = {
            "success": False,
            "picked_cnt": 0,
            "sold_cnt": 0,
        ...
        try:
            ...
            result["rows"] = rows
            result["picked_cnt"] = sum(1 for r in rows if str(r.get("movement_type", "")) == "SCAN_PICKED")
            result["sold_cnt"] = sum(1 for r in rows if str(r.get("movement_type", "")) == "SCAN_SOLD")
            result["outbound_cnt"] = sum(1 for r in rows if str(r.get("movement_type", "")) == "OUTBOUND_FINAL")
            result["success"] = True
            return result
        except Exception as e:
            logger.error(f"출고 최종 상태 요약 오류: {e}", exc_info=True)
            result["errors"].append(str(e))
            return result

    def _map_scan_alert_code(self, message: str) -> str:
        """스캔 예외를 Alert Code로 표준화."""
        msg = str(message or '').lower()
        if 'uid가 비어' in msg:
            return 'SCAN_UID_EMPTY'
        if 'reserved 대상이 없습니다' in msg:
            return 'SCAN_NO_RESERVED_TARGET'
        if '톤백이 없습니다' in msg:
            return 'SCAN_UID_UNKNOWN'
        if 'lot 불일치' in msg:
            return 'SCAN_WRONG_LOT'
        if '상태 불가' in msg:
            return 'SCAN_STATUS_BLOCK'
        if '샘플 톤백' in msg:
            return 'SCAN_SAMPLE_BLOCK'
        if '이미 다른 배정에 연결' in msg:
            return 'SCAN_DUP_ALLOC'
        if '이미 출고 진행/완료' in msg:
            return 'SCAN_ALREADY_PROCESSED'
        return 'SCAN_GENERAL'

    def _record_scan_activity_event(self, movement_type: str, lot_no: str = '', remarks: str = '') -> None:
        """Scan 결과를 Recent Activity용 movement로 기록."""
        try:
            self.db.execute(
                "INSERT INTO stock_movement (lot_no, movement_type, qty_kg, remarks, created_at) "
                "VALUES (?, ?, 0, ?, datetime('now'))",
                (lot_no or '', movement_type, remarks or '')
            )
        except Exception as e:
            logger.debug(f"_record_scan_activity_event 스킵: {e}")

    def _record_scan_alert_event(self, lot_no: str, message: str) -> None:
        """Scan 경고/오류를 ALERT_EVENT로 기록."""
        try:
            code = self._map_scan_alert_code(message)
            remarks = f"{code} | scan | {message}"
            self.db.execute(
                "INSERT INTO stock_movement (lot_no, movement_type, qty_kg, remarks, created_at) "
                "VALUES (?, 'ALERT_EVENT', 0, ?, datetime('now'))",
                (lot_no or '', remarks)
            )
        except Exception as e:
            logger.debug(f"_record_scan_alert_event 스킵: {e}")

    # ═══════════════════════════════════════════════════════════════════
    # v7.3.2 Phase3 포팅: LOT 모드 스캔 브릿지
    # ═══════════════════════════════════════════════════════════════════

    def _find_pending_lot_mode_plan_row(
        self,
        lot_no: str = '',
        sale_ref: str = '',
    ) -> Optional[Dict]:
        """LOT 모드 RESERVED(tonbag_id NULL) 대상 1건 조회."""
        where = ["status='RESERVED'", "tonbag_id IS NULL"]
        params: list = []
        if lot_no:
            where.append("lot_no=?")
            params.append(str(lot_no).strip())
        if sale_ref:
            where.append("sale_ref=?")
            params.append(str(sale_ref).strip())

        return self.db.fetchone(
            f"""
            SELECT id, lot_no, customer, sale_ref, outbound_date, qty_mt, created_at
            FROM allocation_plan
            WHERE {' AND '.join(where)}
            ORDER BY created_at ASC, id ASC
            LIMIT 1
    ...
    def _find_scannable_tonbag_by_uid(self, scanned_uid: str) -> Optional[Dict]:
        """스캔 UID로 AVAILABLE 톤백 조회."""
        uid = str(scanned_uid or '').strip()
        if not uid:
            return None
        return self.db.fetchone(
            """
            SELECT id, lot_no, sub_lt, tonbag_uid, weight, status,
                   COALESCE(is_sample, 0) AS is_sample,
                   COALESCE(location, '') AS location
            FROM inventory_tonbag
            WHERE TRIM(COALESCE(tonbag_uid, '')) = ?
            LIMIT 1
            """,
            (uid,),
        )

    def validate_scanned_uid_for_lot_mode(
        self,
        scanned_uid: str,
        lot_no: str = '',
        sale_ref: str = '',
    ) -> Dict:
        """LOT모드 UID 스캔 예외 점검(Hard Stop/Warning)."""
        result = {
            "success": False,
            "valid": False,
            "hard_stop": False,
        ...
        if not uid:
            ...
            result["errors"].append("UID가 비어 있습니다.")
            try:
                self._record_scan_alert_event(lot_no=lot_no, message="UID가 비어 있습니다.")
            except Exception:
                pass
            return result

        plan = self._find_pending_lot_mode_plan_row(lot_no=lot_no, sale_ref=sale_ref)
        if not plan:
            result["hard_stop"] = True
            result["errors"].append("연결할 LOT 모드 RESERVED 대상이 없습니다.")
            try:
                self._record_scan_alert_event(lot_no=lot_no, message="연결할 LOT 모드 RESERVED 대상이 없습니다.")
            except Exception:
                pass
            return result

        result["plan_id"] = int(plan.get("id", 0) or 0)
        plan_lot = str(plan.get("lot_no", "")).strip()

        tb = self._find_scannable_tonbag_by_uid(uid)
        if not tb:
            tb_any = self.db.fetchone(
                """
                SELECT id, lot_no, sub_lt, tonbag_uid, weight, status, COALESCE(is_sample,0) AS is_sample
                FROM inventory_tonbag
                WHERE TRIM(COALESCE(tonbag_uid, '')) = ?
                LIMIT 1
                """,
            ...
            result["hard_stop"] = True
            if tb_any:
                result["tonbag_id"] = int(tb_any.get("id", 0) or 0)
                result["errors"].append(
                    f"스캔 UID 상태 불가: {tb_any.get('status', '')} (AVAILABLE만 연결 가능)"
                )
            else:
                result["errors"].append(f"UID에 해당하는 톤백이 없습니다: {uid}")
            return result

        result["tonbag_id"] = int(tb.get("id", 0) or 0)
        tb_lot = str(tb.get("lot_no", "")).strip()
        tb_status = str(tb.get("status", "")).strip()

        if tb_lot != plan_lot:
            result["hard_stop"] = True
            result["errors"].append(f"LOT 불일치: 예약 {plan_lot} / 스캔 {tb_lot}")
            return result

        if tb_status != STATUS_AVAILABLE:
            result["hard_stop"] = True
            result["errors"].append(f"스캔 톤백 상태 불가: {tb_status}")
            return result

        if int(tb.get("is_sample", 0) or 0) == 1:
            result["hard_stop"] = True
            result["errors"].append("샘플 톤백 오스캔: 본화물 LOT모드 예약에는 연결할 수 없습니다.")
            return result

        dup_reserved = self.db.fetchone(
            "SELECT id, lot_no, status, workflow_status FROM allocation_plan "
            "WHERE tonbag_id=? AND status IN ('RESERVED','EXECUTED') LIMIT 1",
            (int(tb.get("id")),),
        )
        if dup_reserved:
            result["hard_stop"] = True
            result["errors"].append(
                f"이미 다른 배정에 연결된 UID입니다: plan_id={dup_reserved.get('id')}, lot_no={dup_reserved.get('lot_no')}"
            )
            return result

        dup_pick = self.db.fetchone(
            "SELECT id, lot_no, status FROM inventory_tonbag WHERE id=? AND status IN (?, ?, ?)",
            (int(tb.get("id")), STATUS_PICKED, STATUS_OUTBOUND, STATUS_SOLD),
        )
        if dup_pick:
            result["hard_stop"] = True
            result["errors"].append(
                f"이미 출고 진행/완료된 UID입니다: status={dup_pick.get('status')}"
            )
            return result

        try:
            avail_row = self.db.fetchone(
                "SELECT COUNT(*) AS cnt FROM inventory_tonbag WHERE lot_no=? AND status=? AND COALESCE(is_sample,0)=0",
                (plan_lot, STATUS_AVAILABLE),
            ) or {}
            avail_cnt = int(avail_row.get("cnt", 0) if isinstance(avail_row, dict) else (avail_row[0] if avail_row else 0))
            if avail_cnt <= 1:
                result["warnings"].append(f"{plan_lot}: 판매가능 톤백 잔량이 적습니다 ({avail_cnt}개)")
        except Exception:
            pass
    ...
    def bind_scanned_uid_to_reserved_plan(
        self,
        scanned_uid: str,
        lot_no: str = '',
        sale_ref: str = '',
        ...
        """LOT 모드 RESERVED 건에 스캔 UID를 연결하고 tonbag_id 확정."""
        result = {
            "success": False,
            "plan_id": None,
            "tonbag_id": None,
            "lot_no": "",
            "scanned_uid": str(scanned_uid or '').strip(),
            "picked": 0,
            "errors": [],
            "warnings": [],
        }
        ...
        uid = str(scanned_uid or '').strip()
        if not uid:
            result["errors"].append("scanned_uid가 비어 있습니다.")
            return result
        ...
        tb = self._find_scannable_tonbag_by_uid(uid)
        if not tb:
            result["errors"].append(f"스캔 UID를 가진 톤백이 없습니다: {uid}")
            return result

        target_lot = str(plan.get("lot_no", "")).strip()
        tb_lot = str(tb.get("lot_no", "")).strip()
        if target_lot and tb_lot != target_lot:
            result["errors"].append(f"LOT 불일치: 예약 {target_lot} / 스캔 {tb_lot}")
            return result

        tb_status = str(tb.get("status", "")).strip()
        if tb_status != STATUS_AVAILABLE:
            result["errors"].append(f"스캔 톤백 상태 불가: {tb_status} (AVAILABLE만 가능)")
            return result

        if int(tb.get("is_sample", 0) or 0) == 1:
            result["errors"].append("샘플 톤백은 LOT 모드 본화물 예약에 연결할 수 없습니다.")
            return result

        dup_row = self.db.fetchone(
            "SELECT id, lot_no FROM allocation_plan WHERE tonbag_id=? AND status IN ('RESERVED','EXECUTED') LIMIT 1",
            (int(tb.get("id")),),
        )
        if dup_row:
            result["errors"].append(
                f"이미 다른 배정에 연결된 톤백입니다: plan_id={dup_row.get('id')}, lot_no={dup_row.get('lot_no')}"
            )
            return result

        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        actor = actor or os.environ.get("USERNAME", "") or os.environ.get("USER", "") or "system"
        ...
        with self.db.transaction("IMMEDIATE"):
            self.db.execute(
                ...
                UPDATE allocation_plan
                   SET tonbag_id=?,
                       sub_lt=?,
                       workflow_status=COALESCE(NULLIF(workflow_status, ''), 'APPLIED')
                 WHERE id=? AND status='RESERVED' AND tonbag_id IS NULL
                """,
                (int(tb.get("id")), int(tb.get("sub_lt", 0) or 0), int(plan.get("id"))),
            )
            ...
            if auto_pick:
                self.db.execute(
                    ...
                    (
                        STATUS_PICKED,
                        now,
                        plan.get("outbound_date") or now,
                        plan.get("customer", "") or "",
                        now,
                        int(tb.get("id")),
                        STATUS_AVAILABLE,
                    ),
            ...
            try:
                self.db.execute(
                    ...
                    (
                        target_lot,
                        movement_type,
                        float(tb.get("weight", 0) or 0),
                        f"uid_bind actor={actor}, uid={uid}, plan_id={int(plan.get('id'))}, status={new_status}",
                        now,
                    ),
                )
            except Exception:
            ...
            self._recalc_lot_status(target_lot)

        result["success"] = True
        result["plan_id"] = int(plan.get("id"))
        result["tonbag_id"] = int(tb.get("id"))
        result["lot_no"] = target_lot
        result["picked"] = picked_count
        result["message"] = (
            f"UID 연결 완료: lot={target_lot}, plan_id={result['plan_id']}, tonbag_id={result['tonbag_id']}, "
            f"status={'PICKED' if auto_pick else 'RESERVED'}"
        )
        try:
            self._record_scan_activity_event(
                'SCAN_PICKED' if auto_pick else 'SCAN_RESERVED_BIND',
                lot_no=target_lot,
    ...
    def safe_bind_scanned_uid_to_reserved_plan(
        self,
        scanned_uid: str,
        lot_no: str = '',
        sale_ref: str = '',
        ...
        """validate 후 bind 수행하는 안전 래퍼."""
        check = self.validate_scanned_uid_for_lot_mode(
        ...
        result = self.bind_scanned_uid_to_reserved_plan(
            scanned_uid=scanned_uid,
            lot_no=lot_no,
            sale_ref=sale_ref,
            actor=actor,
        ...
        if result.get("success") and check.get("warnings"):
            result.setdefault("warnings", [])
            result["warnings"].extend(list(check.get("warnings", []) or []))
        return result

    def execute_lot_mode_scanned_outbound(
        self,
        scanned_uid: str,
        lot_no: str = '',
        sale_ref: str = '',
        ...
        """스캔 UID를 LOT모드 예약에 연결하고 즉시 PICKED까지 진행."""
        return self.safe_bind_scanned_uid_to_reserved_plan(
            scanned_uid=scanned_uid,
            lot_no=lot_no,
            sale_ref=sale_ref,
            actor=actor,
    ...
    def get_lot_mode_reservation_summary(self, lot_no: str = '') -> Dict:
        """tonbag_id=NULL LOT모드 예약 요약 조회."""
        result = {
            "success": False,
            "rows": [],
            "lot_count": 0,
            "pending_tonbag_confirm": 0,
            "available_tonbags": 0,
            "errors": [],
        }
        try:
            ...
            rows = self.db.fetchall(
                ...
                FROM allocation_plan
                WHERE {' AND '.join(where)}
                GROUP BY lot_no, customer, sale_ref, outbound_date
                ORDER BY outbound_date, lot_no
                """,
                tuple(params),
            ) or []
            ...
            normalized = []
            for r in rows:
                lno = str(r.get("lot_no", "")).strip()
                avail_row = self.db.fetchone(
                    "SELECT COUNT(*) AS cnt FROM inventory_tonbag WHERE lot_no=? AND status=? AND COALESCE(is_sample,0)=0",
                    (lno, STATUS_AVAILABLE),
                ) or {}
                available_cnt = int(avail_row.get("cnt", 0) if isinstance(avail_row, dict) else (avail_row[0] if avail_row else 0))
                normalized.append({
                    "lot_no": lno,
                    "customer": str(r.get("customer", "") or ""),
                    "sale_ref": str(r.get("sale_ref", "") or ""),
                    "outbound_date": str(r.get("outbound_date", "") or ""),
                    "row_count": int(r.get("row_count", 0) or 0),
                    "total_mt": float(r.get("total_mt", 0) or 0),
                    "available_tonbags": available_cnt,
                    "ready_for_scan": available_cnt > 0,
                })

            result["rows"] = normalized
            result["lot_count"] = len(normalized)
            result["pending_tonbag_confirm"] = sum(int(r.get("row_count", 0) or 0) for r in normalized)
            result["available_tonbags"] = sum(int(r.get("available_tonbags", 0) or 0) for r in normalized)
            result["success"] = True
            return result
        except Exception as e:
            logger.error(f"LOT 모드 예약 요약 조회 오류: {e}", exc_info=True)
            result["errors"].append(str(e))
            return result

    def validate_lot_mode_execution_readiness(self, lot_no: str = '') -> Dict:
        """LOT모드 예약이 스캔/실행 가능한지 사전 점검."""
        result = {
            "success": False,
            "ready": False,
            "rows": [],
        ...
        summary = self.get_lot_mode_reservation_summary(lot_no=lot_no)
        ...
        result["rows"] = rows
        if not rows:
            result["warnings"].append("LOT 모드 예약 대기 건이 없습니다.")
            result["success"] = True
            result["ready"] = True
            return result
        ...
        for r in rows:
            lno = r.get("lot_no", "")
            avail = int(r.get("available_tonbags", 0) or 0)
            if avail <= 0:
                result["errors"].append(f"{lno}: AVAILABLE 톤백 없음")
            elif avail < int(r.get("row_count", 0) or 0):
                result["warnings"].append(
                    f"{lno}: LOT모드 예약 {int(r.get('row_count', 0) or 0)}건 대비 판매가능 톤백 {avail}개"
                )

...
from engine_modules.constants import (
    STATUS_AVAILABLE, STATUS_DEPLETED, STATUS_PICKED,
    STATUS_RESERVED, STATUS_SOLD, STATUS_OUTBOUND,  # v7.2.0
)
import sqlite3
import tkinter as tk
from ..utils.ui_constants import ThemeColors, Spacing, apply_modal_window_options, get_status_display, setup_dialog_geometry_persistence
import logging
...
# ═══════════════════════════════════════════════════════════════
INVENTORY_COLUMNS = [
    # v7.3.2: 기본 8개 표시 (v7.1.8 스타일 한글 헤더) — 나머지는 우클릭 컬럼 선택
    # 순서: LOT번호 → 제품명 → 업체 → 입고일 → 톤백수 → 중량(MT) → 위치 → 상태
    ('row_num',            'No.',            50, 'center', False),   # 기본 숨김
    ('lot_no',             'LOT번호',       130, 'center', True),
    ('product',            '제품명',        160, 'center', True),
    ('sap_no',             '업체',          120, 'center', True),    # SAP NO → 업체로 표시
    ('arrival_date',       '입고일',         95, 'center', True),
    ('avail_bags',         '톤백수',         70, 'center', True),
    ('current_weight',     '중량(MT)',      100, 'e',      True),
    ('warehouse',          '위치',           80, 'center', True),
    ('status',             '상태',           90, 'center', True),
    ('bl_no',              'BL NO',         140, 'center', False),
    ('net_weight',         'NET(Kg)',        100, 'e',      False),
    ('container_no',       'CONTAINER',     130, 'center', False),
    ('mxbg_pallet',        'MXBG',           70, 'center', False),
    ('tb_avail',           '↓Avail개',       62, 'center', False),
    ('tb_reserved',        '↓Resv개',        62, 'center', False),
    ('tb_picked',          '↓Pick개',        62, 'center', False),
    ('tb_sold',            '↓Sold개',        62, 'center', False),
    ('salar_invoice_no',   'INVOICE NO',    100, 'center', False),
    ('ship_date',          'SHIP DATE',      95, 'center', False),
    ('con_return',         'CON RETURN',     95, 'center', False),
...
class InventoryTabMixin:
    ...
    def _setup_inventory_tab(self) -> None:
        ...
        # v7.3.2: 심플 검색바 + 액션 버튼 (v7.1.8 스타일)
        self._inv_view_switch_var = tk.StringVar(value='recovery')
        self._inv_show_all_tonbags = False

        _action_bar = ttk.Frame(self.tab_inventory)
        _action_bar.pack(fill=X, padx=Spacing.SM, pady=(Spacing.SM, Spacing.XS))

        # v7.3.3: 검색바 — 돋보기 아이콘 + Entry
        ttk.Label(_action_bar, text="🔍", font=('', 14)).pack(side=LEFT, padx=(0, 2))
        self._inv_quick_search_var = tk.StringVar()
        _search_entry = ttk.Entry(
            _action_bar, textvariable=self._inv_quick_search_var,
            font=('맑은 고딕', 11), width=30
        )
        _search_entry.pack(side=LEFT, padx=(0, Spacing.SM))
        _search_entry.insert(0, '')
        # placeholder 효과
        _ph_text = '제품명 / LOT번호 / 위치'
        def _on_search_focus_in(e):
            if _search_entry.get() == _ph_text:
                _search_entry.delete(0, 'end')
                _search_entry.config(foreground='')
        def _on_search_focus_out(e):
            if not _search_entry.get().strip():
                _search_entry.insert(0, _ph_text)
        ...
        _search_entry.bind('<Return>', lambda e: self._on_inv_quick_search())

        # v7.3.4: ProButton — 호버/프레스 피드백 + 라이트/다크 자동 대응
        from ..utils.ui_constants import apply_tooltip
        from ..utils.pro_button import ProButton

        _btn_inbound = ProButton(
            _action_bar, text="입고등록", variant="inbound", icon="📥",
            command=lambda: self._safe_call('_on_pdf_inbound'), bold=True
        )
        _btn_inbound.pack(side=LEFT, padx=(0, Spacing.XS))
        apply_tooltip(_btn_inbound, "새 입고 등록 (Ctrl+N)")

        _btn_outbound = ProButton(
            _action_bar, text="출고처리", variant="danger", icon="📤",
            command=lambda: self._safe_call('_on_allocation_input_unified'), bold=True
        )
        _btn_outbound.pack(side=LEFT, padx=(0, Spacing.XS))
        apply_tooltip(_btn_outbound, "선택 LOT 출고 처리")

        _btn_export = ProButton(
            _action_bar, text="Excel 내보내기", variant="report", icon="📊",
            command=lambda: self._on_export_click(option=3)
        )
        _btn_export.pack(side=LEFT, padx=(0, Spacing.XS))
        apply_tooltip(_btn_export, "현재 데이터를 Excel로 내보내기 (Ctrl+E)")

        _btn_refresh = ProButton(
            _action_bar, text="새로고침", variant="secondary", icon="🔄",
            command=lambda: self._safe_refresh()
        )
        _btn_refresh.pack(side=LEFT, padx=(0, Spacing.SM))
        apply_tooltip(_btn_refresh, "전체 새로고침 (F5)")

        # ▼ 상세 필터 토글 + LOT/톤백 전환
        _toggle_row = ttk.Frame(self.tab_inventory)
        _toggle_row.pack(fill=X, padx=Spacing.SM, pady=(0, Spacing.XS))

        self._inv_detail_filter_visible = False
        self._inv_toggle_filter_btn = ttk.Button(
            _toggle_row, text="▶ 상세 필터", width=12,
            command=self._toggle_detail_filter
        )
        self._inv_toggle_filter_btn.pack(side=LEFT, padx=(0, Spacing.MD))

        # LOT / 톤백 전환 라디오
        try:
            _s = ttk.Style()
            _s.configure('InvSwitch.TRadiobutton', font=('맑은 고딕', 10))
            _radio_style = 'InvSwitch.TRadiobutton'
        except Exception:
            _radio_style = None
        _kw = {'style': _radio_style} if _radio_style else {}
        ttk.Radiobutton(
            _toggle_row, text="LOT 리스트", variable=self._inv_view_switch_var, value='recovery',
            command=self._on_inv_view_switch, **_kw
        ).pack(side=LEFT, padx=(0, Spacing.SM))
        ttk.Radiobutton(
            _toggle_row, text="톤백 리스트", variable=self._inv_view_switch_var, value='tonbag',
            command=self._on_inv_view_switch, **_kw
        ).pack(side=LEFT)
        ...
        self._inv_split_panel.pack(fill=BOTH, expand=YES, padx=Spacing.SM, pady=(Spacing.XS, Spacing.SM))
        ...
        # 스크롤바
        v_scroll = tk.Scrollbar(tree_frame, orient=VERTICAL, command=self.tree_inventory.yview)
        h_scroll = tk.Scrollbar(tree_frame, orient='horizontal', command=self.tree_inventory.xview)
        ...
        # v7.3.2: 하단 간결 요약 (v7.1.8 스타일)
        self._inv_status_summary_var = tk.StringVar(value="총 재고: 0개 톤백 / 0.0 MT | AVAILABLE 상태")
        _summary_bar = ttk.Frame(self._inv_recovery_container)
        _summary_bar.pack(fill=X, padx=Spacing.XS, pady=(2, 4))
        ttk.Label(
            _summary_bar, textvariable=self._inv_status_summary_var,
            font=('맑은 고딕', 10), foreground='#94a3b8'
        ).pack(side=LEFT)
    ...
    def _on_inv_quick_search(self) -> None:
        """v7.3.2: 빠른 검색 — 제품명/LOT번호/위치로 트리뷰 필터링"""
        if not hasattr(self, 'tree_inventory'):
            return
        query = getattr(self, '_inv_quick_search_var', None)
        if not query:
            return
        term = query.get().strip()
        ph = '제품명 / LOT번호 / 위치 검색...'
        if not term or term == ph:
            # 빈 검색 → 전체 새로고침
            if hasattr(self, '_safe_refresh'):
                self._safe_refresh()
            return
        term_lower = term.lower()
        # 현재 트리 아이템 중 매칭되지 않는 것 숨기기 (삭제 방식)
        to_remove = []
        ...
        for item_id in to_remove:
            self.tree_inventory.delete(item_id)

    def _toggle_detail_filter(self) -> None:
        """v7.3.2: 상세 필터 영역 토글 (▶ ↔ ▼)"""
        self._inv_detail_filter_visible = not getattr(self, '_inv_detail_filter_visible', False)
        if self._inv_detail_filter_visible:
            if hasattr(self, '_inv_toggle_filter_btn'):
                self._inv_toggle_filter_btn.config(text="▼ 상세 필터")
            if hasattr(self, '_inv_header_row_place'):
                self._inv_header_row_place.pack(fill='x', padx=4, pady=(0, 4),
                                                 before=self._inv_split_panel)
        else:
            if hasattr(self, '_inv_toggle_filter_btn'):
                self._inv_toggle_filter_btn.config(text="▶ 상세 필터")
            if hasattr(self, '_inv_header_row_place'):
                self._inv_header_row_place.pack_forget()
    ...
    def _setup_inv_tonbag_detail_panel(self) -> None:
        ...
        sb = tk.Scrollbar(detail_container, orient=VERTICAL, command=self._inv_tonbag_detail_tree.yview)
        sb_x = tk.Scrollbar(detail_container, orient='horizontal', command=self._inv_tonbag_detail_tree.xview)
    ...
    def _refresh_inv_stats(self) -> None:
        ...
        try:
            ...
            # v7.3.2: 하단 간결 요약바 (v7.1.8 스타일)
            if hasattr(self, '_inv_status_summary_var'):
                self._inv_status_summary_var.set(
                    f"총 재고: {tb_total:,}개 톤백 / {current_kg:,.1f} MT | AVAILABLE 상태"
                )
        except (ValueError, TypeError, KeyError) as e:
    ...
    def _update_inv_filter_values(self, inventory) -> None:
        ...
        try:
            ...
            cnt_avail = sum(1 for i in inventory if (i.get('status') or '') == STATUS_AVAILABLE)
            cnt_reserved = sum(1 for i in inventory if (i.get('status') or '') == STATUS_RESERVED)
            cnt_picked = sum(1 for i in inventory if (i.get('status') or '') == STATUS_PICKED)
            # v7.2.0: OUTBOUND + SOLD 통합 집계
            cnt_sold = sum(1 for i in inventory if (i.get('status') or '') in (STATUS_OUTBOUND, STATUS_SOLD))
            cnt_return = sum(1 for i in inventory if (i.get('status') or '') == 'RETURN')
            status_values = [
    ...
    def _sort_treeview(self, tree, col: str) -> None:
        ...
        try:
            from ..utils.table_styler import TableStyler
            TableStyler.refresh_striped_rows(tree)

...
class AllocationTabMixin:
    ...
    def _setup_allocation_tab(self) -> None:
        ...
        btn_lot_summary = ttk.Button(btn_frame, text="📦 LOT모드 요약", command=self._show_lot_mode_reservation_summary)
        btn_lot_summary.pack(side=LEFT, padx=Spacing.XS)
        apply_tooltip(btn_lot_summary, "LOT 모드 예약 대기 건 요약을 표시합니다.")
        btn_lot_check = ttk.Button(btn_frame, text="🧪 실행 점검", command=self._check_lot_mode_execution_readiness)
        btn_lot_check.pack(side=LEFT, padx=Spacing.XS)
        apply_tooltip(btn_lot_check, "LOT 모드 예약의 실행 준비 상태를 점검합니다.")
    ...
    # ═══════════════════════════════════════════════════════════════════
    # v7.3.2 Phase3 포팅: LOT 모드 요약/점검
    # ═══════════════════════════════════════════════════════════════════

    def _show_lot_mode_reservation_summary(self) -> None:
        """LOT모드 예약 대기 요약 팝업."""
        if not hasattr(self.engine, "get_lot_mode_reservation_summary"):
            CustomMessageBox.showwarning(self.root, "확인", "LOT 모드 요약 엔진이 없습니다.")
            return
        try:
            result = self.engine.get_lot_mode_reservation_summary()
            if not result.get("success"):
                CustomMessageBox.showerror(self.root, "오류", "\n".join(result.get("errors", ["알 수 없는 오류"])))
                return
            rows = list(result.get("rows", []) or [])
            if not rows:
                CustomMessageBox.showinfo(self.root, "LOT 모드 요약", "LOT 모드 예약 대기 건이 없습니다.")
                return
            lines = []
            for r in rows[:12]:
                lines.append(
                    f"{r.get('lot_no','')} | 예약 {int(r.get('row_count',0) or 0)}건 | "
                    f"{float(r.get('total_mt',0) or 0):,.2f} MT | 판매가능 톤백 {int(r.get('available_tonbags',0) or 0)}개"
                )
            if len(rows) > 12:
                lines.append(f"... 외 {len(rows)-12}건")
            msg = (
                f"LOT 모드 대기 LOT: {int(result.get('lot_count',0) or 0)}개\n"
                f"행 수: {int(result.get('pending_tonbag_confirm',0) or 0)}건\n"
                f"판매가능 톤백 합계: {int(result.get('available_tonbags',0) or 0)}개\n\n"
                + "\n".join(lines)
            )
            CustomMessageBox.showinfo(self.root, "LOT 모드 예약 요약", msg)
        except Exception as e:
            logger.error(f"LOT 모드 예약 요약 오류: {e}", exc_info=True)
            CustomMessageBox.showerror(self.root, "오류", str(e))

    def _check_lot_mode_execution_readiness(self) -> None:
        """LOT모드 예약 실행 준비도 점검 팝업."""
        if not hasattr(self.engine, "validate_lot_mode_execution_readiness"):
            CustomMessageBox.showwarning(self.root, "확인", "LOT 모드 점검 엔진이 없습니다.")
            return
        try:
            result = self.engine.validate_lot_mode_execution_readiness()
            if not result.get("success"):
                CustomMessageBox.showerror(self.root, "오류", "\n".join(result.get("errors", ["알 수 없는 오류"])))
                return
            ...
            if errors:
                ...
                if warnings:
                    msg += "\n\n[경고]\n" + "\n".join(warnings[:10])
                CustomMessageBox.showwarning(self.root, "LOT 모드 실행 점검", msg)
            elif warnings:
                CustomMessageBox.showwarning(self.root, "LOT 모드 실행 점검", "[주의]\n" + "\n".join(warnings[:10]))
            else:
                CustomMessageBox.showinfo(self.root, "LOT 모드 실행 점검", "LOT 모드 예약 대기 건은 스캔/실행 준비 상태입니다.")
        except Exception as e:
            logger.error(f"LOT 모드 실행 점검 오류: {e}", exc_info=True)
            CustomMessageBox.showerror(self.root, "오류", str(e))

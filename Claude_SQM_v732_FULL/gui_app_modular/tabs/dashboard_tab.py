...
class DashboardTabMixin:
    ...
    def _setup_dash_hero(self, ctx) -> None:
        ...
        hero_bg  = '#1e293b' if _is_dark else '#1e3a5f'   # 항상 진한 네이비
        border_c = '#334155' if _is_dark else '#2d5480'
        text_1   = '#ffffff'   # 항상 흰색
        ...
        # ── v7.3.2: 4단계 운영 흐름 둥근 배지 (v7.1.8 스타일) ──
        ...
        for idx, (lbl, color) in enumerate(STAGES):
            # 둥근 배지 (Canvas 기반)
            _bw, _bh = 80, 28
            _bc = tk.Canvas(badge_row, width=_bw, height=_bh,
                            bg=hero_bg, highlightthickness=0)
            _bc.pack(side='left')
            # 둥근 사각형
            _r = 12  # 코너 반경
            _bc.create_arc(2, 2, 2+_r*2, 2+_r*2, start=90, extent=90, fill=color, outline=color)
            _bc.create_arc(_bw-2-_r*2, 2, _bw-2, 2+_r*2, start=0, extent=90, fill=color, outline=color)
            _bc.create_arc(2, _bh-2-_r*2, 2+_r*2, _bh-2, start=180, extent=90, fill=color, outline=color)
            _bc.create_arc(_bw-2-_r*2, _bh-2-_r*2, _bw-2, _bh-2, start=270, extent=90, fill=color, outline=color)
            _bc.create_rectangle(2+_r, 2, _bw-2-_r, _bh-2, fill=color, outline=color)
            _bc.create_rectangle(2, 2+_r, _bw-2, _bh-2-_r, fill=color, outline=color)
            _bc.create_text(_bw//2, _bh//2, text=lbl, fill='#ffffff',
                            font=('맑은 고딕', 9, 'bold'))
            if idx < len(STAGES) - 1:
                tk.Label(
                    badge_row, text='  →  ',
    ...
    def _setup_dash_cards(self, ctx) -> None:
        ...
        # v7.3.2: 5단계 카드 — v7.1.8 스타일 색상 (판매가능=초록, 판매배정=청록, 화물결정=빨강, 출고=빨강, 반품=청록)
        card_configs = [
            ('status_available', '판매가능',         '0개', '#10B981'),   # 에메랄드 초록
            ('status_reserved',  '판매배정',         '0개', '#06b6d4'),   # 청록
            ('status_picked',    '화물결정',         '0개', '#ef4444'),   # 빨강
            ('status_sold',      '출고완료',         '0개', '#dc2626'),   # 딥 레드
            ('status_return',    '반품대기',         '0개', '#0891b2'),   # 청록
        ]
    ...
    def _create_dashboard_card(self, parent, title: str, value: str, color: str, fonts=None) -> 'ttk.Frame':
        ...
        # 콘텐츠 영역 (v7.3.2: 넓은 패딩 — v7.1.8 스타일)
        content = tk.Frame(inner, bg=_card_bg, padx=14, pady=10)

...
class SQMInventoryApp:
    ...
    def __init__(self, root=None, db_path: Optional[str] = None):
        ...
        if root is None:
            if HAS_TTKBOOTSTRAP:
                ...
                try:
                    ...
                    # 현재 테마 + 양쪽 모두 패치 (전환 대비)
                    for _t, _c in [('darkly', _gy_dark), ('flatly', _gy_light)]:
                        if _t in STANDARD_THEMES:
                            STANDARD_THEMES[_t]['colors'].update(_c)
        ...
        # v7.3.3: 중복 스타일 오버라이드 제거 — global_tree_style + 탭별 스타일로 충분
        logger.info("✅ v7.3.3: 통합 스타일 적용 (중복 오버라이드 제거됨)")

        # v7.3.3: Windows 11 둥근 모서리 + 다크 타이틀바
        try:
            from .utils.win32_styling import apply_win11_style
            from .utils.ui_constants import ThemeColors
            _is_dark_win = ThemeColors.is_dark_theme(getattr(self, 'current_theme', 'darkly'))
            apply_win11_style(self.root, rounded=True, dark_titlebar=_is_dark_win)
        except Exception as _e:
            logger.debug(f"Windows 11 스타일 적용 건너뜀: {_e}")
    ...
    def _setup_ui(self) -> None:
        ...
        from .utils.constants import ttk, tk, BOTH, YES
        from .utils.ui_constants import ThemeColors
        ...
        # v7.3.3: Notebook 탭 스타일 — ThemeColors 참조 (하드코딩 제거)
        try:
            _nb_style = ttk.Style()
            _is_dark = ThemeColors.is_dark_theme(getattr(self, 'current_theme', 'flatly'))
            _sel_bg = ThemeColors.get('danger', _is_dark)      # 선택 탭: 빨강 계열
            _sel_fg = '#ffffff'
            _unsel_bg = ThemeColors.get('bg_secondary', _is_dark)
            _unsel_fg = ThemeColors.get('text_muted', _is_dark)
            _nb_style.configure('TNotebook.Tab', font=('맑은 고딕', 11, 'bold'),
                                padding=[12, 6])
            _nb_style.map('TNotebook.Tab',
                          background=[('selected', _sel_bg), ('!selected', _unsel_bg)],
                          foreground=[('selected', _sel_fg), ('!selected', _unsel_fg)])
        except (RuntimeError, ValueError, AttributeError, tk.TclError) as _e:
            logger.debug(f"Notebook tab style: {_e}")
        ...
        self.tab_scan = ttk.Frame(self.notebook)  # v7.3.5: 스캔 탭
        ...
        self.notebook.add(self.tab_picked, text=" 🎯 화물결정 ")
        self.notebook.add(self.tab_sold, text=" 🚚 출고 ")
        self.notebook.add(self.tab_scan, text=" 🔍 스캔 ")  # v7.3.5: 스캔 탭
        self.notebook.add(self.tab_cargo_overview, text=" 📊 총괄재고 ")
        ...
        def _on_notebook_tab_changed(event):
            try:
                ...
                # 0=판매가능, 1=판매배정, 2=판매화물 결정, 3=출고, 4=스캔, 5=총괄 재고, 6=통계, 7=로그
                idx_to_key = {0: 'inventory', 1: 'allocation', 2: 'picked', 3: 'sold', 4: 'scan', 5: 'cargo_overview', 6: 'dashboard', 7: 'log'}
                ...
                if key == 'inventory' and hasattr(self, '_refresh_inventory'):
                    ...
                    pass  # 스캔 탭은 바코드 입력 기반, 자동 새로고침 불필요
                elif key == 'dashboard':
                    ...
                    # v7.3.5: KPI 카드 갱신
                    if hasattr(self, '_refresh_ops_center'):
                        try:
    ...
    def _enforce_main_notebook_hidden_tabs(self) -> None:
        ...
        사이드바와 동일한 Sidebar.TNotebook 스타일 재사용."""
    ...
    def _setup_ui_helper(self) -> None:
        ...
        try:
            ...
            # v7.3.3: Treeview 행 호버 하이라이트
            self.root.after(2500, self._enable_tree_hover)

            # v7.3.5: audit_log + outbound_scan_log 테이블 확보
            self.root.after(500, self._ensure_scan_tables)

            # v7.3.5: 사이드바 네비게이션
            self.root.after(3000, self._build_sidebar_safe)

            # v7.3.5: KPI 대시보드 카드 (통계 탭)
            self.root.after(3500, self._build_ops_center_safe)
    ...
    def _enable_tree_hover(self) -> None:
        """v7.3.4: 모든 주요 Treeview에 TreeHoverEngine + 헤더 정렬 적용"""
        try:
            from .utils.tree_enhancements import TreeHoverEngine, style_treeview_header
            for attr in ('tree_inventory', 'tree_sublot', 'tree_allocation',
                ...
                if tree:
                    try:
                        ...
                        engine.attach()
                        style_treeview_header(tree)
                    except Exception:
                        pass
            logger.debug("[v7.3.4] TreeHoverEngine + 헤더 정렬 적용 완료")
        except (ImportError, Exception) as e:
            logger.debug(f"Treeview 호버/정렬 적용 실패: {e}")

    def _ensure_scan_tables(self) -> None:
        """v7.3.5: audit_log + outbound_scan_log 테이블 자동 생성."""
        try:
            from .utils.db_helper import ensure_tables
            ensure_tables(self)
            logger.debug("[v7.3.5] 스캔 관련 테이블 확인 완료")
        except Exception as e:
            logger.debug(f"ensure_scan_tables: {e}")

    def _build_sidebar_safe(self) -> None:
        """v7.3.5: 사이드바 안전 빌드."""
        try:
            if hasattr(self, '_build_sidebar'):
                self._build_sidebar()
        except Exception as e:
            logger.debug(f"sidebar build: {e}")

    def _build_ops_center_safe(self) -> None:
        """v7.3.5: KPI 대시보드 카드 안전 빌드."""
        try:
            if hasattr(self, '_build_ops_center') and hasattr(self, 'tab_dashboard'):
                self._build_ops_center(self.tab_dashboard)
                self._refresh_ops_center()
        except Exception as e:
            logger.debug(f"ops_center build: {e}")

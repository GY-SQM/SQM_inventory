...
class StatusBarMixin:
    ...
    def _setup_status_bar(self) -> None:
        """
        v7.3.2: v7.1.8 스타일 간결 상태바

        Layout:
        ┌──────────────────────────────────────────────────────────────────┐
        │ [●WRITE] [정상 운영 중] | DB: sqm.db | 재고 요약 | 갱신시각   │
        └──────────────────────────────────────────────────────────────────┘
        """
        ...
        # v7.3.3: 상태바 — 깔끔한 간격 + 구분선
        self.statusbar_frame = ttk.Frame(self.main_frame)
        self.statusbar_frame.pack(fill=X, side=BOTTOM, pady=(4, 0))

        # Separator (상단 구분선)
        ttk.Separator(self.statusbar_frame, orient=HORIZONTAL).pack(fill=X)

        # Inner container (패딩 확대)
        status_container = ttk.Frame(self.statusbar_frame, padding=(12, 6))
        status_container.pack(fill=X)

        # ── 좌측: 모드 + 상태 메시지 ──
        # Read/Write mode indicator (간결)
        self.mode_label = ttk.Label(
            status_container,
            text="● WRITE",
            foreground=_p.get('success', '#10B981'),
            font=("맑은 고딕", 9, "bold"),
            anchor="w"
        )
        ...
        ttk.Separator(status_container, orient=VERTICAL).pack(side=LEFT, fill=Y, padx=4)

        # Status message
        self.status_var_bar = tk.StringVar(value="정상 운영 중")
        ...
        try:
            import pathlib
            _db_path = pathlib.Path(getattr(self.engine, 'db_path', 'data/db/sqm.db'))
            _db_name = _db_path.name
        except Exception:
            pass
        ttk.Label(
            status_container, text=f"DB: {_db_name}",
            font=("맑은 고딕", 9), foreground=_p.get('text_secondary', 'gray')
        ...
        self._inv_summary_label.pack(side=LEFT, fill=X, expand=True, padx=(0, 8))
        ...
        self._move_pending_label.pack(side=LEFT, padx=(0, 8))
        ...
        ttk.Separator(right_container, orient=VERTICAL).pack(side=LEFT, fill=Y, padx=4)
        ...
        self._api_status_label = ttk.Label(
            right_container, text=f"{sec_icon}{sec_text}",
            font=("맑은 고딕", 9), foreground='gray')
        self._api_status_label.pack(side=LEFT, padx=(0, 4))

        self._backup_status_var = tk.StringVar(value="💾--:--")
        self._backup_status_label = ttk.Label(
            right_container, textvariable=self._backup_status_var,
            font=("맑은 고딕", 9), foreground='gray')
        self._backup_status_label.pack(side=LEFT, padx=(0, 8))
        ...
        self.task_detail_label.pack(side=LEFT, padx=(0, 4))
    ...
    def _show_module_status_dialog(self) -> None:
        ...
        for i, (mod, desc) in enumerate(modules):
            ...
            ttk.Label(frame, text=desc, foreground="gray").grid(row=i, column=1, sticky=W, padx=5)

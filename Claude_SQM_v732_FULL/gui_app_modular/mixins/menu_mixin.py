...
class MenuMixin:
    ...
    def _setup_native_menu(self) -> None:
        """기존 네이티브 메뉴바 (fallback) — v7.3.2: 입고/출고 독립 최상위 메뉴 분리"""
        ...
        # 📥 입고 메뉴 (최상위)
        # =====================================================
        inbound_menu = tk.Menu(menubar, tearoff=0, font=_font)
        ...
        for entry in FILE_MENU_INBOUND_ITEMS:
            ...
            if method_name == "_show_return_dialog":
                _return_cmd = getattr(self, method_name, None)
                pending = self._get_return_doc_review_pending_count(30)
                badge = self._format_return_review_badge(pending)
                return_label = f"{label}{badge}"
                if callable(_return_cmd):
                    return_sub = tk.Menu(inbound_menu, tearoff=0, font=_font)
                    inbound_menu.add_cascade(label=return_label, menu=return_sub, font=_font)
                    for sub_label, mode in FILE_MENU_INBOUND_RETURN_SUB_ITEMS:
                        return_sub.add_command(label="  " + sub_label, command=lambda md=mode, fn=_return_cmd: fn(md), font=_font)
                else:
            ...
            if callable(cmd):
                inbound_menu.add_command(label="  " + label, command=cmd, font=_font)

        # =====================================================
        # 📤 출고 메뉴 (최상위)
        # =====================================================
        outbound_menu = tk.Menu(menubar, tearoff=0, font=_font)
        ...
        for entry in FILE_MENU_OUTBOUND_ITEMS:
            ...
            cmd = getattr(self, method_name, None)
            if callable(cmd):
                outbound_menu.add_command(label="  " + label, command=cmd, font=_font)

        # =====================================================
        # 📁 파일 메뉴 (내보내기, 백업, 최근 파일, 종료)
        # =====================================================

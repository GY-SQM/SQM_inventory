...
class InboundTemplateDialog:
    ...
    def __init__(self, parent, engine, current_theme='flatly', on_select_callback=None):
        self.parent = parent
        self.engine = engine
        ...
        self.bg = ThemeColors.get('bg_card', is_dark) if ThemeColors else '#FFFFFF'
    ...
    def _build_ui(self):
        ...
        # 헤더 (모드별 텍스트)
        hdr = tk.Frame(self.popup, bg='#1A5276' if _is_select_mode else '#2C3E50',
                       padx=12, pady=8)
        hdr.pack(fill=X)
        _title = ('🔍 파싱 템플릿 선택'
        ...
        tk.Label(hdr, text=_title,
                 font=('맑은 고딕',13,'bold'), bg=hdr.cget('bg'), fg='white').pack(anchor='w')
        tk.Label(hdr, text=_subtitle,
                 font=('맑은 고딕',10), bg=hdr.cget('bg'), fg='#BDC3C7').pack(anchor='w')

        # 본문
        body = tk.Frame(self.popup, bg=self.bg)
        body.pack(fill=BOTH, expand=True, padx=8, pady=6)

        left = ttk.LabelFrame(body, text='  템플릿 목록  ')
    ...
    def _build_tab_basic(self, p):
        ...
        def row(r, label, widget_fn, tip=None):
            ...
            widget_fn(r*2)
            if tip:
                ttk.Label(p, text=tip, font=('맑은 고딕',9),
        ...
        def e_bag(r):
            ...
            self._bag_badge=tk.Label(frm,text='',font=('맑은 고딕',10,'bold'),
                                     fg='white',bg='#2980B9',relief='flat',padx=6,pady=1)
    ...
    def _refresh_bag_badge(self, _=None):
        try:
            ...
            bg = '#2980B9' if v == 500 else '#E67E22'
    ...
    def _build_tab_gemini(self, p):
        ...
        tk.Label(p,
                 ...
                 font=('맑은 고딕',9),fg='gray',anchor='w').grid(
            row=0,column=0,sticky='w',padx=8,pady=(6,2))
        self._text_hint_pl  = self._hint_box(p,'📦 PackingList 힌트', 1)
        self._text_hint_inv = self._hint_box(p,'🧾 Invoice 힌트',     3)
        self._text_hint_bl  = self._hint_box(p,'🚢 B/L 힌트',         5)
    ...
    def _on_new(self):
        self._show_template({'template_id':'','template_name':'',
            ...
            'product_hint':'LITHIUM CARBONATE','weight_format':'EURO',
            'gemini_hint_packing':'','gemini_hint_invoice':'','gemini_hint_bl':'',
            'note':'','is_active':1})
        self._selected_id=''

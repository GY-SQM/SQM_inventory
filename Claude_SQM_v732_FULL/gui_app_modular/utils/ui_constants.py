...
class ThemeColors:
    ...
    # v7.3.3: Soft Minimalism — 깨끗한 화이트 + Tailwind Slate + 에메랄드 통일
    LIGHT = {
        # Semantic (다크와 동일 accent — 통일감)
        'success': '#10b981',    # Emerald
        'warning': '#f59e0b',    # Amber
        'danger':  '#ef4444',    # Red
        'info':    '#0ea5e9',    # Sky Blue
        'primary': '#10B981',    # Emerald (accent 통일)

        # Status (Treeview 행 배경 — 극히 연한 파스텔)
        'available': '#ecfdf5',   # Emerald 50
        'picked':    '#fef2f2',   # Red 50
        'reserved':  '#fffbeb',   # Amber 50
        'shipped':   '#eff6ff',   # Blue 50
        ...
        'bg_toolbar':   '#ffffff',   # Pure White
        'bg_card':      '#ffffff',   # Pure White

        # Border (Subtle)
        'border':       '#e2e8f0',   # Slate 200
        'border_focus': '#10B981',   # Emerald

        # Buttons (다크와 동일 — 통일감)
        'btn_inbound':       '#10b981',
        'btn_inbound_hover': '#34d399',
        'btn_outbound':      '#ef4444',
        'btn_outbound_hover':'#f87171',
        'btn_report':        '#0ea5e9',
        'btn_report_hover':  '#38bdf8',
        'btn_neutral':       '#64748b',
        'btn_neutral_hover': '#94a3b8',

        # Treeview
        'tree_select_bg':  '#dbeafe',   # Blue 100
        'tree_select_fg':  '#1e3a5f',   # Dark Blue
        'tree_stripe':     '#f8fafc',   # Slate 50
        ...
        'search_cursor':      '#1e293b',

        # Statusbar
        'statusbar_bg':       '#1e293b',   # Slate 800
        'statusbar_fg':       '#f8fafc',
        'statusbar_icon_ok':  '#10b981',
        'statusbar_icon_warn':'#f59e0b',
        'statusbar_icon_err': '#ef4444',
        'statusbar_progress': '#0ea5e9',
        'statusbar_progress_done': '#10b981',
        'statusbar_track':    '#334155',

        # Badges
        'badge_db':           '#10b981',
        'badge_version':      '#0ea5e9',
        'badge_text':         '#ffffff',
        ...
        'canvas_highlight':   '#1e293b',
    }

    DARK = {
    # Semantic (GY Navy + Emerald)
    'success': '#10b981',    # Emerald
    'warning': '#f59e0b',    # Amber
    'danger':  '#ef4444',    # Red
    'info':    '#0ea5e9',    # Sky Blue
    ...
    # Status (Row backgrounds — 어두운 톤으로 가독성 확보)
    'available': '#065f46',  # Deep Emerald (판매가능)
    'reserved':  '#92400e',  # Deep Amber (판매배정)
    'picked':    '#5b21b6',  # Deep Purple (판매화물 결정)
    'shipped':   '#1e3a5f',  # Deep Navy (출고/정보)

    # Text (High Contrast on Navy)
    'text_primary':   '#e2e8f0',   # Bright Slate
    'text_secondary': '#94a3b8',   # Slate
    'text_muted':     '#64748b',   # Muted Slate

    # Background (Deep Navy — toolbar/main_app과 동일)
    'bg_primary':   '#0b1120',     # Deep Navy Black
    'bg_secondary': '#111827',     # Slightly lighter Navy
    'bg_hover':     '#1e293b',     # Slate hover
    'bg_toolbar':   '#0b1120',     # Deep Navy Black
    'bg_card':      '#1e293b',     # Slate (card/panel)

    # Border
    'border':       '#1e3a5f',     # Navy Border
    'border_focus': '#10B981',     # Emerald Focus

    # Buttons
    'btn_inbound':        '#10b981',   # Emerald
    'btn_inbound_hover':  '#34d399',
    'btn_outbound':       '#ef4444',   # Red
    'btn_outbound_hover': '#f87171',
    'btn_report':         '#0ea5e9',   # Sky Blue
    'btn_report_hover':   '#38bdf8',
    'btn_neutral':        '#475569',   # Slate
    'btn_neutral_hover':  '#64748b',

    # Treeview selection
    'tree_select_bg':  '#1d4ed8',   # Blue
    'tree_select_fg':  '#ffffff',
    'tree_stripe':     '#0f1a2e',   # Darker Navy stripe
    ...
    'search_bg':  '#111827',
    'search_fg':  '#e2e8f0',
    'search_border':      '#1e3a5f',
    'search_placeholder': '#64748b',
    'search_cursor':      '#e2e8f0',
    ...
    # Misc UI
    'arrow_separator':    '#334155',
    'shortcut_text':      '#94a3b8',
    'shortcut_text_dim':  '#64748b',
    'canvas_highlight':   '#e2e8f0',
}
    ...
    def configure_tags(cls, tree, is_dark: bool = False):
        ...
        tree.tag_configure('depleted', background='#f1f5f9' if not is_dark else '#111827',
                          foreground='#94a3b8' if not is_dark else '#64748b')
...
class ReadableStyle:
    ...
    # Treeview 행 높이 — v7.3.3: 30px 통일 (global_tree_style과 일치)
    ROW_HEIGHT = 30

    # 기본 폰트 — v7.3.3: 11pt 통일 (탭별 Treeview와 일치)
    FONT_FAMILY = '맑은 고딕'
    FONT_SIZE = 11
    HEADING_SIZE = 11
    ...
    def apply(cls, root, theme_name: str = 'flatly'):
        ...
        try:
            ...
            # v7.3.3: ttkbootstrap style.colors 직접 수정 제거
            # → STANDARD_THEMES 패치(main_app.py)가 이미 처리
            ...
            style.map(
                'TEntry',
                foreground=[('disabled', p['text_muted']), ('!disabled', p['text_primary'])],
                fieldbackground=[('disabled', p['bg_secondary']), ('!disabled', p['bg_card'])],
            )
            ...
            # Combobox 드롭다운 Listbox 색상 (option add)
            try:
                root.option_add('*TCombobox*Listbox.foreground', p['text_primary'])
                root.option_add('*TCombobox*Listbox.background', p['bg_card'])
                root.option_add('*TCombobox*Listbox.selectForeground', p['tree_select_fg'])
                root.option_add('*TCombobox*Listbox.selectBackground', p['tree_select_bg'])
            except Exception:
                pass
...
def apply_dialog_theme(dialog, theme_name: str = None) -> None:
    """v7.3.3: 다이얼로그에 통일 테마 적용 (배경색 + DWM 둥근 모서리 + 다크 타이틀바 + ThemeAware 위젯 수정)."""
    try:
        if theme_name is None:
            try:
                from tkinter import ttk as _ttk
                theme_name = _ttk.Style().theme_use() or 'flatly'
            except Exception:
                theme_name = 'flatly'
        is_dark = ThemeColors.is_dark_theme(theme_name)
        ...
        # DWM 둥근 모서리 + 다크 타이틀바
        try:
            from .win32_styling import apply_win11_style
            apply_win11_style(dialog, rounded=True, dark_titlebar=is_dark)
        except (ImportError, Exception):
            pass
        # v7.3.4: ThemeAware — 하드코딩 색상 자동 수정 (다크모드 가시성 보장)
        try:
            from .theme_aware import ThemeAware
            ThemeAware.apply_to_toplevel(dialog)
        except (ImportError, Exception):
            pass
    except Exception as e:
        logger.debug(f"[apply_dialog_theme] Suppressed: {e}")


def apply_contrast_scrollbar_style(root, theme_name: str = 'flatly') -> None:
    """v7.3.3: 전역 Scrollbar — 테마 팔레트 연동, 부드러운 톤."""
    try:
        is_dark = ThemeColors.is_dark_theme(theme_name)
        if is_dark:
            trough = '#111827'   # Navy secondary
            thumb = '#334155'    # Slate 700
            active = '#475569'   # Slate 600
        else:
            trough = '#f1f5f9'   # Slate 100

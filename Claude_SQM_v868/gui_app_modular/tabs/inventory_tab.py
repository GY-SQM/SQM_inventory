...
# ═══════════════════════════════════════════════════════════════
INVENTORY_COLUMNS = [
    ('row_num',            'No.',            58, 'center', True),
    ('lot_no',             'LOT NO',        132, 'center', True),
    ('sap_no',             'SAP NO',        132, 'center', True),
    ('bl_no',              'BL NO',         152, 'center', True),
    ('product',            'PRODUCT',       172, 'center', True),
    ('status',             'STATUS',         98, 'center', True),
    ('current_weight',     'Balance(Kg)',    112, 'center', True),
    ('net_weight',         'NET(Kg)',        112, 'center', True),
    ('container_no',       'CONTAINER',      142, 'center', True),
    ('mxbg_pallet',        'MXBG',           82, 'center', True),
    ('avail_bags',         'Avail',          72, 'center', True),
    ('tb_avail',           '↓Avail개',       74, 'center', False),
    ('tb_reserved',        '↓Resv개',        74, 'center', False),
    ('tb_picked',          '↓Pick개',        74, 'center', False),
    ('tb_sold',            '↓Sold개',        74, 'center', False),
    ('salar_invoice_no',   'INVOICE NO',    112, 'center', True),
    ...
    ('warehouse',          'WH',             92, 'center', True),
    ('customs',            'CUSTOMS',       102, 'center', True),
    ('initial_weight',     'Inbound(Kg)',    112, 'center', True),
    ('outbound_weight',    'Outbound(Kg)',   112, 'center', True),
]
...
class InventoryTabMixin:
    ...
    def _setup_inventory_tab(self) -> None:
        """재고 현황 탭 설정"""
        self._setup_inv_init_vars()
        self._setup_inv_filter_and_toggle()
        self._setup_inv_treeview()
        self._setup_inv_header_filter_connect()
        self._setup_inv_events_and_tonbag_view()

    def _setup_inv_init_vars(self) -> None:
        """재고 탭 초기 변수 + 헤더 + 컨테이너."""
        ...
        self._inv_view_mode = tk.StringVar(value='lot')

    def _setup_inv_filter_and_toggle(self) -> None:
        """재고 탭 필터바 + 컬럼 토글바."""
        from ..utils.constants import ttk, tk, X
    ...
    def _setup_inv_treeview(self) -> None:
        """재고 탭 트리뷰 + 스타일 + 스크롤바."""
    ...
    def _setup_inv_header_filter_connect(self) -> None:
        """재고 탭 헤더 정렬/필터 연결 + 토글바 연결."""
        from ..utils.constants import ttk, tk, X
    ...
    def _setup_inv_events_and_tonbag_view(self) -> None:
        """재고 탭 이벤트 바인딩 + 톤백 보기 뷰."""
        from ..utils.constants import ttk, tk, VERTICAL, BOTH, YES, LEFT, X
        from ..utils.tree_enhancements import TreeviewTotalFooter

        self.tree_inventory.bind('<Double-1>', self._on_inv_double_click)

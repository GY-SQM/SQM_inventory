...
class TonbagTabMixin:
    ...
    def _setup_tonbag_tab(self) -> None:
        ...
        self._setup_tonbag_header_and_columns()
        self._setup_tonbag_filter_bar()
        self._setup_tonbag_treeview()
        self._setup_tonbag_action_bar()
        self._setup_tonbag_events()

    def _setup_tonbag_header_and_columns(self) -> None:
        """톤백 탭 헤더 + 컬럼 정의."""
        # 탭 헤더 (v7.3.7 심플화)
        try:
    ...
    def _setup_tonbag_filter_bar(self) -> None:
        """톤백 탭 필터바 + 컬럼 토글바 생성."""
    ...
    def _setup_tonbag_treeview(self) -> None:
        """톤백 탭 트리뷰 + 스크롤바 + 스타일 생성."""
        from ..utils.constants import ttk, tk, VERTICAL, HORIZONTAL, BOTH, YES, LEFT, RIGHT, X, Y
        from ..utils.ui_constants import Spacing, ThemeColors
    ...
    def _setup_tonbag_action_bar(self) -> None:
        """톤백 탭 액션 버튼 + 상태필터 + 검색 바 생성."""
        from ..utils.constants import ttk, tk, LEFT, RIGHT, X
        from ..utils.ui_constants import Spacing, ThemeColors, apply_tooltip
    ...
    def _setup_tonbag_events(self) -> None:
        """톤백 탭 이벤트 바인딩."""

...
const thStyle = {
  padding: '6px 8px', textAlign: 'center', background: '#f8fafc',
  borderBottom: '2px solid #e2e8f0', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap',
};
...
export default function ReturnPage() {
  ...
  useEffect(() => {
    getReturnStatistics().then(setStats).catch(() => {});
  }, []);
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {tab === 'single' && <SingleTab onDone={() => { setTab('history'); getReturnStatistics().then(setStats).catch(() => {}); }} />}
      {tab === 'excel' && <ExcelTab onDone={() => { setTab('history'); getReturnStatistics().then(setStats).catch(() => {}); }} />}
    </div>
  );
}

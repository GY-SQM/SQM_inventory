...
def run_analysis(pdf_path: str, carrier_id: str = "") -> None:
    """메인 분석 실행"""
    print(f"\n{'='*65}")
    print(f"  BL 선사 패턴 분석 도구  —  SQM v6.4.0")
    print(f"{'='*65}")
    print(f"  파일: {pdf_path}")
    ...
    try:
        ...
        from bl_carrier_registry import detect_carrier, CARRIER_TEMPLATES
    ...
    print(f"\n  {'─'*60}")
    print(f"  📄 bl_carrier_registry.py에 붙여넣을 코드:")
    print(f"  {'─'*60}")
    print(code)
    print(f"  {'─'*60}")
    print(f"  💡 사용법: CARRIER_TEMPLATES 딕셔너리에 위 코드 추가")
    print(f"  💡 이후: python bl_carrier_update_tool.py <pdf> 로 검증")

...
from ..utils.ui_constants import DialogSize, apply_modal_window_options, center_dialog, setup_dialog_geometry_persistence
...
class AllocationPreviewDialog:
    ...
    def _build_ui(self):
        ...
        for product_name, pstats in s['by_product'].items():
            ...
            ttk.Label(summary_frame, text=line, font=('맑은 고딕', 9)).pack(anchor='w')
        ...
        tree.heading('qty_mt', text='QTY(MT)')
        tree.heading('qty_kg', text='QTY(kg)')
        tree.heading('sub_lt', text='톤백#')
        tree.heading('sold_to', text='출고처')
        tree.heading('warehouse', text='WH')
        tree.heading('customs', text='Customs')
        ...
        # 스타일 (샘플 = 파란색 배경)
        tree.tag_configure('sample', background='#EBF5FB', foreground='#0066CC')
        tree.tag_configure('tonbag', background='#FFFFFF')

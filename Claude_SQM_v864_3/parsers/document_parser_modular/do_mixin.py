...
class DOMixin:
    ...
    def _parse_do_one_coord(self, pdf_path: str) -> "Optional[DOData]":
        """v9.2: ONE(Ocean Network Express) D/O 발급확인서 PyMuPDF 좌표 기반 파싱.

        Gemini API 불필요. 한국 법인(ONE KOREA) 발급 고정 레이아웃 → 좌표 직접 추출.
        ...
        추출 필드:
            ...
            gross_weight_kg, cbm
            mrn, msn
            containers[]: {container_no, free_time, free_time_date}
        """
        import fitz
        import re as _re
        from ..document_models import DOData, ContainerInfo, FreeTimeInfo
        ...
        def clean_num(s):
            return _re.sub(r"[^\d.]", "", (s or "").replace(",", ""))
        ...
        is_one_form = (
            ...
            or ("ONEY" in up_text)
            or ("E-D/O" in up_text)
            or ("ONE-LINE.COM" in up_text)
            or ("DELIVERY" in up_text and "D /O" in up_text)
        )
        if not is_one_form:
            logger.debug("[DO-ONE] ONE 양식 키워드 미감지 → 상위 폴백")
            return None

        # ── 실측 좌표 추출 (ONEYSCLG01825300 샘플 기준) ──────
        do_no_raw = by_xy(55.0, 72.0, 7.2, 8.8)
        do_no = join_no_space(do_no_raw)

        bl_no_raw = by_xy(55.0, 73.0, 11.0, 12.5)
        bl_no_joined = join_no_space(bl_no_raw)
        bm = _re.search(r"(ONEY[A-Z0-9]{10,14})", bl_no_joined)
        bl_no = bm.group(1) if bm else bl_no_joined

        issue_raw = by_xy(75.0, 95.0, 7.5, 8.8)
        issue_date = issue_raw.split()[0] if issue_raw else ""

        mrn_raw = by_xy(54.0, 72.0, 15.0, 16.5)
        mrn_joined = join_no_space(mrn_raw)
        mm = _re.search(r"([0-9]{2}[A-Z]{4}[A-Z0-9]{3,8})", mrn_joined)
        mrn = mm.group(1) if mm else ""
        msn_m = _re.search(r"-\s*(\d{3,6})", mrn_joined)
        msn = msn_m.group(1) if msn_m else ""

        arrival_raw = by_xy(82.0, 95.0, 15.0, 16.5)
        arrival_date_obj = parse_ymd(arrival_raw)

        vessel_raw = by_xy(54.0, 73.0, 19.0, 20.2)
        vessel_nospace = join_no_space(vessel_raw)
        VESSEL_PREFIXES = (
            "HMM", "MSC", "EVER", "EVERGREEN", "MAERSK",
            "HYUNDAI", "CMA", "COSCO", "OOCL", "ONE", "YMLU", "HAPAG",
        )
        vessel = vessel_nospace
        for _p in VESSEL_PREFIXES:
            if vessel_nospace.startswith(_p) and len(vessel_nospace) > len(_p):
                vessel = f"{_p} {vessel_nospace[len(_p):]}"
                break

        voyage = by_xy(85.0, 95.0, 19.0, 20.2)

        pol = by_xy(20.0, 40.0, 34.0, 35.5).strip()
        pod = join_no_space(by_xy(20.0, 40.0, 38.0, 39.5))

        gw_raw = by_xy(41.0, 55.0, 38.0, 39.5)
        try:
            gross_weight_kg = float(clean_num(gw_raw)) if gw_raw else 0.0
        except (ValueError, TypeError):
            gross_weight_kg = 0.0

        cbm_raw = by_xy(42.0, 55.0, 41.8, 43.0)
        try:
            cbm = float(clean_num(cbm_raw)) if cbm_raw else 0.0
        except (ValueError, TypeError):
            cbm = 0.0

        dem_ft_raw = by_xy(68.0, 82.0, 30.5, 31.5)
        det_ft_raw = by_xy(68.0, 82.0, 32.5, 33.5)
        dem_free_time = parse_ymd(dem_ft_raw)
        det_free_time = parse_ymd(det_ft_raw)
        rep_ft_obj = dem_free_time or det_free_time
        rep_ft_str = rep_ft_obj.strftime("%Y-%m-%d") if rep_ft_obj else ""

        # ── 컨테이너 테이블 (y=40~60%, 약 2% 간격) ───────────
        CT_RE_ONE = _re.compile(r"([A-Z]{4}\d{7})")
        containers = []
        seen_ct = set()
        y = 39.5
        while y < 60.0:
            ct_raw = by_xy(53.0, 68.0, y, y + 2.1)
            ct_joined = join_no_space(ct_raw).upper()
            cm = CT_RE_ONE.search(ct_joined)
            if cm:
                ct_no = cm.group(1)
                if ct_no not in seen_ct:
                    seen_ct.add(ct_no)
                    type_raw = by_xy(68.0, 75.0, y, y + 2.1)
                    ct_type = join_no_space(type_raw)
                    dem_ft_row = by_xy(75.0, 83.0, y, y + 2.1)
                    det_ft_row = by_xy(83.0, 93.0, y, y + 2.1)

                    def _yyyymmdd_to_ymd(s):
                        mx = _re.search(r"(\d{4})(\d{2})(\d{2})", join_no_space(s))
                        return f"{mx.group(1)}-{mx.group(2)}-{mx.group(3)}" if mx else ""

                    dem_str = _yyyymmdd_to_ymd(dem_ft_row) or rep_ft_str
                    det_str = _yyyymmdd_to_ymd(det_ft_row) or rep_ft_str
                    containers.append({
                        "container_no": ct_no,
                        "type": ct_type,
                        "free_time": dem_str or det_str,
                    })
            y += 2.0
        ...
        if arrival_date_obj is not None:
            result.arrival_date = arrival_date_obj
        result.gross_weight_kg = gross_weight_kg
        result.cbm = cbm
        ...
        for c in containers:
            ci = ContainerInfo()
            ci.container_no = c["container_no"]
            if c.get("type"):
                if hasattr(ci, "container_type"):
                    ci.container_type = c["type"]
                elif hasattr(ci, "type"):
                    setattr(ci, "type", c["type"])
            ft = c.get("free_time", "")
            if ft:
                ci.free_time = ft
                ci.free_time_date = ft
            result.containers.append(ci)

            if c["container_no"] or ft:
                result.free_time_info.append(FreeTimeInfo(
        ...
        result.success = bool(result.bl_no and result.do_no and result.containers)
        logger.info(
            f"[DO-ONE] 좌표 파싱 v9.2: do={do_no} bl={bl_no} "
            f"vessel={vessel!r} voy={voyage} mrn={mrn}/msn={msn} "
            f"arrival={result.arrival_date} pol={pol} pod={pod} "
            f"wt={gross_weight_kg} cbm={cbm} "
            f"containers={len(containers)}개 "
            f"dem_ft={dem_free_time} det_ft={det_free_time}"
        )
        return result

    def _parse_do_hapag_coord(self, pdf_path: str) -> "Optional[DOData]":
        """v9.3: Hapag-Lloyd D/O 발급확인서 PyMuPDF 좌표 기반 파싱.

        Gemini API 불필요. 한국 법인(Hapag-Lloyd Korea) 발급 고정 레이아웃 → 좌표 직접 추출.
        샘플: HLCUSCL260148627 DO.pdf (595x842, A4)

        감지 키워드 (is_hapag_form):
          - "HLCUSCL" (B/L 번호 prefix)
          - "HAPAGLLOYD" (공백/- 제거 후)
          - "HLCU" + "KOREA@SERVICE.HLAG" (한국 법인 서비스 이메일)
          - HAMU\\d{7} regex (컨테이너 번호)
        """
        # v8.7.1 [v864_2 호환]: core.pdf_engine 대신 fitz.open 직접 사용
        import fitz
        import re as _re
        from ..document_models import DOData, ContainerInfo, FreeTimeInfo
        ...
        def parse_yyyymmdd(s):
            if not s:
                return ""
            m = _re.search(r"(\d{4})(\d{2})(\d{2})", s.replace(" ", ""))
            return f"{m.group(1)}-{m.group(2)}-{m.group(3)}" if m else ""

        up_text = (page_text or "").upper()
        is_hapag_form = (
            ("HLCUSCL" in up_text)
            or ("HAPAGLLOYD" in up_text.replace("-", "").replace(" ", ""))
            or ("HLCU" in up_text and "KOREA@SERVICE.HLAG" in up_text)
            or bool(_re.search(r"\bHAMU\d{7}\b", up_text))
        )
        if not is_hapag_form:
            logger.debug("[DO-HAPAG] Hapag 양식 키워드 미감지 → 상위 폴백")
            return None

        do_no = by_xy(55.0, 70.0, 4.0, 5.5).replace(" ", "")

        issue_raw = by_xy(85.0, 98.0, 4.0, 5.5)
        issue_date = issue_raw.split()[0] if issue_raw else ""

        bl_no_raw = by_xy(50.0, 70.0, 8.0, 9.5)
        bm = _re.search(r"(HLCU[A-Z0-9]{8,14})", bl_no_raw.replace(" ", ""))
        bl_no = bm.group(1) if bm else bl_no_raw.replace(" ", "")

        mrn_raw = by_xy(54.0, 72.0, 12.3, 13.5)
        mrn_joined = mrn_raw.replace(" ", "")
        mm = _re.search(r"([0-9]{2}HLCU[A-Z0-9]{4,8})", mrn_joined)
        mrn = mm.group(1) if mm else ""
        msn_m = _re.search(r"-(\d{3,6})", mrn_joined)
        msn = msn_m.group(1) if msn_m else ""

        arrival_raw = by_xy(85.0, 98.0, 12.3, 13.5)
        arrival_date_obj = parse_ymd(arrival_raw)

        vessel_raw = by_xy(50.0, 85.0, 16.3, 17.8)
        vessel = _re.sub(r"\s+", " ", vessel_raw).strip()

        voyage = by_xy(90.0, 98.0, 16.3, 17.8)

        pol = by_xy(15.0, 40.0, 29.8, 31.5).strip()
        pod = by_xy(15.0, 40.0, 34.5, 36.2).strip()
        port_of_delivery = by_xy(15.0, 40.0, 39.0, 40.7).strip()

        pkg_raw = by_xy(42.0, 55.0, 29.8, 31.5)
        pkg_m = _re.search(r"(\d+)", pkg_raw)
        total_package = int(pkg_m.group(1)) if pkg_m else 0

        gw_raw = by_xy(40.0, 55.0, 34.5, 36.2)
        try:
            gross_weight_kg = float(clean_num(gw_raw)) if gw_raw else 0.0
        except (ValueError, TypeError):
            gross_weight_kg = 0.0

        cbm_raw = by_xy(42.0, 55.0, 39.0, 40.7)
        try:
            cbm = float(clean_num(cbm_raw)) if cbm_raw else 0.0
        except (ValueError, TypeError):
            cbm = 0.0

        qty_raw = by_xy(73.0, 80.0, 22.5, 24.0)
        qty_m = _re.search(r"(\d+)", qty_raw)
        total_container_qty = int(qty_m.group(1)) if qty_m else 0
        ...
        y = 29.0
        while y < 45.0:
            ct_raw = by_xy(52.0, 64.0, y, y + 2.2)
            cm = CT_RE.search(ct_raw.replace(" ", ""))
            if cm:
                ct_no = cm.group(1)
                if ct_no in seen_ct:
                    y += 2.0
                ...
                # v8.7.1 [TYPE-FIX]: 샘플 실측 — container_type은 x=64%, 형식 "45GP"
                # 슬래시(/) 제외: 63.5~67.0% 범위로 좁혀 "/"/"45GP"/"/"에서 중간값만
                type_raw = by_xy(63.5, 67.0, y, y + 2.2)
                # 슬래시 및 공백 제거 후 영문+숫자 조합만 추출 (45GP, 20DR 등)
                _tm = _re.search(r'([0-9]{2}[A-Z]{1,3}[0-9]?)', type_raw.replace(" ", ""))
                ct_type = _tm.group(1) if _tm else type_raw.replace("/", "").replace(" ", "")

                dem_ft_row = by_xy(68.0, 77.0, y, y + 2.2)
                det_ft_row = by_xy(77.0, 85.0, y, y + 2.2)
                # v8.7.1 [YARD-FIX]: x=85.5%의 '/' 제외, 실 yard명(x=87%+)만
                return_yard_raw = by_xy(86.5, 99.0, y, y + 2.5)
                # 혹시 남은 선행 '/'/공백 정리
                return_yard = _re.sub(r'^[\s/]+', '', return_yard_raw).strip()

                dem_str = parse_yyyymmdd(dem_ft_row)
                det_str = parse_yyyymmdd(det_ft_row)
                rep_str = dem_str or det_str

                containers.append({
                    "container_no": ct_no,
                    "type": ct_type,
                    "free_time": rep_str,
                    "return_yard": return_yard.strip(),
                })
            y += 2.0
        ...
        if hasattr(result, "port_of_delivery"):
            result.port_of_delivery = port_of_delivery
        if hasattr(result, "mrn"):
            result.mrn = mrn
        if hasattr(result, "msn"):
            result.msn = msn
        ...
        for c in containers:
            ci = ContainerInfo()
            ci.container_no = c["container_no"]
            # v8.7.1 [TYPE-FIX]: v864_2의 ContainerInfo 모델은 size_type 필드 사용
            # (bundle의 container_type/type는 v864_2에 존재하지 않아 기존엔 "?" 표시)
            if c.get("type"):
                if hasattr(ci, "size_type"):
                    ci.size_type = c["type"]
                elif hasattr(ci, "container_type"):
                    ci.container_type = c["type"]
                elif hasattr(ci, "type"):
                    setattr(ci, "type", c["type"])
            ft = c.get("free_time", "")
            if ft:
                # v864_2의 FreeTimeInfo는 free_time_date 사용, ContainerInfo엔 해당 필드 없을 수 있음
                if hasattr(ci, "free_time"):
                    ci.free_time = ft
                if hasattr(ci, "free_time_date"):
                    ci.free_time_date = ft
            if c.get("return_yard") and hasattr(ci, "return_yard"):
                ci.return_yard = c["return_yard"]
            result.containers.append(ci)
            ...
            if c["container_no"] or ft:
                result.free_time_info.append(FreeTimeInfo(
                    container_no=c["container_no"],
                    free_time_date=ft,
                    return_location=c.get("return_yard", ""),
                    storage_free_days=0,
                ))
        ...
        logger.info(
            f"[DO-HAPAG] 좌표 파싱 v9.3: do={do_no} bl={bl_no} "
            f"vessel={vessel!r} voy={voyage} mrn={mrn}/msn={msn} "
            f"arrival={result.arrival_date} pol={pol} pod={pod} "
            f"delivery={port_of_delivery} wt={gross_weight_kg} cbm={cbm} "
            f"pkg={total_package} qty={total_container_qty} "
            f"containers={len(containers)}개"
        )
        return result
    ...
    def parse_do(self, pdf_path: Optional[str] = None, image_bytes: Optional[bytes] = None,
        ...
        if pdf_path and not _carrier:
            ...
            _HAPAG_PREFIX = ('HLCU', 'HAMU')  # v9.3: Hapag-Lloyd
            ...
            elif any(_stem.startswith(p) for p in ('ONEY', 'ONEU')):
                _carrier = 'ONE'
        ...
        if pdf_path and not image_bytes and _carrier:
            ...
            # [B2-ONE] ONE → 고정 좌표 파싱 (v9.2: 실측 좌표)
            if _carrier in ("ONE", "ONEY"):
                try:
            ...
            # [B3-HAPAG] HAPAG → 고정 좌표 파싱 (v9.3: 실측 좌표, 샘플 검증 완료)

...
class BLMixin:
    ...
    CARRIER_COORD_TABLE = {
        ...
        "ONE": {
            # v9.2: 실제 ONE BL PDF(ONEYSCLG01825300 BL.pdf) 좌표 직접 측정 결과
            # BL No: ONEYSCLG01825300 at x=80.4%, y=8.0% (라벨 "SEA WAYBILL NO." 는 y=7.3%)
            "bl_no": (78.0, 95.0, 7.5, 9.0),
            # Vessel: "RIO DE JANEIRO EXPRESS" at x=5-21%, y=35.2%
            # (라벨 "OCEAN VESSEL VOYAGE NO. FLAG" 는 y=34.4%)
            "vessel": (5.0, 20.0, 34.8, 36.0),
            # Voyage: "2601W" at x=21.2%, y=35.2%
            "voyage_no": (20.0, 30.0, 34.8, 36.0),
            # Port of Loading: "PUERTO ANGAMOS, CHILE" at x=31.2-52%, y=35.5%
            "port_of_loading": (31.0, 55.0, 34.8, 36.0),
            # Port of Discharge: "KWANGYANG" at x=5.0%, y=38.3%
            # (라벨 "PORT OF DISCHARGE" 는 y=37.3% / PLACE OF DELIVERY 는 x=31.2%)
            "port_of_discharge": (3.0, 20.0, 37.8, 39.0),
            # Date Laden on Board / Shipped Date: "19 FEB 2026" at x=82.1-93.1%, y=80.1%
            # (라벨 "DATE LADEN ON BOARD" y=74.2% / "DATED" y=79.3%)
            "ship_date": (80.0, 95.0, 79.5, 81.0),
        },
        "HAPAG": {
            # v9.3: 실제 Hapag BL PDF(2200034659 BL.pdf) 좌표 직접 측정 결과
            # 페이지 크기 612 x 792 (ONE/MSC/MAERSK의 595x842와 다름)
            # SWB-No: HLCUSCL260148627 at x=62.3%, y=11.1% (페이지번호 "2/3"은 x=84.7%부터 → 제외)
            "bl_no": (58.0, 82.0, 10.5, 12.0),
            # Vessel: BUENAVENTURA EXPRESS at x=7.0-28%, y=36.8%
            "vessel": (5.0, 40.0, 36.0, 37.8),
            # Voyage-No: 2602W at x=42.3%, y=36.8% ("Place of Delivery" 라벨은 x=49.3% → 제외)
            "voyage_no": (40.0, 47.0, 36.0, 37.8),
            # Port of Loading: PUERTO ANGAMOS, CHILE at x=7.0-28%, y=41.3%
            "port_of_loading": (5.0, 40.0, 40.5, 42.3),
            # Port of Discharge: BUSAN, SOUTH KOREA at x=7.0-25%, y=44.3% (DO의 KRPUS)
            "port_of_discharge": (5.0, 40.0, 43.5, 45.3),
            # Place and date of issue: VINA DEL MAR 26.FEB.2026 at x=49-82%, y=88.7%
            "ship_date": (45.0, 85.0, 88.0, 89.5),
        },
    }
    ...
    def _detect_carrier_from_words(self, words: List[Dict], full_text: str = "", explicit: str = "") -> str:
        """v8.7.0 [POLICY]: 점수 기반 자동 감지 제거 — 사용자 입력(explicit)만 신뢰.

        사용자가 UI에서 선사를 선택하면 `explicit`으로 전달되고, 그 값만 사용한다.
        explicit이 비어 있으면 "" 반환 → 상위 로직이 좌표 파싱 스킵하고 Gemini로 폴백.

        제거 사유:
          - Track B 감사 R1: 키워드 점수 동점 시 MSC로 편향되는 silent misparse 리스크
          - 사용자 정책: "철저히 사용자 입력하는 베이스로 갈거야"
        """
        if explicit:
            ...
            return chosen
        # v8.7.0: explicit 없으면 빈 문자열 반환 — 자동 감지 안 함
        logger.info("[BL] carrier: 사용자 미지정 — 자동 감지 안 함 (Gemini 폴백)")
        return ""

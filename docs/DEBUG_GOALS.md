...
- [ ] **🔴 A1** `main_webview.py:~775` 스플래시가 `/api/health`에서 `res.ok` 검증 없이 `res.json()` → 백엔드 500이어도 lots/bags/mt=0으로 진행. **재현 확인 후** `if(!res.ok) throw` 추가.
- [ ] **🔴 A2** `main_webview.py:~1109` `wait_for_api` 타임아웃 후 에러 HTML이 `on_loaded` 미발화로 안 그려져 **흰 화면 고착(추정)**. → 에러 상태에서 화면 표시 강제.
- [ ] **🔴 A3** `backend/api/__init__.py:~137` DB 마이그레이션 실패를 `logging.warning`만 하고 시작 계속 → 테이블 미생성 → 이후 API "no such table" → UI 빈 상태. → 핵심 마이그레이션 실패 시 명시적 차단/재시도.
- [ ] **🟡 A4** `frontend/js/api-client.js:~45` 빈/204 응답을 `{}`로 반환 → 상위가 "정상"으로 오인. → status·success 명시 검증.
- [ ] **🟡 A5** `frontend/js/pages/inventory.js:~19` 응답을 배열로 가정하나 `{total,data}` 객체일 수 있음 → `filter()` 에러로 화면 멈춤. → `extractRows` 사용.
...
- [ ] **🔴 C1** `backend/api/outbound_api.py:~801` PICKED→SOLD 전환 후 `stock_movement` INSERT 누락 → 이력 0행(감사추적 끊김). → INSERT 추가 + 재현 테스트.
...
- [ ] **🔴 D1** `engine_modules/inventory_modular/return_mixin.py:~332` 반품이 `RETURN` 상태에서 멈추고 `finalize_return_to_available` 미호출 → 가용재고로 안 돌아옴(2단계 수동). → 자동 진행 또는 API 공개.
- [ ] **🔴 D2** `backend/api/status_revert_api.py:~291` 상태복원이 inventory 상태만 바꾸고 `current_weight/picked_weight` 미복구 → SOLD→PICKED 복원 시 `initial=current+picked` 깨짐. → 복원 후 재계산.
...
### 다음 액션 (사용자 선택)
1. **검증 우선:** 🔴 16건부터 "재현 스크립트로 진짜 끊기는지" 확인 → 진짜만 골로 승격 후 수정.
2. **영역 단위:** A(시작/연결)부터 한 영역씩 재현→수정→테스트→커밋.
3. 각 골 수정 착수 전 위 *템플릿*으로 상세 블록(완료기준 포함)을 펼쳐 쓴다.
> ⚠️ 줄번호·"추정" 항목은 코드가 바뀌었을 수 있으니 **고치기 직전 grep로 위치 재확인**.

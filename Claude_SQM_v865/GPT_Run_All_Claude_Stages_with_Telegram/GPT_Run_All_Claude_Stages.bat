...
REM 사용 예:
REM   GPT_Run_All_Claude_Stages.bat
REM   GPT_Run_All_Claude_Stages.bat FULL
REM   GPT_Run_All_Claude_Stages.bat P0
REM   GPT_Run_All_Claude_Stages.bat P1
REM   GPT_Run_All_Claude_Stages.bat P2
REM
REM 사전 준비:
REM   1) 이 파일을 프로젝트 루트에 둡니다.
REM   2) GPT_Run_Claude_Stage.bat / GPT_Run_Claude_Stage.ps1 도 프로젝트 루트에 둡니다.
REM   3) auto_tasks\B00_prepare.md ... B13_final_validation.md 준비
REM   4) 아래 TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID 값을 실제 값으로 수정
REM =============================================================
...
if not exist "%PROMPT_FILE%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File "%TELEGRAM_PS1%" -BotToken "%TELEGRAM_BOT_TOKEN%" -ChatId "%TELEGRAM_CHAT_ID%" -Message "[SQM] %STAGE% 실패 - 프롬프트 파일 없음: %PROMPT_FILE%"
    exit /b 1
)

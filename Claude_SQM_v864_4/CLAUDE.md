# SQM Inventory v864.3 — PyWebView Migration Project
...
## 🧠 프로젝트 전용 메모리 규칙 (v864.3 한정) — 최우선

> **등록일:** 2026-04-21 12:13 KST
> **수정일:** 2026-04-21 12:15 KST (하드코딩 절대경로 제거, 상대경로 일반화)
> **등록자:** Nam Ki-dong (사장님)
> **승인자:** Ruby (Senior Software Architect)

> ⚠️ **계층 안내:** 범용(모든 프로그램 공통) 규칙은 **상위 `F:\program\CLAUDE.md`** 에 있음. 이 파일은 **v864.3 프로젝트 전용** 규칙만 담음.
> Claude Code는 작업 폴더에서 부모 방향으로 올라가며 모든 CLAUDE.md를 자동으로 합쳐 읽으므로, 두 파일은 자동 상속됨.

**v864.3 프로젝트 작업 시 반드시 "이 프로젝트 폴더(현재 폴더 = `Claude_SQM_v864_3\`)"의 내용을 전부 참조·반영한다. 절대경로 하드코딩 금지.**

구체적으로:

1. **작업 시작 전 필수 확인 (이 프로젝트 한정):**
   - `./CLAUDE.md` (이 파일 — v864.3 규칙/아키텍처)
...
5. **다른 프로젝트와의 격리:**
   ...
   - v864.3 핸드오프 JSON 3종은 **이 프로젝트 안에서만** 의미 있음
...
Tkinter + ttkbootstrap로 만들어진 **SQM Inventory v864.2** (광양 GY Logis 물류창고 재고관리 시스템)를 **PyWebView + HTML/CSS/JS + FastAPI** 기반 **v864.3**으로 마이그레이션.

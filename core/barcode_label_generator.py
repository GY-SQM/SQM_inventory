...
def generate_barcode_labels(tonbags: list, output_dir: str = 'output/labels',
    ...
    for idx, tb in enumerate(tonbags):
        uid = tb.get('tonbag_uid') or f"{tb.get('lot_no','?')}-{tb.get('sub_lt',0):02d}"
        lot_no = tb.get('lot_no', '')
        sub_lt = tb.get('sub_lt', 0)
        ...
        y -= 6*mm
        c.drawString(margin, y, f"Tonbag: #{sub_lt}  |  {weight:.0f} kg")
        y -= 8*mm
        ...
        if qrcode:
            try:
                ...
                qr.add_data(f"LOT:{lot_no}|TB:{sub_lt}|UID:{uid}|W:{weight}")
                qr.make(fit=True)

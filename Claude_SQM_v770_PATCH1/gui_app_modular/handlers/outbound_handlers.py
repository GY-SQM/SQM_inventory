    ...
    def _on_picking_list_upload(self) -> None:
        ...
        try:
            ...
            if not hasattr(picking_result, '_tpl_customer'):
                ...
                picking_result._tpl_bag_weight  = int(_tpl.get('bag_weight_kg', 500))
        ...
        try:
            ...
            # v7.7.0-P1: 폴백 파서는 반환 타입이 다를 수 있음 (dict vs PickingDoc)
            try:
                from parsers import parse_picking_list_pdf as _parse
                parse_picking_list_pdf = _parse
                logger.warning(
                    "[Picking] features.parsers 임포트 실패 → parsers 폴백 사용. "
                    "반환 타입이 다를 수 있습니다 (dict vs PickingDoc)."
                )
            except ImportError:
                try:
                    ...
                    parse_picking_list_pdf = _parse
                    logger.warning(
                        "[Picking] features.parsers 임포트 실패 → "
                        "parsers.picking_list_parser 폴백 사용. "
                        "반환 타입이 다를 수 있습니다 (dict vs PickingDoc)."
                    )
                except ImportError:
                    pass

...
class InboundDocDetector:
    ...
    TEXT_KEYWORD_MAP: dict[str, list[str]] = {
        ...
        "INVOICE": [
            "invoice", "factura",          # factura → SQM FA 전체 커버
            "commercial invoice", "export invoice",
            "송장", "화인보이스", "unit price", "total value",
            "factura de exportacion",
        ],
        "BL": [
            "bill of lading", "sea waybill", "seawaybill",
            "선하증권", "maersk", "msc", "hmm", "cma cgm",
            "vessel", "port of loading", "port of discharge",
            "shipper", "consignee", "notify party",
        ],
        "DO": [
            "delivery order", "d/o", "인도지시서", "free time",
            "container release", "container pickup",
        ],
    }
    ...
    def detect_from_folder(self, folder: str, file_names: list[str]) -> dict:
        ...
        for name in file_names:
            ...
            # v6.4.3: BL 번호 자체가 파일명 (263764814.pdf / MEDUFP963996.pdf)
            stem = os.path.splitext(name)[0]
            is_bl_number = bool(
                re.fullmatch(r'\d{9,12}', stem)               # 9~12자리 숫자
                or re.fullmatch(r'[A-Z]{4}[A-Z0-9]{5,}', stem)  # MEDU/MAEU/HLCU 등
            )

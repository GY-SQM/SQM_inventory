...
class DashboardDataMixin:
        ...
        try:
            ...
            try:
                ...
                for _fr in (_ft_rows or []):
                    ...
                    alerts.append({
                        'icon': '⏰',
                        'message': f"{_lot}: 컨테이너 반납 D-3 이내 ({_con})",
                        'severity': 'error',
                        'lot_no': _lot
                    })
    ...
    def _get_integrity_summary(self) -> dict:
        """입고 총량 = 현재재고 + 출고누계 정합성 집계."""
        try:
            db = self.engine.db
            # 입고 누계 (inventory_tonbag 전체)
            r_total = db.fetchone(
            ...
            # 현재 재고 (AVAILABLE+RESERVED+PICKED+RETURN)
            r_cur = db.fetchone(
                "SELECT COUNT(*) AS cnt, COALESCE(SUM(weight),0) AS kg "
                "FROM inventory_tonbag "

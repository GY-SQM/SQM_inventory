...
class SoldTabMixin:
    ...
    def _setup_sold_tab(self) -> None:
        ...
        _is_dark_footer = ThemeColors.is_dark_theme(getattr(self, 'current_theme', 'flatly'))
        self._sold_footer = TreeviewTotalFooter(
            ...
            is_dark=_is_dark_footer,
        )
        self._sold_footer.pack(fill=X, pady=(Spacing.XS, 0))
        _btn_sold_excel = ttk.Button(self._sold_footer.frame, text="📥 Excel 내보내기", command=self._on_sold_export_excel)
        _btn_sold_excel.pack(side=tk.RIGHT, padx=Spacing.XS)
        apply_tooltip(_btn_sold_excel, "SOLD 데이터를 Excel로 내보내기 (정산/보고용)")
    ...
    def _refresh_sold(self) -> None:
        ...
        try:
            ...
            if hasattr(self, '_sold_footer') and self._sold_footer:
    ...
    def _on_finalize_return_selected(self) -> None:
        ...
        for iid in sel:
            ...
            lot_no = vals[0] if vals else None
            if lot_no:
                try:

...
# 색상 상수 (다크 테마 기준)
# ─────────────────────────────────────────
C_BG        = '#0B1220'   # bg_primary
C_BG2       = '#162033'   # bg_secondary (card)
C_FG        = '#E5E7EB'   # text_primary
C_FG2       = '#9CA3AF'   # text_secondary
C_ACCENT    = '#22C5D6'   # border_focus (teal)
C_SUCCESS   = '#34D399'   # success
C_WARNING   = '#FBBF24'   # warning
C_DANGER    = '#F87171'   # danger
C_STRIPE    = '#111B2E'   # tree_stripe
C_SELECT    = '#2563EB'   # tree_select_bg
C_SAMPLE    = '#D97706'   # amber — 샘플 행 강조
C_BTN_DL    = '#2563EB'   # 다운로드 버튼
C_BTN_CLOSE = '#475569'   # 닫기 버튼
...
# v7.8.0: C양식 (SQM 기존 양식) — 템플릿 1
_C_COLUMNS = [
    'LOT NO', 'QTY (MT)', 'SOLD TO', 'OUTBOUND DATE', 'SAP NO',
    'PRODUCT', 'SALE REF', 'WH', 'BL NO', 'CONTAINER',
    'SUB LT', 'ETA', 'DATE IN STOCK', 'CUSTOMS', 'GW', 'EXPORT',
]

_C_ROWS = [
    ('1126010932', '5.001', 'PT LBM',  '2026-03-25', '2200034273', 'LITHIUM CARBONATE', 'SQM-2026-001', 'GY', 'MAEU263764814', 'MRSU6898402', '', '2026-03-10', '2026-03-12', 'CLEARED', '5131.25', ''),
    ('1126010933', '5.001', 'PT LBM',  '2026-03-25', '2200034273', 'LITHIUM CARBONATE', 'SQM-2026-001', 'GY', 'MAEU263764814', 'MRSU6898402', '', '2026-03-10', '2026-03-12', 'CLEARED', '5131.25', ''),
    ('1126010934', '5.001', 'CATL',    '2026-03-28', '2200034274', 'LITHIUM CARBONATE', 'SQM-2026-002', 'GY', 'MEDUFP963970',  'MSDU752148',  '', '2026-03-11', '2026-03-13', '',        '5131.25', ''),
    ('1126010935', '5.001', 'CATL',    '2026-03-28', '2200034274', 'LITHIUM CARBONATE', 'SQM-2026-002', 'GY', 'MEDUFP963970',  'MSDU752148',  '', '2026-03-11', '2026-03-13', '',        '5131.25', ''),
    ('1126010936', '5.001', 'POSCO',   '2026-04-01', '2200034275', 'LITHIUM CARBONATE', 'SQM-2026-003', 'GY', 'COSU8234567',   'CSNU456789',  '', '2026-03-15', '2026-03-17', 'CLEARED', '5131.25', ''),
    ('1126010937', '5.001', 'POSCO',   '2026-04-01', '2200034275', 'LITHIUM CARBONATE', 'SQM-2026-003', 'GY', 'COSU8234567',   'CSNU456789',  '', '2026-03-15', '2026-03-17', 'CLEARED', '5131.25', ''),
]
...
class AllocationTemplateDialog:
    ...
    # 양식 정의 (파일명, 내장 데이터) — v7.8.0: C양식(양식1) 우선
    FORMATS = [
        {
            'tab_label': '① C양식 (SQM 기존)',
            'template_file': 'allocation_template_c.xlsx',
            'description': (
                '양식 1 — C양식 │ 1행: 타이틀, 2행: 빈 행, 3행: 헤더, 4행~: 데이터  │  '
                'LOT NO / QTY (MT) / SOLD TO / OUTBOUND DATE / SAP NO / PRODUCT / '
                'SALE REF / WH / BL NO / CONTAINER / SUB LT / ETA / DATE IN STOCK / CUSTOMS / GW / EXPORT  │  '
                '1 LOT = 1행 (톤백 단위 아님)'
            ),
            'columns': _C_COLUMNS,
            'rows': _C_ROWS,
        },
        {
            ...
            'description': (
                '양식 2 — Song 양식 │ 컬럼: Product / SAP NO / Date in stock / QTY (MT) / '
                'Lot No / WH / Customs / SOLD TO / SALE REF / GW  │  '
        ...
        },
        {
            ...
            'description': (
                '양식 3 — Woo 양식 │ 컬럼: Product / SAP NO / Date in stock / QTY (MT) / '
    ...
    def _build_ui(self):
        ...
        tk.Label(legend_frame, text=' 본 제품 행',
                 fg=C_FG2, bg=C_BG, font=('Malgun Gothic', 9)).pack(side='left')
        ...
        self._tree.tag_configure('sample', background='#3e2a00', foreground=C_SAMPLE)
        self._tree.tag_configure('sample_stripe', background='#2e1e00', foreground=C_SAMPLE)
    ...
    def _populate_tree(self, columns: List[str], rows: List[tuple]):
        ...
        # 컬럼 헤더 설정
        col_widths = {
            'Product': 130, 'PRODUCT': 130, 'SAP NO': 100, 'SAP_NO': 100,
            'Date in stock': 100, 'DATE IN STOCK': 100,
            'QTY (MT)': 70, 'Lot No': 110, 'LOT NO': 110,
            'WH': 40, 'Customs': 80, 'CUSTOMS': 80,
            'Export': 60, 'EXPORT': 60, 'SOLD TO': 150,
            'SALE REF': 90, 'Balance': 70, 'GW': 65, 'Remark': 80,
            'OUTBOUND DATE': 100, 'BL NO': 120, 'CONTAINER': 110,
            'SUB LT': 60, 'ETA': 90,
        }
        for col in columns:

...
class HeaderSortFilterRow:
    ...
    def __init__(self, parent, tree, columns: List[Tuple[str, str, int]],
        ...
        # v7.8.0: ThemeColors 기반 라벨 색상
        try:
            from .ui_constants import ThemeColors
            _cell_bg = ThemeColors.get('bg_secondary', is_dark=is_dark)
            _label_fg = ThemeColors.get('text_primary', is_dark=is_dark)
        except Exception:
            _cell_bg = '#111B2E' if is_dark else '#F1F5F9'
            _label_fg = '#E5E7EB' if is_dark else '#0F172A'
        ...
        for col_id, label, width in columns:
            ...
            # 컬럼명 + 클릭 시 정렬 (tk.Label: 배경/글씨색 직접 지정)
            lbl = tk.Label(cell, text=label, font=('맑은 고딕', 9, 'bold'),
                           bg=_cell_bg, fg=_label_fg, bd=0)
            ...
            sort_lbl = tk.Label(cell, text="", font=('맑은 고딕', 9), width=2,
                                bg=_cell_bg, fg=_label_fg, bd=0)
        ...
        if show_opt_row and date_from_var is not None and date_to_var is not None:
            ttk.Label(self._opt_frame, text="기간:").pack(side=tk.LEFT, padx=(8, 6))
            ttk.Entry(self._opt_frame, textvariable=date_from_var, width=11).pack(side=tk.LEFT, padx=(0, 4))
            _btn_cal_from = ttk.Button(self._opt_frame, text="📅", width=2,
                command=lambda: show_date_calendar(
                    self.frame.winfo_toplevel(), date_from_var.get(),
                    lambda ymd: (date_from_var.set(ymd), self.on_filter())))
            _btn_cal_from.pack(side=tk.LEFT, padx=(0, 8))
            ttk.Label(self._opt_frame, text=" ~ ").pack(side=tk.LEFT, padx=(0, 8))
            ttk.Entry(self._opt_frame, textvariable=date_to_var, width=11).pack(side=tk.LEFT, padx=(0, 4))
            _btn_cal_to = ttk.Button(self._opt_frame, text="📅", width=2,
                command=lambda: show_date_calendar(
                    self.frame.winfo_toplevel(), date_to_var.get(),
                    lambda ymd: (date_to_var.set(ymd), self.on_filter())))
            _btn_cal_to.pack(side=tk.LEFT, padx=(0, 16))
        ...
        if show_opt_row:
            ttk.Button(self._opt_frame, text="✖ 초기화", width=8, command=self._reset_filters).pack(side=tk.LEFT, padx=(0, 8))
...
class TreeviewTotalFooter:
    ...
    def __init__(self, parent, tree, summable_column_ids: List[str],
        ...
        Args:
            ...
            is_dark: 다크 테마 여부 (tk.Label 배경/전경 설정용)
        """
        import tkinter as tk
        from tkinter import ttk
        ...
        _bg = ThemeColors.get('bg_primary', is_dark)

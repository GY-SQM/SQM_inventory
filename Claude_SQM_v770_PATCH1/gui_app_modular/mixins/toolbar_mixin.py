...
class ToolbarMixin:
    ...
    def _load_toolbar_colors(self) -> None:
        ...
        try:
            ...
            self._tb_fg_normal = ThemeColors.get('statusbar_fg', _dark)
            self._tb_fg_active = ThemeColors.get('text_primary', _dark)
            self._tb_fg_hover = ThemeColors.get('info', _dark)
    ...
    def _setup_toolbar(self) -> None:
        ...
        # v7.7.2: Row2 오른쪽에 새로고침/Dark/Light/버전 배지 배치
        self._right_actions = tk.Frame(self._row2, bg=self._tb_bg)
        self._right_actions.pack(side='right', padx=Spacing.MD)
        self._build_refresh_button(self._right_actions)
        self._build_quick_theme_buttons(self._right_actions)
        try:
    ...
    def _build_all_menus(self) -> None:
        ...
        for item in menus:
            ...
            # v7.7.2: 상단 메뉴는 작게 (하위 탭이 더 크게)
            _btn_font = self._tb_font_scale.small()
    ...
    def _build_settings_menu(self) -> 'tk.Menu':
        ...
        theme.add_command(label="  🌙 Dark Mode (darkly)", command=lambda: self._change_theme('darkly'))
        theme.add_command(label="  ☀️ Light Mode (flatly)", command=lambda: self._change_theme('flatly'))
    ...
    def _build_tab_buttons(self) -> None:
        ...
        self._tab_buttons = {}
        # v7.7.2: 탭은 상단 메뉴보다 크게 (Phase3: FontScale.heading + Spacing)
        _tab_font = self._tb_font_scale.heading()

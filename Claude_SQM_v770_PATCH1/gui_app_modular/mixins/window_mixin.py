...
class WindowMixin:
    ...
    def _load_window_config(self) -> None:
        ...
        try:
            from ..utils.ui_constants import load_all_geometry
            ...
            if config and 'width' in config:
                ...
                if x is not None and y is not None:
                    ...
                    self.root.geometry(f"{width}x{height}")
                logger.info(f"Window config loaded: {width}x{height}+{x}+{y}")
                return
    ...
    def _save_window_config(self) -> None:
        ...
        try:
            ...
            if match:
                ...
                logger.info(f"Window config saved: {config.get('width')}x{config.get('height')}+{config.get('x')}+{config.get('y')}")
            else:
                logger.warning(f"Window geometry parse failed: {geometry}")

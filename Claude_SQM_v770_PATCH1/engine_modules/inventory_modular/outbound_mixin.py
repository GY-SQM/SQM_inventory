...
class OutboundMixin(InventoryBaseMixin):
    ...
    def fix_lot_status_integrity(self) -> Dict:
        ...
        try:
            # v7.7.0-P6: 다중 LOT 상태 수정을 트랜잭션으로 보호
            with self.db.transaction("IMMEDIATE"):
                result = self._fix_lot_status_integrity_impl(result)
            result['success'] = True
            result['message'] = f"LOT 상태 정합성 복구 완료: {result['fixed']}건"
        except (sqlite3.OperationalError, sqlite3.IntegrityError, OSError) as e:
            result['errors'].append(str(e))
            logger.error(f"[fix_integrity] DB 오류: {e}")
        except ValueError as e:
            result['errors'].append(str(e))
            logger.error(f"[fix_integrity] 검증 오류: {e}")
        return result

    def _fix_lot_status_integrity_impl(self, result: Dict) -> Dict:
        """v7.7.0-P6: fix_lot_status_integrity 내부 구현 (트랜잭션 안에서 호출)."""
        try:
            ...
            result['errors'].append(str(e))
            logger.error(f"[fix_integrity_impl] DB 오류: {e}")
        return result
    ...
    def process_outbound(self, allocation_data, source: str = 'AUTO', stop_at_picked: bool = False) -> Dict:
        ...
        try:
            ...
            # v7.7.0-P9: 건수 검증 (요청 vs 처리)
            if result['processed'] != len(allocations):
                _cnt_msg = (
                    f"[ROW_COUNT_MISMATCH] 출고 요청 {len(allocations)}건 중 "
                    f"{result['processed']}건만 처리됨"
                )
                result['warnings'].append(_cnt_msg)
                logger.warning(_cnt_msg)
    ...
    def cancel_outbound_tonbag(self, lot_no: str, sub_lt: int) -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                if was_sold:
                    # v7.7.0-P6: broad exception → 구체적 예외 + 로깅
                    try:
                        self.db.execute(
                    ...
                    except (sqlite3.OperationalError, sqlite3.IntegrityError) as _e:
                        logger.debug(f"[cancel_outbound] sold_table 업데이트 스킵: {_e}")
                    try:
                        self.db.execute(
                        ...
                        logger.debug(f"[cancel_outbound] picking_table 업데이트 스킵: {_e}")
                    try:
                        self.db.execute(
                        ...
                        logger.debug(f"[cancel_outbound] allocation_plan 업데이트 스킵: {_e}")
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                for _ba in (allocation_rows or []):
                    # v7.8.0: 샘플 행은 배치 합산에서 제외 (공급 쿼리도 샘플 제외이므로 대칭)
                    if _alloc_val(_ba, 'is_sample'):
                        continue
                ...
                # v7.8.0: 이번 배치에서 생성한 레코드는 AL-06/LOT_MODE_DUP 판정에서 제외
                _pre_batch_max_id = 0
                try:
                    _mx = self.db.fetchone("SELECT COALESCE(MAX(id),0) AS mx FROM allocation_plan")
                    _pre_batch_max_id = int((_mx.get('mx') if isinstance(_mx, dict) else _mx[0]) or 0) if _mx else 0
                except Exception:
                    pass
                        ...
                        _conflict_row = self.db.fetchone(
                            f"""SELECT id FROM allocation_plan
                               WHERE lot_no = ? AND customer = ? AND sale_ref = ?
                                 ...
                                 AND id <= ?
                               LIMIT 1""",
                            (lot_no, customer, sale_ref, outbound_date, outbound_date, _pre_batch_max_id)
                        )
                        if _conflict_row:
                            ...
                            _al06_dup = self.db.fetchone(
                                """SELECT id, lot_no, customer FROM allocation_plan
                                   WHERE sale_ref = ?
                                     ...
                                     AND id <= ?
                                   LIMIT 1""",
                                (sale_ref, lot_no, _pre_batch_max_id)
                            )
                            if _al06_dup:
...
# [v6.8.6 top-level import로 이동]                     from engine_modules.constants import STATUS_PICKED, STATUS_RESERVED, STATUS_SOLD, get_tonbag_unit_weight
    ...
    def confirm_outbound(self, lot_no: str = None, force_all: bool = False) -> Dict:
        ...
        try:
            ...
            with self.db.transaction("IMMEDIATE"):
                touched_lots = set()
                # v7.7.0: tonbag STATUS UPDATE → executemany 일괄 처리
                _sold_upd_rows = [
                    (STATUS_SOLD, now, now, tb['id']) for tb in tonbags
                ]
                self.db.executemany(

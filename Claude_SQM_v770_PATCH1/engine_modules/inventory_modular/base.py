...
class InventoryBaseMixin:
    ...
    def _validate_lot_no(self, lot_no: str) -> bool:
        """LOT 번호 유효성 검증. v7.0.0 [REFACTOR-2]: validators 단일 소스 위임"""
        from engine_modules.validators import validate_lot_no as _vln
        ok, _ = _vln(str(lot_no or ''))
        return ok

    def _require_lot_no(self, lot_no, caller: str = "") -> str:
        """v7.7.0-P4: lot_no 필수 검증 — None/빈값 시 ValueError raise.

        _validate_lot_no()와 달리 bool이 아닌 raise 방식이라 호출부에서 무시 불가.
        반환값은 strip된 lot_no 문자열.
        """
        if lot_no is None:
            raise ValueError(f"[{caller}] lot_no가 None입니다.")
        lot_no = str(lot_no).strip()
        if not lot_no:
            raise ValueError(f"[{caller}] lot_no가 빈 문자열입니다.")
        return lot_no

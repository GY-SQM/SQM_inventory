    ...
    def process_inbound(self, packing_data, invoice_data=None, 
        ...
        try:
            ...
            with self.db.transaction():
                ...
                # v7.7.0-P9: 건수 검증 (parsed vs saved)
                expected_min = len(tonbags)      # 샘플 이미 존재 시
                expected_max = len(tonbags) + 1  # 정상: 본품 + 샘플 자동 생성
                if tonbag_count < expected_min or tonbag_count > expected_max:
                    _cnt_msg = (
                        f"[ROW_COUNT_MISMATCH] LOT {lot_no}: "
                        f"파싱 {len(tonbags)}건, 저장 {tonbag_count}건 — 확인 필요"
                    )

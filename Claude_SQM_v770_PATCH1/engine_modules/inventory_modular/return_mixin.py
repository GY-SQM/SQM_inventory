...
from engine_modules.constants import (
    ...
    # STATUS_OUTBOUND: SQL 문자열 'OUTBOUND' 직접 사용 (하위호환)
)  # v7.5.0: 하드코딩 상수 중앙화
import json
import logging
import sqlite3
...
class ReturnMixin:
    ...
    def bulk_return_by_lot(self, lot_no: str, reason: str = None) -> Dict:
        ...
        """
        # v7.7.0-P4: lot_no 필수 검증
        lot_no = self._require_lot_no(lot_no, "bulk_return_by_lot")
    ...
    def finalize_return_to_available(
        ...
        try:
            # v7.7.0-P4: lot_no 필수 검증
            lot_no = self._require_lot_no(lot_no, "finalize_return_to_available")
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            # v7.7.0-P5: 상태 전이 검증 (RETURN → AVAILABLE)
            from engine_modules.validators import validate_status_transition
            current_status = tb.get('status', '')
            try:
                validate_status_transition(
                    current_status, 'AVAILABLE',
                    context="finalize_return_to_available"
                )
            ...
            # v7.7.0-P2: All-or-Nothing 트랜잭션 래핑
            # 3개 DB write가 원자적으로 실행되어야 정합성 유지
            with self.db.transaction("IMMEDIATE"):
                # RETURN → AVAILABLE 전환
                self.db.execute(
                    "UPDATE inventory_tonbag "
                ...
                self.db.execute(
                    "UPDATE inventory "
                    "SET current_weight = current_weight + ?, updated_at = ? "
                    "WHERE lot_no = ?",
                    (float(tb.get('weight') or 0), now, lot_no)
                )
                ...
                self.db.execute(
                    ...
                    (lot_no, float(tb.get('weight') or 0),

        ...
        try:
            ...
            depleted_with_stock = self.db.fetchall("""
                SELECT inv.lot_no, inv.current_weight, inv.net_weight,
                       ...
                       (SELECT COUNT(*) FROM inventory_tonbag tb
                        WHERE tb.lot_no=inv.lot_no AND tb.status='SOLD'
...
# ============================================================================
# v7.7.0-P5: 톤백 상태 전이 검증
# ============================================================================

# 톤백 단위 허용 상태 전이 맵
_ALLOWED_TONBAG_TRANSITIONS = {
    'AVAILABLE': {'RESERVED', 'PICKED', 'RETURN'},
    'RESERVED':  {'PICKED', 'AVAILABLE', 'CANCELLED'},
    'PICKED':    {'SOLD', 'OUTBOUND', 'AVAILABLE', 'SHIPPED'},
    'SOLD':      {'AVAILABLE', 'RETURN'},           # 반품
    'OUTBOUND':  {'AVAILABLE', 'RETURN'},           # 반품
    'SHIPPED':   {'AVAILABLE', 'RETURN'},            # 반품
    'RETURN':    {'AVAILABLE'},                      # 반품 확정
    'DEPLETED':  set(),                              # 종료 상태
}
...
def validate_status_transition(old_status: str, new_status: str,
                               context: str = "") -> None:
    """톤백 상태 전이 검증 — 허용되지 않은 전이 시 ValueError.

    Args:
        old_status: 현재 상태
        new_status: 변경할 상태
        context:    호출 컨텍스트 (에러 메시지용)

    Raises:
        ValueError: 허용되지 않은 전이
    """
    allowed = _ALLOWED_TONBAG_TRANSITIONS.get(old_status, set())
    if new_status not in allowed:
        raise ValueError(
            f"[{context}] 허용되지 않은 상태 전이: "

...
from engine_modules.constants import SAMPLE_MT_THRESHOLD, STATUS_SOLD
import logging
...
class AllocationParser:
    ...
    def _select_best_sheet(self, excel_path: str) -> Optional[str]:
        ...
        for sh in sheet_names:
            try:
                df_sh = pd.read_excel(excel_path, sheet_name=sh, header=None, nrows=15, dtype=str)
            except Exception:
                continue
    ...
    def _extract_rows(self, df: pd.DataFrame, header: AllocationHeader) -> List[AllocationRow]:
        ...
        # v7.8.0: 필수 컬럼 검증 — lot_no 매핑 실패 시 조기 종료
        if 'lot_no' not in col_map:
            _detected = [h for h in headers if h][:8]
            self.warnings.append(
                f"⚠️ LOT NO 컬럼을 찾을 수 없습니다. "
                f"이 파일은 Allocation 양식이 아닐 수 있습니다.\n"
                f"  감지된 헤더(행{header_row_idx + 1}): {_detected}"
            )
            logger.warning(
                "[allocation_parser] lot_no 컬럼 매핑 실패 — headers=%s",
                _detected,
            )
            return []
        ...
        for i in range(header_row_idx + 1, len(df)):
            ...
            # v2.6.0: 샘플 행 자동 감지 (qty_mt < SAMPLE_MT_THRESHOLD = 샘플)
            if row.qty_mt > 0 and row.qty_mt < SAMPLE_MT_THRESHOLD:
                row.is_sample = True
    ...
    def _map_columns(self, headers: List[str]) -> Dict[str, int]:
        ...
        alias_patterns = {
            ...
            'qty_mt': ['QTY_MT', 'QTY (MT)', 'QTY(MT)', 'QTY', 'QUANTITY', '수량',
                      'WEIGHT', 'NET_WEIGHT', 'NET WEIGHT'],  # v7.8.0: BALANCE 제거 (SQM-Inventory 오매핑 방지)

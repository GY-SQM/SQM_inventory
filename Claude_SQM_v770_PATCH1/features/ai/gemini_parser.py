...
# =============================================================================
# v7.8.0: 파싱 실패 디버그 리포트
# =============================================================================

def save_parse_debug_report(
    doc_type: str,
    pdf_filename: str,
    failure_reason: str,
    raw_response: str = "",
    ...
    """
    파싱 실패 시 디버그 리포트를 JSON으로 저장.

    Returns:
        저장된 파일 절대 경로. 실패 시 빈 문자열.
    """
    import json
    ...
    try:
        ...
        report = {
            ...
            "raw_response": str(raw_response or "")[:50000],
            "carrier_id": carrier_id or "",
            "hints_used": str(hints_used or "")[:5000],
            "partial_results": partial_results or {},
        }

        debug_dir = Path(__file__).resolve().parent.parent.parent / "logs" / "parse_debug"
        debug_dir.mkdir(parents=True, exist_ok=True)

        fname = f"parse_fail_{now.strftime('%Y%m%d_%H%M%S')}_{doc_type}.json"
        fpath = debug_dir / fname

        with open(fpath, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)

        logger.info(f"[ParseDebug] 디버그 리포트 저장: {fpath}")
        return str(fpath)
    except Exception as _e:
        logger.debug(f"[ParseDebug] 리포트 저장 실패: {_e}")
        return ""
...
class PackingListResult:
    ...
    bag_weight_kg: int = 500          # v7.2.0: 입고 템플릿에서 주입된 톤백 단가
    raw_response: str = ""
    error_message: str = ""
...
class GeminiDocumentParser:
    ...
    def parse_packing_list(self, pdf_path: str,
        ...
        result.bag_weight_kg = bag_weight_kg  # v7.2.0: 템플릿 단가 보존
        ...
        try:
            ...
            if not images:
                ...
                save_parse_debug_report("PL", Path(pdf_path).name, result.error_message)
                return result
...
위 구조대로 JSON만 출력하세요."""
            ...
            # ── v7.3.0: 선사별 Gemini 힌트 주입 ──────────────────────────────
            if gemini_hint and gemini_hint.strip():
                prompt += f"\n\n[선사별 추가 파싱 힌트]\n{gemini_hint.strip()}"
                logger.debug(f"[GeminiParser] PL 힌트 주입 ({len(gemini_hint)}자)")
            ...
            if not data:
                ...
                save_parse_debug_report(
                    "PL", Path(pdf_path).name, result.error_message,
                    raw_response=response_text,
                    hints_used=gemini_hint,
                )
                return result
...
JSON만 출력하세요."""
            ...
            # ★ v7.3.7 [GEMINI-PL-FIX2]: 1페이지 누락 감지 → 힌트 기반 보강 재시도
            # 변경 이유: 동일 프롬프트 재시도는 Gemini가 동일하게 누락 → 0개 추가로 끝남
            # 수정: 이미 파싱된 LOT 목록을 힌트로 제공 → "이 LOT들은 이미 있으니 나머지 추출"
            if len(images) == 1 and 0 < len(result.lots) < 30:
                _before_retry = len(result.lots)
                _parsed_nos = [lot.lot_no for lot in result.lots]
                logger.warning(
                    f"[GeminiParser] PL 1페이지인데 {_before_retry}개만 추출됨 — "
                    f"힌트 기반 보강 재시도 실행 (v7.3.7)"
                )
                try:
                    # ★ 핵심: 이미 파싱된 LOT를 힌트로 줘서 누락된 행만 추출 유도
                    _hint_prompt = (
                        f"오직 JSON만 출력하세요. 설명·마크다운·코드블럭(```) 표시 금지.\n\n"
                        f"이 Packing List 문서에서 아래 LOT 번호들은 이미 추출됐습니다:\n"
                        f"{', '.join(_parsed_nos)}\n\n"
                        f"위 목록에 없는 LOT 행이 문서에 더 있으면 해당 행만 추출하세요.\n"
                        f"없으면 lots를 빈 배열로 반환하세요.\n\n"
                        f"JSON 형식: {{\"lots\": [{{\"lot_no\": \"...\", "
                        f"\"mxbg\": 0, \"net_weight_kg\": 0.0, "
                        f"\"gross_weight_kg\": 0.0}}]}}"
                    )
                    _retry_text = self._call_gemini(_hint_prompt, images[0])
                    ...
                    logger.warning(f"[GeminiParser] PL 힌트 재시도 실패: {_retry_err}")

            # v7.7.2: pdfplumber 정규식 기반 LOT 보강 (Gemini 누락 LOT 복구)
            # Invoice와 동일한 방식 — PDF 원본에서 LOT 번호를 직접 추출해 누락분 보완
            try:
                ...
                _regex_unique = list(dict.fromkeys(_regex_lots))
                _gemini_lot_nos = {lot.lot_no for lot in result.lots}
                _missing = [l for l in _regex_unique if l not in _gemini_lot_nos]

                if _missing and result.lots:
                    _ref = result.lots[-1]
                    _added = 0
                    for _lot_no in _missing:
                        _new_lot = LOTItem(
                            list_no=len(result.lots) + 1,
                            container_no=_ref.container_no,
                            lot_no=_lot_no,
                            lot_sqm='',
                            mxbg=_ref.mxbg,
                            net_weight_kg=_ref.net_weight_kg,
                            gross_weight_kg=_ref.gross_weight_kg,
                        )
                        result.lots.append(_new_lot)
                        _added += 1
                    if _added:
                        result.total_net_weight_kg = sum(
                            l.net_weight_kg for l in result.lots
                        )
                        result.total_gross_weight_kg = sum(
                            l.gross_weight_kg for l in result.lots
                        )
                        logger.info(
                            "[GeminiParser] PL pdfplumber 보강: %d개 추가 "
                            "(Gemini %d개 → 합계 %d개)",
                            _added, len(result.lots) - _added,
                            len(result.lots),
                        )
            except Exception as _pdf_err:
                logger.debug(
                    "[GeminiParser] PL pdfplumber 보강 스킵: %s", _pdf_err
                )
            ...
            save_parse_debug_report(
                ...
                hints_used=gemini_hint,
                partial_results={"lots_count": len(result.lots)},
            )
    ...
    def parse_invoice(self, pdf_path: str, gemini_hint: str = '') -> InvoiceResult:
        ...
        try:
            ...
            if not images:
                result.error_message = "PDF 이미지 변환 실패"
                save_parse_debug_report("INV", Path(pdf_path).name, result.error_message)
                return result
...
모든 LOT 번호를 빠짐없이 추출해주세요. JSON만 응답해주세요."""
            ...
            if gemini_hint and gemini_hint.strip():
                prompt += f"\n\n[선사별 추가 파싱 힌트]\n{gemini_hint.strip()}"
                logger.debug(f"[GeminiParser] Invoice 힌트 주입 ({len(gemini_hint)}자)")
...
JSON만 출력하세요."""
            ...
            save_parse_debug_report(
                "INV", Path(pdf_path).name, result.error_message,
                raw_response=getattr(result, 'raw_response', ''),
                hints_used=gemini_hint,
                partial_results={"lot_count": len(result.lot_numbers)},
            )
    ...
    def parse_bl(self, pdf_path: str, gemini_hint: str = '') -> BLResult:
        ...
        try:
            ...
            if not images:
                result.error_message = "PDF 이미지 변환 실패"
                save_parse_debug_report(
                    "BL", Path(pdf_path).name, result.error_message,
                    carrier_id=getattr(_tmpl, 'carrier_id', '') if _tmpl else '',
                )
                return result
...
모든 컨테이너를 빠짐없이 추출해주세요. JSON만 응답해주세요."""
            ...
            if gemini_hint and gemini_hint.strip():
                prompt += f"\n\n[선사별 추가 파싱 힌트]\n{gemini_hint.strip()}"
                logger.debug(f"[GeminiParser] BL 힌트 주입 ({len(gemini_hint)}자)")
            ...
            save_parse_debug_report(
                ...
                carrier_id=getattr(_tmpl, 'carrier_id', '') if _tmpl else '',
                hints_used=gemini_hint,
                partial_results={"bl_no": result.bl_no, "containers": len(result.containers)},
            )
...
모든 컨테이너를 빠짐없이 추출해주세요. JSON만 응답해주세요."""
    ...
    def parse_do(self, pdf_path: str) -> DOResult:
        ...
        try:
            ...
            if not images:
                result.error_message = "PDF 이미지 변환 실패"
                save_parse_debug_report("DO", Path(pdf_path).name, result.error_message)
                return result
            ...
            save_parse_debug_report(
                "DO", Path(pdf_path).name, result.error_message,
                raw_response=getattr(result, 'raw_response', ''),
                partial_results={"bl_no": result.bl_no, "do_no": result.do_no},
            )
        return result
    ...
    def parse_do_from_image(self, image_bytes: bytes, mime_type: str = "image/png") -> DOResult:
        ...
        try:
            ...
            save_parse_debug_report(
                "DO", "(image_capture)", result.error_message,
                raw_response=getattr(result, 'raw_response', ''),
            )
        return result

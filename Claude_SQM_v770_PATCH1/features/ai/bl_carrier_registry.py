...
CARRIER_TEMPLATES: dict[str, CarrierTemplate] = {
    ...
    # 실제 ONE BL 샘플 없음 → 보수적 패턴, Gemini fallback
    "ONE": CarrierTemplate(
        carrier_id="ONE",
        carrier_name="Ocean Network Express",
        detect_keywords=["OCEAN NETWORK EXPRESS", "ONE", "ONE (OCEAN NETWORK EXPRESS)"],
        detect_pattern=r"OCEAN NETWORK EXPRESS",
        bl_extract_pattern=r"B(?:/L|ILL OF LADING)\s*(?:No\.?|:)\s*([A-Z0-9]{6,20})",
        bl_page_scope="page0",
        bl_format_hint="ONEYABCD1234567",  # 예시 — 실제 형식 확인 후 업데이트 필요
        sap_page_hint="all",
        bl_no_prompt_hint=(
            "【ONE (Ocean Network Express) Bill of Lading 전용 규칙】\n"
            "BL No 위치: 1페이지 상단 'B/L No.' 라벨 근처\n"
            "⚠️ 주의: 실제 ONE BL 샘플 기반이 아니므로 추출 후 반드시 확인하세요."
        ),
    ),
}
...
def extract_bl_no_by_template(
    ...
    for page_i, text in enumerate(search_pages):
        ...
        if m:
            ...
            # v7.7.2: 숫자가 하나도 없으면 마스킹/편집된 PDF → 스킵
            if not re.search(r'\d', bl_no):
                logger.warning(
                    f"[BLExtract] {tmpl.carrier_id} 추출값에 숫자 없음"
                    f"({bl_no!r}) → 스킵 (마스킹된 PDF 가능성)"
                )
                continue
            logger.info(

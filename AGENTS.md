...
## Excel/데이터 입력 원칙 (Upload Principle) — **영구 유효**

- **프로그램이 사용하는 엑셀/데이터 형식은 프로그램 안에서 자체 템플릿(형식)으로 내장한다.**
- **사용자 입력 방식은 항상 다음 두 가지로 통일한다.**
  1. **데이터 붙여넣기**: 프로그램이 내장한 형식에 맞춰 표를 띄우고, 사용자가 Excel 등에서 복사한 데이터를 붙여넣기(Ctrl+V) 후 반영.
  2. **파일 업로드**: 이미 채운 엑셀 파일을 선택하여 업로드.
- 이 원칙은 **기능별로 따로 말하지 않아도** 입고·출고·반품·위치·Allocation 등 **전체 프로그램에 동일하게 적용**한다.
- **프로그램 수명 동안 항상 유효**하며, 새 기능 추가 시에도 동일한 패턴을 따른다.

    ...
    def reserve_from_allocation(self, allocation_rows: list, source_file: str = '', reservation_mode: str = '') -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                _batch_processed_lots = set()  # v8.6.5: 같은 배치에서 이미 처리한 LOT (DUP 스킵용)
...
# [v6.8.6 top-level import로 이동]                         from engine_modules.constants import normalize_customer
                    ...
                    # v8.6.5: 동일 sale_ref를 여러 LOT에 배정하는 것은 정상 업무
                    # (1개 Sales Order에 88개 LOT 배정 = 정상)
                    # HARD STOP 제거 → 동일 LOT+sale_ref 중복만 차단
                    if sale_ref:
                        try:
                            # v8.6.5: 같은 배치에서 이미 처리한 LOT는 DUP 스킵
                            # (본품+샘플이 같은 LOT로 2행 → 정상)
                            _lot_key = (sale_ref, lot_no)
                            if _lot_key not in _batch_processed_lots:
                                # 동일 LOT + 동일 sale_ref가 이전 배치에서 이미 RESERVED인지 확인
    ...
    def confirm_outbound(self, lot_no: str = None, force_all: bool = False) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[F 이중SOLD체크] safety guard 실패 — 출고 차단: {_fe}")
                result['errors'].append(f"이중 SOLD 체크 실패: {_fe}")
                return result

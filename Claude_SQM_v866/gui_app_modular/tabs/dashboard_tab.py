                        ...
                        for w in card.winfo_children():
                            try:
                                ...
                                for ww in w.winfo_children():
                                    ...
                                    except Exception as e: logger.debug(f"[Dashboard] card child config: {e}")
                            except Exception as e: logger.debug(f"[Dashboard] card widget config: {e}")
                        # 좌측 색바 갱신 (첫 번째 자식의 첫 번째 자식)
                        try:
                            inner = card.winfo_children()[0]
                            color_bar = inner.winfo_children()[0]
                            color_bar.config(bg=color)
                        except (IndexError, Exception) as e: logger.debug(f"[Dashboard] color bar config: {e}")
                        # 값/제목 레이블 색상 갱신

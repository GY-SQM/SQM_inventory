...
export default function InventoryPage() {
  ...
  const loadFilterOptions = async (signal) => {
    try {
      const data = await apiGet("/inventory/filters", { signal });
      setOptions({
      ...
      setError("필터 옵션을 불러오지 못했습니다. 새로고침 해주세요.");
    }
  };

  const loadInventory = async (nextFilters = filters, signal) => {
    setLoading(true);
    setError("");
    try {
      const query = buildQuery(nextFilters);
      const data = await apiGet(`/inventory/search${query}`, { signal });
      setRows(data.rows || []);
  ...
  useEffect(() => {
    ...
    loadFilterOptions(ctrl.signal);
    loadInventory(undefined, ctrl.signal);
    return () => ctrl.abort();

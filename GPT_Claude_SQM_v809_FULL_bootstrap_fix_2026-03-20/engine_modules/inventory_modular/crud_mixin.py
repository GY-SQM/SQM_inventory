...
class CRUDMixin:
    ...
    def _recalc_current_weight(self, lot_no: str, reason: str = '') -> float:
        ...
        try:
            ...
            row_avail = self.db.fetchone(
                ...
                "WHERE lot_no = ? AND status IN ('AVAILABLE','RESERVED') "
    ...
    def _recalc_current_weight_many(self, lot_nos, reason: str = '') -> dict:
        """v8.0.0 [P2]: 여러 LOT의 current/picked weight 중앙 재계산."""
        result = {'recalculated': 0, 'skipped': 0, 'lot_nos': []}
        seen = set()
        for lot_no in lot_nos or []:
            lot_no = str(lot_no or '').strip()
            if not lot_no or lot_no in seen:
                result['skipped'] += 1
                continue
            seen.add(lot_no)
            self._recalc_current_weight(lot_no, reason=reason)
            result['recalculated'] += 1
            result['lot_nos'].append(lot_no)
        return result

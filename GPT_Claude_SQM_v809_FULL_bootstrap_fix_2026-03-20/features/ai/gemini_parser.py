...
JSON만 출력하세요."""
            ...
            # ★ v7.3.6 [GEMINI-PL-FIX]: 1페이지 누락 감지 → 보강 재시도
            # 1페이지 PL에서 LOT가 30개 미만으로 파싱됐을 때 재시도 1회 수행
            # (기존: 19개만 탐지, 재시도 없음 → 23개 누락 등 조용히 통과하던 버그 수정)
            if len(images) == 1 and 0 < len(result.lots) < 30:
                _before_retry = len(result.lots)
                logger.warning(
                    f"[GeminiParser] PL 1페이지인데 {_before_retry}개만 추출됨 — "
                    f"누락 가능성 감지, 보강 재시도 실행 (v7.3.6)"
                )
                try:
                    ...
                    if _retry_data and isinstance(_retry_data.get('lots'), list):
                        ...
                        if _added > 0:
                            ...
                            logger.info(
                                f"[GeminiParser] PL 보강 재시도: 추가 LOT 없음 "
                                f"({_before_retry}개가 최종)"
                            )
                except Exception as _retry_err:
                    logger.warning(f"[GeminiParser] PL 보강 재시도 실패: {_retry_err}")

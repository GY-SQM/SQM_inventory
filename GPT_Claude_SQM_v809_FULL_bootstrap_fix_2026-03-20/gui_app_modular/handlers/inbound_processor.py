...
class InboundProcessorMixin:
    ...
    def _on_pdf_inbound(self, initial_files: dict = None) -> None:
        ...
        try:
            ...
            self._log(f"❌ PDF 스캔 입고 실행 실패: {e}")
            logger.error(f"PDF 스캔 입고 실행 실패: {e}", exc_info=True)
            try:
                from ..utils.custom_messagebox import CustomMessageBox
                CustomMessageBox.showerror(self.root, "PDF 스캔 입고 오류", f"원스톱 입고 창을 열지 못했습니다.\n\n{e}")
            except Exception:
                logger.debug("[SUPPRESSED] exception in inbound_processor.py")  # noqa

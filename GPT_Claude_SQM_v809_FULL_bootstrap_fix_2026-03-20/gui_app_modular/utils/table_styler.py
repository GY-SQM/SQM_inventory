...
class TableStyler:
    ...
    COLORS_DARK = {
        ...
        'header_bg': '#2d2b55',  # v6.3.2: 다크 퍼플 헤더
        'header_fg': '#f0f0f0',
        'border': '#555555',
        'foreground': '#f0f0f0',
    ...
    def refresh_striped_rows(
        ...
        dark_mode = cls._detect_dark()
        colors = cls.COLORS_DARK if dark_mode else cls.COLORS
        fg = colors.get('foreground', '#f0f0f0') if dark_mode else '#1a1a1a'
        treeview.tag_configure(tag_even, background=colors['row_even'], foreground=fg)
        treeview.tag_configure(tag_odd, background=colors['row_odd'], foreground=fg)

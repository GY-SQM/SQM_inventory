...
def _refresh_single_treeview(tree: ttk.Treeview, colors: dict, style: ttk.Style) -> None:
    ...
    try:
        ...
        TableStyler.update_grid_style_for_theme(tree, dark_mode)
    except (ImportError, Exception) as e:
        logger.debug(f"[_refresh_single_treeview] Suppressed: {e}")
...
def _refresh_native_widget(widget, colors: dict) -> None:
    ...
    if not needs_fix:
        ...
        wrong_bg_for_theme = (dark_mode and bg_b > 170) or ((not dark_mode) and bg_b < 85)
        wrong_fg_for_theme = (dark_mode and fg_b < 120) or ((not dark_mode) and fg_b > 180)

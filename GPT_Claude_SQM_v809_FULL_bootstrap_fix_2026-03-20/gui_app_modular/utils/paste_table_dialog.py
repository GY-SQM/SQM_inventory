...
def show_paste_table_dialog(
    ...
    try:
        ...
        dark_mode = _is_dark_theme(win)
    except Exception:
        dark_mode = False
    bg_cell = ThemeColors.get("bg_card", dark_mode) or "#FFFFFF"
    fg_cell = ThemeColors.get("text_primary", dark_mode) or "#333333"
    header_bg = ThemeColors.get("btn_report", dark_mode) or "#4472C4"
    ...
    sb_trough = "#111111" if not dark_mode else "#f2f2f2"
    sb_thumb = "#f2f2f2" if not dark_mode else "#111111"
    sb_active = "#ffffff" if not dark_mode else "#000000"
    sb_width = 16

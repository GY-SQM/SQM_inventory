...
OUTBOUND_SCHEDULED_COLUMNS = [
    ...
    ('current_weight_after_allocation', 'Balance(Kg)', 100, 'e', True),
    ('allocated_kg',       '예약(Kg)',       90, 'e', True),
    ('allocated_count',    '예약(개)',       70, 'center', True),
...
class OutboundScheduledTabMixin:
    ...
    def _setup_ob_allocation_detail_panel(self) -> None:
        ...
        for cid, txt, w in [
            ...
            self._ob_alloc_detail_tree.heading(cid, text=txt)
            self._ob_alloc_detail_tree.column(cid, width=w)
        sb = tk.Scrollbar(detail_container, orient=VERTICAL, command=self._ob_alloc_detail_tree.yview)
    ...
    def _show_tonbag_included_popup(self) -> None:
        ...
        tk.Label(header, text="판매배정·판매화물 결정·출고·선적 상태 톤백 (LOT별 정렬)",
                 font=('맑은 고딕', 9), bg=bg, fg=ThemeColors.get('text_secondary', dark_mode)).pack(anchor='w')
    ...
    def _show_lot_outbound_history_popup(self, lot_no: str) -> None:
        ...
        tk.Label(header, text="출고된 톤백·샘플 이력 (여러 번에 걸친 출고 포함)",
                 font=('맑은 고딕', 10), bg=bg, fg=ThemeColors.get('text_secondary', dark_mode)).pack(anchor='w')

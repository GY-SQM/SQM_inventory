...
class PickedTabMixin:
    ...
    def _setup_picked_tab(self) -> None:
        ...
        try:
            apply_striped_rows(self.tree_picked, dark_mode)
    ...
    def _apply_picked_edit(self, row_iid, col_idx, col_key, new_val, lot_no, vals) -> None:
        ...
        try:
            ...
            try:
                ...
                self.tree_picked.tag_configure(
                    _orig_tag, background=_flash_bg, foreground=_flash_fg)
                self.tree_picked.item(row_iid, tags=(_orig_tag,))
                def _restore_tag(_iid=row_iid):
                    try:
                        self.tree_picked.item(_iid, tags=())
                    except Exception:
                        pass
                self.root.after(800, _restore_tag)
            except Exception as _he:
                logger.debug("[InlineEdit/Picked] highlight 무시: %s", _he)

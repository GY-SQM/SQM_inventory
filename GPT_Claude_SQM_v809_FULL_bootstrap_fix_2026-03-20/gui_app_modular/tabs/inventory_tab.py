...
# ═══════════════════════════════════════════════════════════════
INVENTORY_COLUMNS = [
    # v7.3.7: 핵심 10개 기본 ON, 나머지 OFF (메뉴 → 컬럼 표시에서 언제든 ON 가능)
    ...
    ('avail_bags',         'Avail',          60, 'center', False),  # v7.3.7 OFF
    # 톤백 상태 카운트 — 기본 OFF (필요 시 컬럼 표시 메뉴에서 ON)
    ('tb_avail',           '↓Avail개',       62, 'center', False),  # v7.3.7 OFF
    ('tb_reserved',        '↓Resv개',        62, 'center', False),  # v7.3.7 OFF
    ('tb_picked',          '↓Pick개',        62, 'center', False),  # v7.3.7 OFF
    ('tb_sold',            '↓Sold개',        62, 'center', False),  # v7.3.7 OFF
    ('salar_invoice_no',   'INVOICE NO',    100, 'center', False),  # v7.3.7 OFF
    ('ship_date',          'SHIP DATE',      95, 'center', False),  # v7.3.7 OFF
    ('arrival_date',       'ARRIVAL',        95, 'center', True),   # 입고일 — 실무 필수
    ('con_return',         'CON RETURN',     95, 'center', False),  # v7.3.7 OFF
    ('free_time',          'FREE TIME',      80, 'center', False),  # v7.3.7 OFF
    ('warehouse',          'WH',             80, 'center', False),  # v7.3.7 OFF
    ('customs',            'CUSTOMS',        90, 'center', False),  # v7.3.7 OFF
    ('initial_weight',     'Inbound(Kg)',    100, 'e',      False),  # v7.3.7 OFF
    ('outbound_weight',    'Outbound(Kg)',   100, 'e',      False),  # v7.3.7 OFF
]
...
class InventoryTabMixin:
    ...
    def _setup_inventory_tab(self) -> None:
        ...
        self._inv_split_panel = MasterDetailSplitPanel(
            ...
            master_weight=5,   # v7.9.9 [Q1]: 3→5 비율로 오른쪽 패널 축소
            detail_weight=1,
            master_minsize=200,
        )
        ...
        dark_mode_tv = is_dark()
        _tv_bg = ThemeColors.get('bg_card', dark_mode_tv)
        _tv_fg = ThemeColors.get('text_primary', dark_mode_tv)
        _tv_field = _tv_bg
        _tv_head_bg = ThemeColors.get('bg_secondary', dark_mode_tv)
        _tv_head_fg = ThemeColors.get('text_primary', dark_mode_tv)
        ...
        _style.map('Inv.Treeview',
                   background=[('selected', ThemeColors.get('tree_select_bg', dark_mode_tv))],
                   foreground=[
                       ('selected', ThemeColors.get('tree_select_fg', dark_mode_tv)),
                       ('!selected', _tv_fg),
                   ])
        ...
        # self._inv_footer = FooterTotalBar(self.tab_inventory, is_dark=dark_mode_filter)
    ...
    def _on_inv_double_click(self, event) -> None:
        ...
        try:
            ...
            if lot_info:
                ...
                for i, (lbl, val) in enumerate(fields):
                    r, c = divmod(i, 3)
                    _ttk.Label(info_frame, text=f"{lbl}:", font=('맑은 고딕', 9, 'bold')).grid(
                        row=r, column=c*2, sticky='e', padx=(8,2), pady=2)
                    _ttk.Label(info_frame, text=str(val), font=('맑은 고딕', 9)).grid(
        ...
        tb_frame.pack(fill='both', expand=True, padx=10, pady=(0, 10))

        cols = ('no', 'sub_lt', 'weight', 'status', 'location', 'picked_to', 'outbound_date')
        tree = _ttk.Treeview(tb_frame, columns=cols, show='headings', height=12)
    ...
    def _apply_inventory_theme_colors(self) -> None:
        ...
        dark_mode = is_dark()
        ThemeColors.configure_tags(self.tree_inventory, dark_mode)
        try:
            from ..utils.table_styler import TableStyler
            TableStyler.update_grid_style_for_theme(self.tree_inventory, dark_mode)
        except (ImportError, Exception) as e:
    ...
    def _update_inv_filter_values(self, inventory) -> None:
        ...
        try:
            ...
            cnt_return = sum(1 for i in inventory if (i.get('status') or '') == STATUS_RETURN)
            status_values = [
    ...
    def _show_empty_state_hint(self) -> None:
        ...
        try:
            dark_mode = is_dark()
            _fg = ThemeColors.get('text_primary', dark_mode)
            _msg = (
                "표시할 데이터가 없습니다. "
            ...
            self._empty_hint = tk.Label(
                ...
                font=('맑은 고딕', 10),
                fg=_fg,
                bg=ThemeColors.get('bg_card', dark_mode),
                wraplength=500,
            )

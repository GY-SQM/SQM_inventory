...
class ContextMenuMixin:
    ...
    def _show_lot_history_timeline(self) -> None:
        ...
        _accent = ThemeColors.get('statusbar_progress', dark_mode)
        _green = ThemeColors.get('badge_db', dark_mode)
        _orange = ThemeColors.get('statusbar_icon_warn', dark_mode)
        _red = ThemeColors.get('statusbar_icon_err', dark_mode)
        ...
        header.pack(fill=X)
        _header_fg = ThemeColors.get('badge_text', dark_mode)
        ...
        for idx, (icon, date, title, detail, color) in enumerate(events):
            ...
            tk.Label(content, text=detail, bg=_bg, fg=ThemeColors.get('text_muted', dark_mode),
        ...
        tk.Label(pf, text=f"입고: {init_w:,.0f}kg | 출고: {init_w-curr_w:,.0f}kg | 잔량: {curr_w:,.0f}kg",
                 bg=_bg, fg=ThemeColors.get('text_muted', dark_mode), font=('맑은 고딕', 9)).pack(anchor=W)

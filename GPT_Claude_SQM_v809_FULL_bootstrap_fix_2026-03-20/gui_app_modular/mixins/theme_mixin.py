...
class ThemeMixin:
        ...
        try:
            if hasattr(self.root, 'style'):
                try:
                    ...
                    set_global_theme(theme_name)  # v9.0: 전역 동기화
            ...
            try:
                ...
                bg_color = ThemeColors.get('bg_primary', dark_mode)
                try:
    ...
    def _update_theme_colors_fallback(self) -> None:
        ...
        dark_mode = ThemeColors.is_dark_theme(self.current_theme)
        ...
        if hasattr(self, 'tree_sublot'):
            ThemeColors.configure_tags(self.tree_sublot, dark_mode)
            try:
                from ..utils.table_styler import TableStyler
                TableStyler.update_grid_style_for_theme(self.tree_sublot, dark_mode)
            except (ImportError, Exception):
                logger.debug("[THEME-FAIL] file=exception in theme_mixin.py reason=theme_apply_error")  # noqa
        try:

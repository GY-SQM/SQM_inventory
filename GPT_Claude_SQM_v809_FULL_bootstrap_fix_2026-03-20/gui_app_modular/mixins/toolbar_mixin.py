...
from ..utils.theme_helpers import safe_is_dark
...
class ToolbarMixin:
    ...
    def _create_menu(self, parent=None) -> 'tk.Menu':
        ...
        # 지역명 is_dark 금지: import한 is_dark()와 충돌 시 UnboundLocalError
        dark_mode = safe_is_dark(self)
        menu_bg = ThemeColors.get('bg_card', dark_mode)
        menu_fg = ThemeColors.get('text_primary', dark_mode)
        menu_abg = ThemeColors.get('bg_hover', dark_mode)
        menu_afg = ThemeColors.get('text_primary', dark_mode)
        menu_dis = ThemeColors.get('text_muted', dark_mode)
    ...
    def _build_quick_theme_buttons(self, parent) -> None:
        ...
        try:
            ...
            for icon, label, theme in [("🌙", "Dark", "darkly"), ("☀", "Light", "flatly")]:
                btn = tk.Label(
    ...
    def _refresh_toolbar_colors(self) -> None:
        ...
        try:
            ...
            for child in self._row1.winfo_children():
                try:
                    child.config(bg=self._tb_bg, fg=self._tb_fg_normal)
    ...
    def _build_sidebar_tab_buttons(self) -> None:
        ...
        tab_defs = [
            ('inventory',      '📦', '판매가능',
             'LOT 리스트(판매가능). 필터·검색 후 더블클릭 시 LOT 상세·톤백.'),
            ('allocation',     '📋', '판매배정',
        ...
        for key, icon, label, tip in tab_defs:
            ...
            def make_handlers(w, i_lbl, t_lbl):
                def on_enter(e):
                    if not w._active:
                ...
                def on_leave(e):
                    if not w._active:
                        ...
                        i_lbl.config(fg=ICON_FG)
                        t_lbl.config(fg=TEXT_FG)
                return on_enter, on_leave

            on_enter, on_leave = make_handlers(wrapper, icon_lbl, text_lbl)
        ...
        def _toggle_theme_from_sidebar():
            try:
                current = getattr(self, 'current_theme', 'flatly')
                from gui_app_modular.utils.constants import DARK_THEMES, LIGHT_THEMES
                dark_now = current in DARK_THEMES if hasattr(__import__('gui_app_modular.utils.constants', fromlist=['DARK_THEMES']), 'DARK_THEMES') else 'dark' in current.lower()
                next_theme = 'darkly' if not dark_now else 'flatly'
        ...
        self._attach_tooltip(theme_btn, ' 테마 전환 (밝음/어두움) ')
    ...
    def _highlight_active_tab(self) -> None:
        ...
        for key, wrapper in wrappers.items():
            ...
            icon_fg = ACTIVE_FG  if is_active else ICON_FG
    ...
    def _show_search_popup(self) -> None:
        ...
        tk.Label(df, text='  (YYYY-MM-DD)', font=_small_font, fg=ThemeColors.get('text_muted', dark_mode)
    ...
    def _safe_call(self, method_name: str):
        """메서드 안전 호출 (호출/실패를 로그와 팝업으로 가시화)."""
        fn = getattr(self, method_name, None)
        if fn and callable(fn):
            try:
                logger.info(f"[toolbar] 호출: {method_name}")
                fn()
            except Exception as e:
                logger.error(f"[toolbar] {method_name} 실행 오류: {e}", exc_info=True)
                try:
                    CustomMessageBox.showerror(
                        self.root if hasattr(self, "root") else None,
                        "실행 오류",
                        f"{method_name} 실행 중 오류가 발생했습니다.\n\n{e}",
                    )
                except Exception as _e:
                    logger.debug(f"[toolbar] 오류 팝업 실패: {_e}")
        else:

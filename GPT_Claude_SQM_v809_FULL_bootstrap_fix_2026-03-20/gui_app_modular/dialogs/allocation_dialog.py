...
class AllocationDialog:
    ...
    def _on_reserve(self):
        ...
        # ── v7.9.3 [PRE-DUP]: 이미 예약된 LOT 사전 팝업 확인 ───────────────────
        pre_dup = self._check_pre_dup_lots()
        if pre_dup.get('found'):
            dup_lots  = pre_dup.get('lots', [])
            dup_count = pre_dup.get('count', 0)
            preview   = ', '.join(dup_lots[:10])
            more      = f'\n... 외 {dup_count - 10}건' if dup_count > 10 else ''
            sale_ref_info = ''
            if pre_dup.get('sale_refs'):
                sale_ref_info = f'\nSALE REF: {", ".join(pre_dup["sale_refs"])}'
            pre_dup_msg = (
                f'⚠️ 이미 예약(RESERVED)된 LOT {dup_count}개가 포함되어 있습니다.\n\n'
                f'{preview}{more}{sale_ref_info}\n\n'
                '─────────────────────────────\n'
                '[ 선택 안내 ]\n'
                '  ✅ 예 → 계속 진행\n'
                '        (해당 LOT은 LOT_MODE_DUP으로 자동 스킵됩니다)\n\n'
                '  ❌ 아니오 → 취소\n'
                '        [Allocation 탭] → SALE REF 일괄 취소 후 재업로드 권장\n'
                '─────────────────────────────\n'
                '계속 진행하시겠습니까?'
            )
            if not CustomMessageBox.askyesno(self.dialog, '⚠️ 기존 예약 LOT 감지', pre_dup_msg):
                return
    ...
    def _show_reservation_status(self):
        ...
        try:
            ...
            dark_mode = is_dark()
            status_win.configure(bg=ThemeColors.get('bg_card', dark_mode))

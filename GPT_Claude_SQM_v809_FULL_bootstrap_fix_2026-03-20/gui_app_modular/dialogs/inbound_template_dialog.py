...
class InboundTemplateDialog:
    ...
    def __init__(self, parent, engine, current_theme='flatly', on_select_callback=None):
        ...
        self.bg = ThemeColors.get('bg_card', dark_mode) if ThemeColors else '#FFFFFF'
    ...
    def _build_tab_basic(self, p):
        ...
        def e_bl_fmt(r):
            ...
            ttk.Label(frm, text='  예) 숫자9=MAERSK, MSCU7=MSC',
                      font=('맑은 고딕', 9), foreground=tc('text_muted')).pack(side=tk.LEFT)

        row(6,'BL 번호 형식', e_bl_fmt, '직접 지정 시 선사 감지 없이 해당 형식으로만 추출')
    ...
    def _build_tab_gemini(self, p):
        ...
        tk.Label(p,
                 ...
                 font=('맑은 고딕',9),fg=tc('text_muted'),anchor='w').grid(

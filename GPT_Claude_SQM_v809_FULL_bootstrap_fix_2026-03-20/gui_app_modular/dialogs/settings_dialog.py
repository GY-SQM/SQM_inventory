...
class SettingsDialogMixin:
    ...
    def _show_api_settings(self) -> None:
        ...
        dark_mode = is_dark()
        _bg = ThemeColors.get('bg_card', dark_mode)
        _fg = ThemeColors.get('text_primary', dark_mode)
        dialog.configure(bg=_bg)

        # 헤더 (v8.7.0 Phase2: ThemeColors, Phase3: Spacing)
        header = tk.Frame(dialog, bg=ThemeColors.get('info', dark_mode), pady=Spacing.SM)
        header.pack(fill=X)
        tk.Label(header, text="🔐 Gemini API 키 설정", bg=ThemeColors.get('info', dark_mode), fg=ThemeColors.get('badge_text', dark_mode),
        ...
        tk.Label(info_frame, text=f"  모델: {GEMINI_MODEL}", bg=_bg, fg=ThemeColors.get('text_muted', dark_mode), font=('맑은 고딕', 9)).pack(anchor=W)
        ...
        def save_key():
            ...
            if key:
                if not key.startswith('AIza'):
                    result_label.config(text="⚠️ Google API 키 형식이 아닙니다 (AIza...)", fg=ThemeColors.get('statusbar_icon_warn', dark_mode))
                    return
                method = save_api_key_secure(key)
                if method == 'KEYRING':
                    result_label.config(text="✅ API 키·모델 저장됨 (다음 실행부터 모델 적용)", fg=ThemeColors.get('badge_db'))
                elif method == 'INI':
                    result_label.config(text="✅ API 키·모델 저장됨 (다음 실행부터 모델 적용)", fg=ThemeColors.get('badge_db', dark_mode))
                else:

...
class PreParseSelectDialog:
    ...
    def __init__(
        ...
        self._use_multi_template = tk.BooleanVar(value=True)
    ...
    tc
        ...
        # ── ③ 멀티템플릿 후보 옵션 ──────────────────────────────
        sec3 = tk.Frame(dlg, bg=sec_bg, bd=1, relief="solid")
        ...
        mt_row = tk.Frame(sec3, bg=sec_bg)
        mt_row.pack(fill=tk.X, padx=10, pady=(0, 6))

        tk.Checkbutton(
            mt_row,
            text="🔍 다중 템플릿 후보 자동 비교 (권장)",
            variable=self._use_multi_template,
            font=("맑은 고딕", 11),
            bg=sec_bg, fg=fg,
            activebackground=sec_bg,
            cursor="hand2",
        ).pack(side=tk.LEFT)

        tk.Label(
            mt_row,
            text="  — BL/DO를 선사별 최적 템플릿으로 자동 선택",
            font=("맑은 고딕", 10),
            bg=sec_bg, fg=tc('text_muted'),
        ).pack(side=tk.LEFT)

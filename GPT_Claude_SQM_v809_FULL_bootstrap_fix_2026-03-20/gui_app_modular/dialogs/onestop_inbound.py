...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def _create_dialog(self) -> None:
        ...
        self.btn_reparse.grid(row=0, column=5, padx=(0, 8))
        ...
        self.btn_reparse_carrier.grid(row=0, column=6, padx=(0, 8))
        self._attach_doc_tooltip(
        ...
        # v9.0: D/O 나중에 버튼 → _tpl_row로 이동 (아래 참조)
        # self.btn_add_do_later는 _tpl_row에서 생성됨
        ...
        # 숨겨진 콤보박스 (하위호환용 — 실제 UI는 테이블)
        self._tpl_var = tk.StringVar(value='')
        self._tpl_combo = ttk.Combobox(
            _tpl_row, textvariable=self._tpl_var, state='readonly', width=0
        )
        ...
        self._tpl_badge = tk.Label(
            _tpl_row, text='',
            font=('맑은 고딕', 11, 'bold'),
            fg=tc('text_primary'), bg=tc('btn_report'),
            relief='flat', padx=8, pady=2, bd=0
        )
        self._tpl_badge.pack(side=LEFT, padx=(0, 8))

        # 테이블 선택 버튼
        ttk.Button(
            _tpl_row, text='📋 템플릿 선택',
            command=self._show_template_table_picker
    ...
    def _on_template_selected(self, _event=None):
        ...
        # 뱃지 색상: 500=파랑, 1000=주황
        bg = tc('btn_report') if bag == 500 else '#E67E22'
        try:
            self._tpl_badge.config(
                text=f'  ⚖️ {bag:,} kg/백  ', bg=bg, fg=tc('text_primary')
            )
        except Exception:
            logger.debug("[SUPPRESSED] exception in onestop_inbound.py")  # noqa
    ...
    def _show_template_table_picker(self) -> None:
        """v8.0.0 [TEMPLATE-TABLE]: 템플릿 테이블 선택 팝업.
        선사 + 템플릿 정보를 한 표에 표시, 체크로 선택.
        """
        import tkinter as _tk
        from tkinter import ttk as _ttk
        ...
        hdr.pack(fill='x')
        _ttk.Label(hdr, text="템플릿을 클릭하여 선택하세요",
                   font=('맑은 고딕', 12, 'bold')).pack(side='left')
        _ttk.Button(hdr, text="⚙ 템플릿 관리",
                    command=lambda: (popup.destroy(), self._open_template_manager())
                    ).pack(side='right')
        ...
        templates = list(getattr(self, '_template_map', {}).values())
        ...
        def on_click(event):
            ...
            for i, values in [(i, tree.item(i, 'values')) for i in iid_map]:
                ...
                tag = 'selected_row' if n == name else ('even_row' if list(iid_map.keys()).index(i) % 2 == 0 else 'odd_row')
    ...
    def _on_preparse_execute(
        ...
        if template_data:
            ...
            try:
                ...
                bag = int(template_data.get('bag_weight_kg', 500))
                bg = tc('btn_report') if bag == 500 else '#E67E22'
                if hasattr(self, '_tpl_var'):
                    self._tpl_var.set(tname)
                if hasattr(self, '_tpl_badge'):
                    self._tpl_badge.config(
                        text=f'  ⚖️ {bag:,} kg/백  ', bg=bg, fg=tc('text_primary')
                    )
                self._load_template_combo()
    ...
    def _parse_thread(self) -> None:
        ...
        try:
                                        ...
                                        if self.dialog and self.dialog.winfo_exists():
                                            self.dialog.after(
                                                ...
                                                lambda c=_carrier_id: (
                                                    ...
                                                    self._carrier_manual_var.set(c)
                                                )
                                            )
                                        # 힌트 교체 (템플릿 힌트 없을 때만)
            ...
            if getattr(self, 'preview_data', None) and len(self.preview_data) > 0:
                try:

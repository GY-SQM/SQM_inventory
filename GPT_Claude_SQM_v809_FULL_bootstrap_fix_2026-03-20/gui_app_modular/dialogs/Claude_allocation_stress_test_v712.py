...
class AllocationStressTestDialog:
    ...
    def _build_ui(self):
        ...
        tk.Label(bf, textvariable=self._sv, fg=tc('text_muted')
        ...
        _apply_tv_theme(self._tv, parent=tf)  # v9.0
        ...
        # 구분선 — Bug6 항목 강조 안내
        note_f = tk.Frame(self.win, bg=tc('bg_secondary'), pady=3)
        note_f.pack(fill=tk.X, padx=12)
        tk.Label(note_f,
                 text="  ℹ️  Bug6: continue 16건 전수감사 — 실제 위험 0건 확인 (v7.1.2)",
                 bg=tc('bg_secondary'), fg=tc('text_muted'), font=("맑은 고딕", 9)
                 ).pack(side=tk.LEFT)

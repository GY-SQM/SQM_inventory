...
class AllocationApprovalDialog:
    ...
    def show_queue(self) -> None:
        ...
        dark_mode = is_dark()
        win.configure(bg=ThemeColors.get("bg_card", dark_mode))
                    ...
                    for pid in sel:
                        cur = self.engine.db.execute(
                            """UPDATE allocation_plan
                               SET workflow_status=ALLOC_WF_APPROVED, approved_by=?, approved_at=?
            ...
            try:
                with self.engine.db.transaction("IMMEDIATE"):
                    for pid in sel:
                        self.engine.db.execute(
                            """UPDATE allocation_plan
                               SET workflow_status=ALLOC_WF_REJECTED, rejected_reason=?, approved_by=?, approved_at=?
                               WHERE id=? AND status='STAGED' AND workflow_status='PENDING_APPROVAL'""",

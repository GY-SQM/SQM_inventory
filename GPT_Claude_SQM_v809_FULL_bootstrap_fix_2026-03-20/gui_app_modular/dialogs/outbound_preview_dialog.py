...
class OutboundPreviewDialogMixin:
    ...
    def _show_outbound_preview(self, outbound_data: List[Dict], callback=None) -> None:
        ...
        # === UI 통일성: 테마 인식 색상 ===
        # 에러/경고용 색상
        error_bg = ThemeColors.get('picked', dark_mode)
        warning_bg = ThemeColors.get('reserved', dark_mode)
        ...
        if errors:
            ...
            result_color = ThemeColors.get('danger', dark_mode)
        elif warnings:
            result_text = f"⚠️ {len(warnings)} warnings - Review before proceeding"
            result_color = ThemeColors.get('warning', dark_mode)
        else:
            result_text = "✅ All items validated - Ready to proceed"
            result_color = ThemeColors.get('success', dark_mode)

...
class DocumentParserBase:
    ...
    def __init__(self, gemini_api_key: str = None):
        """
        Args:
            gemini_api_key: Gemini API 키 (이미지 PDF 처리용)
    ...
    def _require_gemini_api_key(self):
        """모든 문서 파싱을 Gemini API로 강제할 때 사용하는 게이트.
        ...
        if not key or key.startswith('your-'):
            raise RuntimeError(
                "Gemini API Key가 필요합니다. 설정(도구/설정 메뉴)에서 API Key를 입력한 뒤 다시 시도하세요."
            )
    ...
    def _call_gemini_smart(self, prompt: str, pdf_path: str,
                           gemini_parser=None) -> str:
        """v8.0.0 [SMART-PARSE]: 3단계 파싱 전략.

        1단계 — PDF 텍스트 추출 후 Gemini에 텍스트로 전달
                 (텍스트 레이어가 있는 PDF — 가장 빠르고 정확)
        2단계 — PDF 파일 직접 Gemini API 전송 (application/pdf)
                 (텍스트 추출 실패 or 결과 빈 경우)
        3단계 — PDF → 이미지 변환 후 Gemini Vision 전송
                 (스캔본 등 PDF 직접 전송도 실패한 경우)

        Args:
            prompt:        Gemini에 전달할 프롬프트
            pdf_path:      PDF 파일 경로
            gemini_parser: GeminiDocumentParser 인스턴스
        Returns:
            Gemini 응답 텍스트
        """
        import os
        ...
        if gemini_parser is None:
            raise ValueError("gemini_parser 필요")
        ...
        try:
            ...
            logger.debug(f"[SMART-PARSE] 텍스트 추출: {len(extracted_text)}자")
        except Exception as _e:
            logger.debug(f"[SMART-PARSE] pdfplumber 실패: {_e}")

        if extracted_text and len(extracted_text) > 50:
            # 텍스트가 충분히 있으면 텍스트로 전달
            text_prompt = (
                f"{prompt}\n\n"
                f"[문서 텍스트]\n{extracted_text[:12000]}"
            )
            try:
                result = gemini_parser._call_gemini(text_prompt)
                if result and len(result.strip()) > 10:
                    logger.info("[SMART-PARSE] 1단계 성공: 텍스트 추출 방식")
                    return result
            except Exception as _e:
                logger.debug(f"[SMART-PARSE] 1단계 실패: {_e}")

        # ── 2단계: PDF 직접 전송 ─────────────────────────────────────────
        try:
            result = gemini_parser._call_gemini_pdf(prompt, pdf_path)
            if result and len(result.strip()) > 10:
                logger.info("[SMART-PARSE] 2단계 성공: PDF 직접 전송")
                return result
        except Exception as _e:
            logger.debug(f"[SMART-PARSE] 2단계 실패: {_e}")

        # ── 3단계: 이미지 변환 폴백 ──────────────────────────────────────
        logger.warning("[SMART-PARSE] 3단계: 이미지 변환 폴백 (스캔본 가능성)")
        try:
            images = self._pdf_to_images(pdf_path)
            if images:
                result = gemini_parser._call_gemini(prompt, images[0])
                if result:
                    logger.info("[SMART-PARSE] 3단계 성공: 이미지 변환 방식")
                    return result
        except Exception as _e:
            logger.debug(f"[SMART-PARSE] 3단계 실패: {_e}")

        logger.error("[SMART-PARSE] 전체 실패")
        return ''

...
class PackingMixin:
    ...
    def _parse_packing_list_coord(self, pdf_path: str,
                                   bag_weight_kg: int = 500) -> "Optional[object]":
        ...
        try:
            ...
            raise RuntimeError(f"[PL-COORD] PDF 읽기 실패: {e}")
        ...
        result.sap_no               = (
            re.search(r"(\d{10})", os.path.basename(pdf_path) or "")
            or type("", (), {"group": lambda s, i: ""})()
        ).group(1) if pdf_path else ""
    ...
    def parse_packing_list(self, pdf_path: str,
        ...
        gemini_parser = GeminiDocumentParser(self.gemini_api_key)  # type: ignore[attr-defined]  # Mixin
        gemini_result = None
        try:
            gemini_result = self._gemini_with_retry(  # type: ignore[attr-defined]  # Mixin

    ...
    def parse_bl(self, pdf_path: str, gemini_hint: str = "", bl_format: str = "") -> Optional[BLData]:
        ...
        try:
            ...
            if carrier == 'MSC':
                ...
                if not vessel_m:
                    vessel_m = re.search(
                        ...
                        r'CMA\s+CGM\s+[A-Z ]+|ONE\s+[A-Z ]+)\s*[-\u2013]\s*(\d{3,4}[A-Z]?)',
                        text, re.IGNORECASE)
                    if vessel_m:
                        result.vessel = vessel_m.group(0).strip()
                if vessel_m and not result.vessel:
                    result.vessel = vessel_m.group(1).strip()

                # Voyage 분리
                voy_m = re.search(r'[-\u2013]\s*(\d{3,4}[A-Z]?)\s*$', result.vessel or '', re.MULTILINE)
                if voy_m:
                    result.voyage = voy_m.group(1).strip()
                    result.vessel = result.vessel[:result.vessel.rfind('-')].strip()

                # POL: PORT OF LOADING 다음 줄
                ...
                # Ship Date: "29-Jan-2026"
                ship_m = re.search(
                    r'SHIPPED?\s+ON\s+BOARD\s+DATE\s*\n?\s*'
                    r'(\d{1,2}[-/]\w{2,10}[-/]\d{2,4}|\w{3,9}\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2})',
                    text, re.IGNORECASE)
                if ship_m:
                    result.ship_date = self._normalize_date(ship_m.group(1))

                # Issue Date: PLACE AND DATE OF ISSUE → 다음 2줄에 날짜
                issue_m = re.search(
                    r'PLACE AND DATE OF ISSUE\s*\n[^\n]*\n\s*'
                    r'(\d{1,2}[-/]\w{2,10}[-/]\d{2,4}|\w{3,9}\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2})',
                    text, re.IGNORECASE)
                if issue_m:
            ...
            # ═══════════════ Maersk 전용 ════════════════════════
                ...
                bl_m = re.search(r'B/L[:\s]*No\.?\s*(\d{9})', text, re.IGNORECASE)
                if bl_m:
                    result.bl_no = bl_m.group(1)
                else:
                    bl_m = re.search(r'B/L:\s*(\d{9})', text)
                    if bl_m:
                        result.bl_no = bl_m.group(1)

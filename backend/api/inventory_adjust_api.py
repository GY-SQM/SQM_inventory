...
def execute_adjust(req: ExecuteRequest):
    ...
    # Phase 2-1: 재고 수정 전 자동 백업
    try:
        from backend.api.actions import auto_backup
        auto_backup("inventory_adjust")
    ...
    try:
        ...
        # Phase 2-2: 재고 수정 결과 감사 로그 기록
        try:
            from backend.api.audit_helper import write_audit
            db_path_str = str(_db_path())
            for lot_info in (result.success or []):
                lot_no = lot_info if isinstance(lot_info, str) else lot_info.get("lot_no", "")
                write_audit(
                    db_path_str,
                    "INVENTORY_ADJUST",
                    ...
                    user_note=f"재고 수동 수정 — {operator}",
                    created_by=operator,
                )
        except Exception as _ae:
            logger.warning(f"[execute_adjust] 감사 로그 기록 실패 (무시): {_ae}")

...
def _ensure_default_carriers(con):
    defaults = [
        ("MAERSK", "Maersk", "", 500.0, "기본 선사 프로파일"),
        ("MSC", "MSC", "", 500.0, "기본 선사 프로파일"),
        ("HAPAG", "Hapag-Lloyd", "", 500.0, "기본 선사 프로파일"),
        ("ONE", "ONE", "", 500.0, "기본 선사 프로파일"),
    ]
    for carrier_id, display_name, default_product, bag_weight_kg, note in defaults:
        con.execute(
            """INSERT OR IGNORE INTO carrier_profile
    ...
    try:
        ...
        row = con.execute(
            """SELECT * FROM carrier_profile
               WHERE UPPER(carrier_id)=UPPER(?) OR UPPER(display_name)=UPPER(?)""",

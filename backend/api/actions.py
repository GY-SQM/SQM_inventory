...
MAX_BACKUPS = 30   # Phase 2-1: 5 → 30개 보관


# ── 자동 백업 헬퍼 (Phase 2-1) ────────────────────────────────────
def auto_backup(operation: str = "auto") -> str:
    """중요 쓰기 작업 전 자동 백업. 백업 경로 반환. 실패해도 예외 발생 안 함."""
    try:
        root = _project_root()
        db_src = _db_path()
        ...
        safe_op = operation.replace(" ", "_")[:20]
        dst_name = f"sqm_auto_{safe_op}_{ts}.db"
        ...
        logger.info(f"[auto_backup] {dst_name} ({round(os.path.getsize(dst_path)/1024,1)} KB)")
        return dst_path
    except Exception as e:
        logger.warning(f"[auto_backup] 백업 실패 (무시): {e}")
        return ""
...
def _load_version() -> dict:
    ...
    try:
        ...
        return {"app_name": "SQM 재고관리 시스템", "version": "9.0.7.2"}
...
# ── LOT 리스트 JSON (v8.7.0 — 화면 테이블 렌더용) ────────────────────────
...
@router.get("/lot-list-json", summary="📋 LOT 리스트 JSON (화면 렌더용 v8.7.0)")
def lot_list_json():
    """
    ...
    try:
        ...
        rows = con.execute(_LOT_LIST_EXCEL_SQL).fetchall()
        rows = _append_lot_candidate_summary(rows, con)
        # v8.7.0: 톤백 regular/sample 분리 집계
        try:

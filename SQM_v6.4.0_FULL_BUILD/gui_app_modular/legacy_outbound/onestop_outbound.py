...
except Exception:  # 테마 유틸 미탑재 환경 호환
    ThemeAware = None
...
class S1OneStopOutboundDialog:
    ...
    def _show_placeholder(self):
        ...
        self.paste_text.config(fg='#9ca3af')
    ...
    def _refresh_tab2(self):
        ...
        for batch in self.batches:
            ...
            # 태그 설정 — 선택/미선택
            sel_bg = '#064e3b' if self._is_dark else '#d1fae5'
            sel_fg = '#6ee7b7' if self._is_dark else '#065f46'
            tree.tag_configure('sel', background=sel_bg, foreground=sel_fg)
            tree.tag_configure('unsel', background='', foreground='')
    ...
    def _refresh_tab3(self):
        ...
        # ★ 1단계: 상태별 행 배경색 태그 설정
        tree.tag_configure('row_FINALIZED', background='#d1fae5', foreground='#065f46')
        tree.tag_configure('row_REVIEW_REQUIRED', background='#ffedd5', foreground='#9a3412')
        tree.tag_configure('row_ERROR', background='#fee2e2', foreground='#991b1b')
        tree.tag_configure('row_WAIT_SCAN', background='#e0f2fe', foreground='#075985')
        # ★ 다크모드 대응
        try:
            from theme_aware import ThemeAware
            if ThemeAware.is_dark():
                tree.tag_configure('row_FINALIZED', background='#064e3b', foreground='#6ee7b7')
                tree.tag_configure('row_REVIEW_REQUIRED', background='#7c2d12', foreground='#fed7aa')
                tree.tag_configure('row_ERROR', background='#7f1d1d', foreground='#fca5a5')
                tree.tag_configure('row_WAIT_SCAN', background='#0c4a6e', foreground='#bae6fd')
    ...
    def _build_tab4(self):
        ...
        for var, label, color in [
            ...
            tk.Label(card, text=label, bg=color, fg='#e0e0e0',
                     font=('맑은 고딕', 9)).pack()
    ...
    def _show_audit_log_viewer(self):
        ...
        vsb = tk.Scrollbar(tree, orient=VERTICAL, command=tree.yview)

...
# ───────────────────────────────────────────────────────────
# [P4] 로케이션 코드 4파트 확장 (A-01-01-01)
# 파일: gui_app_modular/utils/tonbag_location_uploader.py
# ───────────────────────────────────────────────────────────
PATCHES.append({
    'id': 'P4',
    'desc': '로케이션 코드 4파트 확장 (구역-열-층-칸)',
    'file': 'gui_app_modular/utils/tonbag_location_uploader.py',
    'old': """\
    # 기본 형식 체크 (A-1-3 형식)
    ...
    return True, "OK\"""",
    'new': """\
    # v5.9.8 P4: 3파트(A-01-03) 또는 4파트(A-01-03-01) 모두 허용
    parts = location.split('-')
    if len(parts) not in (3, 4):
        return False, "형식이 올바르지 않습니다 (예: A-01-03 또는 A-01-03-01)"
    ...
    for i, part in enumerate(parts[1:]):
        if not part.isdigit():
            return False, f"{labels[i]}은 숫자여야 합니다 (예: 01)"

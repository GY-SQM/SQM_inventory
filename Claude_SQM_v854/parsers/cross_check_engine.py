...
class CrossCheckEngine:
    ...
    def _check_bl_no(self, result, invoice, pl, bl, do):
        ...
        for name, obj, attr in [("Invoice", invoice, "bl_no"), ("B/L", bl, "bl_no"), ("D/O", do, "bl_no")]:
            val = _safe_str(obj, attr)
            if val:
                # v9.0: Invoice bl_no가 SAP No 형식(22xxxxxxxxxx, 10자리)이면 비교 제외
                # — Gemini가 SAP No를 BL No로 오인식하는 경우 방지
                if name == "Invoice" and val.startswith("22") and len(val) == 10:
                    logger.debug(f"[CrossCheck] Invoice bl_no='{val}' → SAP No 형식 → bl_no 비교 제외")
                    continue
                sources[name] = val
    ...
    def _check_vessel(self, result, invoice, pl, bl, do):
        ...
        # v8.4.5 Rev2: 환적 자동 인식
        # BL/PL/Invoice 선박은 동일하고 D/O 선박만 다른 경우 → 환적으로 판단 → INFO
        # 그 외(BL vs PL 등 선적 서류끼리 불일치) → WARNING 유지
        do_vessel = sources.get("D/O", "")
        non_do_vessels = {k: v for k, v in sources.items() if k != "D/O"}
        is_transshipment = False

        if do_vessel and non_do_vessels:
            # 선적 서류(BL/PL/Invoice)끼리는 일치하는지 확인
            non_do_vals = list(non_do_vessels.values())
            non_do_match = all(
                _vessel_fuzzy_match(non_do_vals[0], v) for v in non_do_vals[1:]
            ) if len(non_do_vals) > 1 else True

            # D/O 선박만 다르고 선적 서류끼리 일치하면 환적
            if non_do_match and not _vessel_fuzzy_match(non_do_vals[0], do_vessel):
                is_transshipment = True

        if is_transshipment:
            # 환적: 팝업 없이 INFO 로그만
            bl_vessel  = non_do_vals[0] if non_do_vals else ""
            logger.info(
            ...
            # 환적 아님(선적 서류끼리 불일치 등) → WARNING 유지
            result.add("Vessel", CheckLevel.WARNING,
                       f"선박명 불일치: {', '.join(mismatches)}", sources)

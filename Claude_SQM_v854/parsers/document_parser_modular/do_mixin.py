...
class DOMixin:
    ...
    def _parse_do_maersk_coord(self, pdf_path: str) -> "Optional[DOData]":
        ...
        for i, ct_no in enumerate(containers):
            ...
            ci.free_time     = ft_info.get("free_time", "")
            ci.return_yard   = ft_info.get("return_yard", "")
            if hasattr(result, "containers"):
                result.containers.append(ci)

        result.success = bool(result.do_no and result.bl_no)
        logger.info(
            f"[DO-MAERSK] 좌표 파싱 완료: do={do_no} bl={bl_no} "
    ...
    def _parse_do_msc_coord(self, pdf_path: str) -> "Optional[DOData]":
        ...
        # Free time (반납기한): 컨테이너와 동일 날짜 적용
        ft_m = _re.search(r'반납기한.{0,30}?(\d{4}-\d{1,2}-\d{1,2})', all_text, _re.DOTALL)
        ...
        logger.info(
            f"[DO-MSC] 좌표 파싱 v8.2.4: bl={bl_no} do={getattr(result,'do_no','')} "
    ...
    def _extract_do_by_carrier_rule(self, pdf_path: str,
        ...
        logger.info(
            f"[DO-RULE] carrier_rule 파싱 완료: carrier={carrier_id} "
    ...
    def _parse_do_gemini(self, pdf_path: str, result: DOData) -> DOData:
        ...
        try:
            # v8.0.0 [PDF-DIRECT]: 이미지 변환 제거 → PDF 직접 전송
            client = genai.Client(api_key=self.gemini_api_key)

            prompt = """
이 D/O(Delivery Order/화물인도지시서) 문서에서 다음 정보를 추출해주세요:
...
"""
            ...
            # _call_gemini_smart를 직접 사용하기 위해 base 인스턴스 활용
            _smart_result = None
            try:
                from parsers.document_parser_modular.base import DocumentParserBase as _Base
                _base_inst = _Base.__new__(_Base)

                # gemini_parser 역할을 하는 임시 객체 생성
                class _GProxy:
                    def __init__(self, api_key, model):
                        self.gemini_api_key = api_key
                        self.model_name = model
                    def _call_gemini(self, p, b=None, mime_type="image/png"):
                        _client = genai.Client(api_key=self.gemini_api_key)
                        import google.genai.types as _t
                        _contents = []
                        if b:
                            _contents.append(_t.Part.from_bytes(data=b, mime_type=mime_type))
                        _contents.append(p)
                        _r = _client.models.generate_content(
                            model=self.model_name, contents=_contents)
                        return _r.text or ''
                    def _call_gemini_pdf(self, p, pdf_path):
                        with open(pdf_path, 'rb') as _f:
                            _b = _f.read()
                        return self._call_gemini(p, _b, mime_type="application/pdf")

                _proxy = _GProxy(self.gemini_api_key,
                                 getattr(self, 'model_name', 'gemini-2.5-flash'))
                _smart_result = _base_inst._call_gemini_smart(prompt, pdf_path, _proxy)
            except Exception as _se:
                logger.debug(f"[DO] SMART-PARSE 실패, 직접 방식 폴백: {_se}")

            if _smart_result:
                response_text = _smart_result
            else:
                # 직접 폴백

    ...
    try:
        with get_db() as db:
            ...
            rows = db.fetchall(
                ...
                f"COALESCE(m.tonbag_no,'') AS tonbag_no, "
                f"COALESCE(m.from_location,'') AS from_location, "
                f"COALESCE(m.to_location,'') AS to_location, "
                f"COALESCE(m.source_type,'') AS source_type, "
                f"COALESCE(m.operator,'') AS operator, "
                f"COALESCE(m.remark,'') AS remark, "
                f"COALESCE(m.status,'') AS status, "
                f"COALESCE(m.approver,'') AS approver, "
                f"m.move_date, m.created_at "

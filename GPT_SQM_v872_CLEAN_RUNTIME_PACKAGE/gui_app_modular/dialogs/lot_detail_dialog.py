...
class LotDetailDialogMixin:
    ...
    def _show_lot_detail_popup(self, lot_no: str, source_tab: str = None) -> None:
        ...
        lot_info, tonbags, movements = self._load_lot_detail_data(lot_no, source_tab)
        popup, bg, fg, card_bg = self._create_lot_detail_window(lot_no, lot_info)
        avail_cnt = self._build_lot_detail_content(popup, lot_no, lot_info, tonbags, movements, bg, fg, card_bg)
        self._build_lot_detail_buttons(popup, lot_no, bg, avail_cnt)

    def _load_lot_detail_data(self, lot_no: str, source_tab: str = None):
        """LOT 상세 데이터 로드 — LOT 정보, 톤백, 이력."""
        ...
        return lot_info, tonbags, movements

    def _create_lot_detail_window(self, lot_no, lot_info):
        """LOT 상세 팝업 윈도우 생성 + 테마."""
        from ..utils.constants import LEFT, X, tk
        ...
        return popup, bg, fg, card_bg

    def _build_lot_detail_content(self, popup, lot_no, lot_info, tonbags, movements, bg, fg, card_bg):
        """LOT 상세 팝업 탭 콘텐츠 구성 (톤백 + 이력 + allocation)."""
    ...
    def _build_lot_detail_buttons(self, popup, lot_no, bg, avail_cnt):
        """LOT 상세 팝업 하단 버튼."""
        from ..utils.constants import LEFT, RIGHT, X, tk, ttk

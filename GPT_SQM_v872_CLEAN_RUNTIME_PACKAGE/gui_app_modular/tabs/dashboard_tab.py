...
class DashboardTabMixin:
    ...
    def _setup_dashboard_tab(self) -> None:
        ...
        3구역: 제품별 현황 테이블 (하단)
        """
        self._setup_dashboard_theme()
        self._setup_dashboard_zone1_matrix()
        self._setup_dashboard_zone2_alerts()
        self._setup_dashboard_footer()
        self._refresh_dashboard()

    def _setup_dashboard_theme(self) -> None:
        """대시보드 탭 테마/색상/컨테이너 초기화."""
        ...
        self._dash_main_container = mc

    def _setup_dashboard_zone1_matrix(self) -> None:
        """대시보드 1구역: 제품×상태 통합 매트릭스."""
        from ..utils.constants import BOTH, LEFT, RIGHT, YES, X, Y, tk, ttk
        ...
        product_frame.columnconfigure(0, weight=9, minsize=360)
        product_frame.columnconfigure(1, weight=11, minsize=300)
        ...
        for cid, text, w in [
            ("product", "Product", 108), ("available", "판매가능", 68),
            ("reserved", "판매배정", 68), ("picked", "판매화물", 68),
            ("outbound", "출고완료", 68), ("return", "반품대기", 62),
            ("total", "합계", 68), ("sample", "샘플", 46),
        ]:
            self.tree_dashboard_product.heading(cid, text=text, anchor='center')
            # 숫자·상태 열은 고정 폭 — 남는 폭은 Product만 흡수 (좌측 과확장 완화)
            self.tree_dashboard_product.column(
                cid, width=w, anchor='center', stretch=(cid == "product"),
            )
        ...
        for cid, text, w in [
            ("bal_open", "기초재고", 92), ("bal_in", "입고", 76),
            ("bal_out", "출고", 76), ("bal_close", "기말재고", 92),
            ("bal_check", "검증", 58),
        ]:
            self._tree_balance.heading(cid, text=text, anchor='center')
            self._tree_balance.column(cid, width=w, anchor='center', stretch=True)
    ...
    def _setup_dashboard_zone2_alerts(self) -> None:
        """대시보드 2구역: 알림 패널."""
        from ..utils.constants import BOTH, LEFT, RIGHT, YES, X, Y, tk
    ...
    def _setup_dashboard_footer(self) -> None:
        """대시보드 하단: 자동새로고침 + 상태 표시."""
        from ..utils.constants import BOTH, LEFT, RIGHT, X, tk
        ...
        zone3_placeholder.pack(fill=BOTH, expand=True)

...
class OneStopInboundDialog(InboundUploadMixin, InboundDialogBase):
    ...
    def __init__(self, parent, engine, log_fn=None, app=None):
        ...
        self.btn_excel = None
        self.btn_review = None  # Phase B: 검수센터
        self.btn_undo = None
        self.btn_redo = None
...
self.btn_review.pack(side=LEFT, padx=(0, 5))
self._attach_doc_tooltip(self.btn_review,
    "SUSPECT/불일치 데이터를 사람이 직접 보정합니다\n\n• 원본 PDF 보기\n• 드래그(ROI) 또는 캡처로 텍스트 추출\n• 정답 입력 후 즉시 반영 + QC 재검증")
...
# ==========================
# Phase B: 검수센터 (Review Center)
# ==========================
def _open_review_center(self):
    """SUSPECT/불일치 값을 사람이 직접 보정하는 검수센터를 연다."""
    try:
        from .review_center import ReviewCenterDialog
    except Exception as e:
        msgbox.showerror("검수센터 로드 실패", f"review_center 모듈을 불러올 수 없습니다.\n\n{e}")
        return

    # 문서 경로: 사용자가 선택한 파일들
    doc_paths = dict(self.file_paths or {})
    # 미리보기 데이터 참조
    preview_rows = getattr(self, 'preview_data', []) or []
    if not preview_rows:
    ...
    dlg = ReviewCenterDialog(
        ...
        engine=self.engine,
        doc_paths=doc_paths,
        preview_data_ref=self.preview_data,   # 리스트 레퍼런스(직접 수정)
        refresh_preview_cb=getattr(self, '_refresh_preview_table', None),
        rerun_qc_cb=getattr(self, '_run_gate_validation', None),
    )
    dlg.show()

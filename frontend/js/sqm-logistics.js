/* SQM Inventory v8.7.0 — sqm-logistics.js (Logistics — 입고·출고·반품·이동·로그·스캔) */
(function () {
  'use strict';
  ...
  window._outboundPeriod   = window._outboundPeriod   || 'all'; // v870: 기간 필터

  /* v870: SOLD 그룹Â·기간 칩 버튼 헬퍼 */
  // v870: 클릭 시 DOM 즉시 하이라이트 업데이트
  ...
  /* ── SOLD 날짜 헬퍼 ── */
  // v8.7.0: toISOString() UTC 버그 수정 — 로컨 날짜 헬퍼
  ...
  function _soldTodayStr() {
    // v870: Intl 기반 KST 날짜 — PyWebView UTC 버그 우회
    try {
  ...
  function loadOutboundPage() {
    ...
    // v870: 날짜 파라미터 — _outboundDateFrom/To 기준 (period & 그룹 독립)
    ...
    apiGet('/api/q/sold-list?limit=2000' + _sd + _ed).then(function(res){
      ...
      // v870: 그룹 모드 분기 — container/bl/customer/date
  ...
  window.showReturnActionMenu = function(btn) {
    ...
    window._openContextMenu(btn, [
      ...
      '-',
      { icon:'✅', label:'검사완료 → AVAILABLE', kbd:'A', color:'#22c55e', fn:async function(){
          if(!(await window.sqmConfirmAsync('['+lot+'] 검사 완료 — AVAILABLE로 전환하시겠습니까?'))) return;
          window.returnToAvailable(lot, true);
        }
      },
    ]);
  };
  window.returnToAvailable = async function(lotNo, skipConfirm) {
    if (!skipConfirm && !(await window.sqmConfirmAsync('LOT ' + lotNo + ' 검사완료 처리합니다.\nAVAILABLE로 전환하시겠습니까?'))) return;

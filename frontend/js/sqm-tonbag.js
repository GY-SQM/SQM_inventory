...
(function () {
  ...
  /* ===================================================
     ...
     =================================================== */
  /* ===================================================================
     [REMOVED 20260604] OneStop 출고 중복 블록(구 435~1320) 제거.
     정식 구현은 sqm-inline.js 단일본. 본 모듈의 onestop 전역(_ooState, oo 핸들러,
     showOneStopOutboundModal)은 로드 순서상 항상 sqm-inline.js 버전이 전역을 차지했고
     (死코드), data-action 라우터도 v8.7.0 에서 비활성화됨. dispatchAction 의
     quick-outbound 분기는 전역 window.showOneStopOutboundModal(=inline)으로 폴백.
     ===================================================================== */
        ...
        fetch(url, {
          ...
          .then(function(res) {
            if (res.ok === false || res.detail) {
              if (window.showToast) window.showToast('error', label + ' 실패: ' + (res.detail || res.message || JSON.stringify(res)));
            } else {
              if (window.showToast) window.showToast('success', label + ' 완료 ' + (res.message || ''));
              window._bmaRefresh();
            }
          })
          .catch(function(e) { if (window.showToast) window.showToast('error', label + ' 오류: ' + String(e)); });
      };
  ...
  async function dispatchAction(action) {
    ...
    if (conf.m === 'JS') {
      ...
      if (conf.u === 'tonbag-location-upload') {
        /* v8.7.0: 신규 위치 매핑 워크플로우 모달 (좌측 미배정 톤백 + 우측 실시간 매핑) */
  ...
  /* ===================================================
     ...
     =================================================== */
  function bindAll() {
    // v8.7.0 fix (2026-05-14): data-action 바인딩 비활성화
    //   - 원본은 v8.7.0 era 옛 ENDPOINTS 사용 (line 4001) → onWarehouseDashboard 등 신규 매핑 누락
    //   - ev.stopPropagation() 이 sqm-inline.js 의 신규 라우터까지 차단함
    //   - 신규 라우터(sqm-inline.js)가 모든 data-action 을 처리하도록 위임
    /* DISABLED v8.7.0 router (kept for reference, do not re-enable)
    ...
    */
    ...
    console.info('[SQM v8.7.0] bindAll complete');
  }

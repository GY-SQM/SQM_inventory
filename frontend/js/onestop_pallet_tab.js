...
(function () {
  ...
  function buildSummaryCard(rows) {
    ...
    /* 창고 가용률 — 광양 GY Logis 6,572셀 기준 (v8.7.0 랙 이름 규칙 확정)
  ...
  /* 외부(파싱 완료 시점)에서 호출 — packing_type 초기화 + 자동 적용
     v8.7.0 Hybrid: 사용자가 사전 결정한 defaultPacking 우선 적용 */
  ...
  /* ─────────────────────────────────────────────────────────────────────
    랙 위치(Cell) 검증 — v8.7.0 광양 GY Logis 규칙 확정
    형식: G{동}-{칸}-{열}-{층}
      동:   5 | 6
      칸:   01~16 (랙 번호)
      열:   01~31

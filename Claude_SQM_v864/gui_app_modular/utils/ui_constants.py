...
def apply_tooltip(widget, text: str, delay: int = 250):
    ...
    ttkbootstrap 미설치 시 tk Toplevel 툴팁으로 대체.
    ...
    def _bind_tk_tooltip(w, tip: str, ms: int):
        ...
        def show():
            ...
            try:
                ...
                logger.debug("tooltip position: %s", ex)
                return
            ...
            tk.Label(
                ...
                relief="solid",
                borderwidth=1,
                font=("맑은 고딕", 6),  # v8.4.5: 툴팁 글씨 30% 축소 (9→6)
                padx=8,
                pady=4,
                wraplength=320,
            ).pack()
...
def make_tab_header(parent, title: str, status_color: str = '#3b82f6',
                    count_var=None, is_dark: bool = None,
                    inner_bg: str = None, outer_border: str = None):
    ...
    Args:
        ...
        inner_bg:     None → tc(bg_secondary); 대시보드 등 톤 맞춤용 오버라이드
        outer_border: None → 팔레트 border; 헤더 외곽선 색 오버라이드
    ...
    bg     = inner_bg if inner_bg is not None else tc('bg_secondary', _dark)
    fg     = tc('text_primary',  _dark)
    border = outer_border if outer_border is not None else _palette.get(
        'border', '#334155' if _dark else '#e2e8f0')

    outer = tk.Frame(parent, bg=border, height=Spacing.Tab.HEADER_H)
    outer.pack(fill='x', padx=0, pady=Spacing.Tab.HEADER_PADY)

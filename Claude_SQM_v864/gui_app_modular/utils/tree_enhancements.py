...
class HeaderFilterBar:
    ...
    def __init__(self, parent, tree, filter_columns: List[Tuple[str, str, int]],
        ...
        if date_from_var is not None and date_to_var is not None:
            _lbl_period = ttk.Label(self.frame, text="조회 기간 ", font=('맑은 고딕', 9))
...
class TreeviewTotalFooter:
    ...
    def __init__(self, parent, tree, summable_column_ids: List[str],
                 ...
                 skip_item_tags: Optional[Tuple[str, ...]] = None):
        """
        Args:
            ...
            bar_bg, accent_color, border_color: None이면 테마 기본 (대시보드 등에서 톤 오버라이드)
            skip_item_tags: 합계에서 제외할 행 태그 (예: 합계 행)
        """
        import tkinter as tk
        ...
        self._skip_item_tags: FrozenSet[str] = frozenset(skip_item_tags or ())

        _d = is_dark()
        # v8.1.8: 배경을 약간 다르게 — footer 영역 시각적 분리
        _bg     = bar_bg if bar_bg is not None else ThemeColors.get('statusbar_bg', _d)
        _accent = accent_color if accent_color is not None else (
            '#00E5A0' if _d else '#0369a1')
        _border = border_color if border_color is not None else _accent
    ...
    def update_totals(self) -> None:
        ...
        for item_id in children:
            try:
                if self._skip_item_tags:
                    _tags = self.tree.item(item_id, 'tags') or ()
                    if self._skip_item_tags.intersection(_tags):
                        continue
                counted += 1

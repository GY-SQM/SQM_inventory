...
class AllocationDialog:
    ...
    def _exclude_duplicate_lots_for_integrity(self, rows: list) -> dict:
        ...
        v8.6.0 수정: LOT당 본제품 1행 + 샘플 1행(QTY < 0.01 MT) = 2행은
        Allocation 정상 구조이므로 중복으로 판정하지 않는다.
        본제품 행(QTY >= 0.01)이 2개 이상인 LOT만 진짜 중복으로 제외한다.
        """
        if not rows:
            return {"filtered_rows": [], "excluded_rows": 0, "excluded_lots": []}

        # LOT별 본제품 행 수 카운트 (샘플 행은 중복 판정에서 제외)
        from collections import defaultdict
        lot_main_count = defaultdict(int)  # LOT별 본제품(QTY >= 0.01) 행 수
        for r in rows:
            ...
            if qty >= 0.01:  # 본제품 행만 카운트
                lot_main_count[lot] += 1
    ...
    def _show_upload_summary_dialog(self, dup: dict, shortage_warnings: list = None) -> bool:
        ...
        def _cell(parent, text, row, col, *, bg, fg, bold=False):
            tk.Label(
                ...
                bg=bg, fg=fg,
                font=("맑은 고딕", 9, "bold" if bold else "normal"),
    ...
    def _build_reserve_shortage_warnings(self) -> list:
        ...
        for lot_no, need_count in by_lot.items():
            # ★ v8.1.5 [MXBG-FIX]: 총 cargo (MXBG 기준, 샘플 제외)
            ...
            # v8.6.0: 샘플도 배정 대상이므로 최대 = MXBG + 샘플1개
            _max_tb = total_tb + 1  # SQM 대원칙: 1 LOT = 톤백N + 샘플1
            if total_tb > 0 and (already_tb + need_count) > _max_tb:
                remain_tb = max(0, _max_tb - already_tb)
                warnings.append(
                    f"{lot_no}: 이미배정 {already_tb}개 + 요청 {need_count}개 > MXBG {_max_tb}개 "

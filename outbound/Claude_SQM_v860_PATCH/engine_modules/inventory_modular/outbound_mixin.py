        ...
        try:
            ...
            for _r in needs_avail:
                ...
                _sold_cnt_fix = self.db.fetchone(
                    ...
                    "WHERE lot_no=? AND status IN ('SOLD','OUTBOUND')", (_lot,)
                )
    ...
    def reserve_from_allocation(self, allocation_rows: list, source_file: str = '', reservation_mode: str = '') -> Dict:
        ...
        try:
            with self.db.transaction("IMMEDIATE"):
                ...
                # v8.1.5 [G5-MXBG]: 배치 내 동일 LOT 합산 + 기존 RESERVED 누적 MXBG 초과 차단
                ...
                for _bln, _bqt_sum in _batch_lot_qty.items():
                    ...
                    # v8.1.5 [G5-MXBG-FIX]: 총 cargo (MXBG 기준, 샘플 제외)
                    _btotal_row = self.db.fetchone(
                    ...
                    _btotal_kg = float(
                        (_btotal_row.get('total_kg') if isinstance(_btotal_row, dict) else _btotal_row[0])
                        if _btotal_row else 0
                    )
                    # v8.1.5 [G5-MXBG-FIX]: 기존 RESERVED 톤백 수 (COUNT 기반, LOT/TONBAG 모드 공용)
                    _balready_row = self.db.fetchone(
                        "SELECT COALESCE(COUNT(*),0) * 500 AS already_kg "
                        "FROM allocation_plan "
                        "WHERE lot_no=? AND status='RESERVED' AND qty_mt >= 0.01",
                        (_bln,)
                    )
                    _balready_kg = float(
                        (_balready_row.get('already_kg') if isinstance(_balready_row, dict) else _balready_row[0])
                        if _balready_row else 0
                    )
                    # 검증: 기존예약 + 이번배치 > MXBG 총량
                    # v8.6.0: 샘플도 배정 대상 → 최대 = MXBG총량 + 샘플1개(1kg)
                    _b_max_kg = _btotal_kg + 1.0  # 샘플 1kg 허용
                    if _btotal_kg > 0 and (_balready_kg + _bqt_kg) > _b_max_kg + 0.5:
                        _bremain_kg = max(0.0, _b_max_kg - _balready_kg)
                        _g5_msg = (
                            f"[G5-MXBG-EXCEED] {_bln}: "
                            f"기존예약 {_balready_kg:.0f}kg + 이번배치 {_bqt_kg:.0f}kg"
                            f" > MXBG총량 {_b_max_kg:.0f}kg"
                ...
                # ── v7.7.1 [PRE-DUP]: 업로드 전 기존 예약 LOT 사전 중복 감지 ──────────
                # 동일 sale_ref로 이미 RESERVED된 LOT과 이번 배치 LOT을 미리 교차 확인
                # → LOT_MODE_DUP 도달 전에 사용자에게 경고 정보 제공 (처리는 계속)
                _pre_dup_warnings = []
                try:
                    # 이번 배치의 sale_ref 추출 (대표값)
                    _batch_sale_refs = set()
                    for _ba in (allocation_rows or []):
                        _bsr = str(_alloc_val(_ba, 'sale_ref') or '').strip()
                        _bqt = float(_alloc_val(_ba, 'qty_mt') or 0)
                        if _bsr and _bqt >= 0.01:  # 샘플 행 제외
                    ...
                    for _bsr in _batch_sale_refs:
                        ...
                        if _already_lots:
                            _batch_main_lots = {
                                (normalize_lot(_alloc_val(_ba, 'lot_no')) or '').strip()
                                for _ba in (allocation_rows or [])
                                if float(_alloc_val(_ba, 'qty_mt') or 0) >= 0.01
                            }
...
# [v6.8.6 top-level import로 이동]                         from engine_modules.constants import normalize_customer
                    ...
                    # v7.7.1 [정책 명확화]: qty 기반 자동 스킵 금지
                    # SQM은 정수 단위 운영. 샘플(0.001MT=1kg)도 allocation에 포함되며
                    ...
                    # ── Gate A: 입력 유효성 (AL-09 ZERO_QTY / 음수 / 고객 누락) ── ──────
                    ...
                    if not customer:
                        ...
                        continue
                    # AL-FIX-4: sale_ref 공란 → 경고만 (v8.6.0: 필수→권장으로 완화)
                    # Allocation 파일에 SALE REF 컬럼이 없을 수 있으므로 차단하지 않음
                    if not sale_ref:
                        logger.warning(
                            f"[INVALID_SALE_REF] LOT {lot_no}: sale_ref가 비어 있음 "
                            f"(판매참조번호 지정 필수)"
                        )
                        # continue 제거 — sale_ref 없어도 예약 진행
                    # ─────────────────────────────────────────────────────────────────
                    ...
                    # ── Gate B: sale_ref 중복 (AL-06 SALE_REF_CONFLICT) ──────────
                    # 동일 sale_ref가 RESERVED/PENDING_APPROVAL 상태면 즉시 차단
                    ...
                    # v8.1.5 [G2-MXBG-FIX]: 총 cargo (MXBG 기준, 샘플 제외)
                    if qty_mt > 0:
                        ...
                        # v8.1.5 [G2-MXBG-FIX]: 기존 RESERVED 톤백 수 (COUNT 기반, LOT/TONBAG 모드 공용)
                        ...
                        # v8.6.0: 샘플도 배정 대상 → 최대 = MXBG총량 + 샘플1개(1kg)
                        _g2_max_kg = _g2_total_kg + 1.0  # 샘플 1kg 허용
                        if _g2_total_kg > 0 and (_g2_already_kg + _g2_req_kg) > _g2_max_kg + 0.5:
                            _g2_remain_kg = max(0.0, _g2_max_kg - _g2_already_kg)
                            _g2_msg = (
                                f"[G2-MXBG-EXCEED] {lot_no}: "
                                f"기존예약 {_g2_already_kg:.0f}kg + 이번요청 {_g2_req_kg:.0f}kg"
                                f" > MXBG총량 {_g2_max_kg:.0f}kg"
...
# [v6.8.6 top-level import로 이동]                     from engine_modules.constants import STATUS_PICKED, STATUS_RESERVED, STATUS_SOLD, get_tonbag_unit_weight
                    ...
                    # v8.6.0: 샘플 요청은 전용 S00 행에서 배정하므로 일반 톤백 STAGED와 무관
                    ...
                    if reserved_in_lot > 0:
                        ...
                        # v8.3.0 [Phase 9]: RESERVED audit_log
                        try:
        ...
        # ══ [Phase 3] 결과 집계 + import_batch 통계 업데이트 ═════════
    ...
    def confirm_outbound(self, lot_no: str = None, force_all: bool = False) -> Dict:
        ...
        try:
            ...
            with self.db.transaction("IMMEDIATE"):
                ...
                for tb in tonbags:
                    ...
                    try:
                        ...
                        picking_id = None
                    try:
                        # v8.5.7: sold_table INSERT 보강 — 출고 보고서 생성에 필요한 전체 필드
                        # inventory에서 sap_no, bl_no, product_code, gross_weight 조회
                        ...
                        # picking_table에서 sales_order_no, picking_no 조회
                        ...
                        # allocation_plan에서 customer, sale_ref 조회 (fallback)
                        ...
                        # GW 계산: LOT 전체 GW / 톤백수 (샘플은 별도)
                        _is_sample = tb.get('is_sample', 0) or (1 if tb.get('sub_lt', -1) == 0 else 0)
                        _tb_gw_kg = 0.0
                        if _is_sample:
                            _tb_gw_kg = (tb.get('weight') or 0) * 1.025  # 샘플 GW ≈ NW × 1.025
                        ...
                        self.db.execute(
                            ...
                            (tb['lot_no'], tb_id, tb.get('sub_lt', 0), uid_val, picking_id,
                             ...
                             now[:10],  # delivery_date = 출고일
                             1 if not _is_sample else 1,  # ct_plt = 톤백 1개당 1
            ...
            # v8.0.2 [P2]: 출고 확정 후 touched_lots 전체 재계산
    ...
    def gate1_verify_picking(
        ...
        try:
            ...
            for lot_no in sorted(matched):
                ...
                if _is_lot_mode:
                    # LOT 모드: COUNT 기반 (LOT/TONBAG 모드 공용, qty_mt 단위 차이 무관)
                    db_row = self.db.fetchone(
                        """SELECT COUNT(*) AS plan_count,
                                  COALESCE(COUNT(CASE WHEN qty_mt >= 0.01 THEN 1 END), 0) * 500 AS total_kg
                           FROM allocation_plan

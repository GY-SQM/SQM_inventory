...
export default function InboundModal({ open, onClose, mode = 'pdf' }) {
  // mode='pdf' → PDF 업로드부터, mode='excel' → 바로 수동 입력 폼
  const [step,        setStep]        = useState(mode === 'excel' ? 'preview' : 'upload');  // upload|preview|form|result
  ...
  // mode 변경 시 step 리셋
  useEffect(() => {
    if (open) setStep(mode === 'excel' ? 'preview' : 'upload');
  }, [open, mode]);
  ...
  const handleParsePreview = async () => {
    ...
    // ★ P2-11: 스마트 파싱 시도 (실패 시 기존 방식 폴백 — R7 보장)
    try {
      const smartFd = new FormData();
      const firstFile = files.bl || files.pl || files.fa;
      if (firstFile) {
        smartFd.append('file', firstFile);
        smartFd.append('doc_type', files.bl ? 'BL' : files.pl ? 'PL' : 'FA');
        const smartRes = await fetch('/api/smart-inbound/parse', { method: 'POST', body: smartFd });
        const smartData = await smartRes.json();
        if (smartData.success && smartData.confidence?.overall >= 70) {
          // 스마트 파싱 성공 → 결과 사용
          const pd = smartData.data || {};
          setForm({
            lot_no: pd.lot_no || '', product_name: pd.product || '',
            bl_no: pd.bl_no || '', sap_no: pd.sap_no || '',
            total_weight_kg: pd.net_weight || '', bag_count: pd.tonbag_count || '',
            location: '', container_no: pd.container_no || '',
            invoice_no: pd.invoice_no || '', ship_date: pd.ship_date || '',
            arrival_date: pd.arrival_date || '', warehouse: pd.warehouse || '',
          });
          setPreview({ ...smartData, data: pd, smart: true });
          setStep('preview');
          setLoading(false);
          return;
        }
      ...
      // 스마트 파싱 실패 → 기존 방식으로 폴백 (R7)
      console.log('Smart parse fallback:', e.message);
    }
  ...
  return (
    <Modal open={open} onClose={handleClose} title={mode === 'excel' ? '📊 엑셀/수동 입고' : '📥 PDF 입고 (원스톱)'} width={700}>

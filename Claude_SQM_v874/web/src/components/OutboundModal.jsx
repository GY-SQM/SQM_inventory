...
export default function OutboundModal({ open, onClose, mode = 'form' }) {
  // mode='form' → 기존 개별 입력, mode='paste' → 붙여넣기 일괄 입력
  ...
  const parsePaste = () => {
    ...
    const parsed = lines.map(line => {
      ...
      return { lot_no: parts[0] || '', sub_lt: parts[1] || '1', qty_kg: parts[2] || '' };
    }).filter(p => p.lot_no);
  ...
  return (
    <Modal open={open} onClose={handleClose} title={mode === 'paste' ? '📤 빠른 출고 (붙여넣기)' : '🚀 즉시 출고 (원스톱)'} width={700}>
      {!result ? (
        <div>
          {/* 고객 정보 */}
          ...
          {mode === 'paste' && (
            <div style={{ marginBottom: 16 }}>
              <span style={label}>LOT_NO, Sub_LT, 수량(kg)  — 탭/콤마 구분, 행마다 1건</span>
              <textarea
                style={{ ...input, height: 100, fontFamily: 'monospace', fontSize: 12 }}
                placeholder={'LOT001\t1\t1000\nLOT002\t2\t500'}
                value={pasteText}
                onChange={(e) => setPasteText(e.target.value)}
              />
              <button style={{ ...btn('#3b82f6'), padding: '6px 14px', fontSize: 12, marginTop: 6 }} onClick={parsePaste}>
                📋 파싱 적용
              </button>
            </div>
          )}
          ...
          {/* 톤백 선택 */}
          <h4 style={{ fontSize: 13, marginBottom: 8 }}>출고 톤백 {items.length > 1 && `(${items.length}건)`}</h4>
          {items.map((item, idx) => (

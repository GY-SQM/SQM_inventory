...
const tdBase = { padding: '5px 6px', borderBottom: '1px dashed rgba(51,65,85,0.3)', fontSize: 12, whiteSpace: 'nowrap' };
...
export default function InventoryPage({ onLotClick }) {
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    try {
      const saved = localStorage.getItem('sqm_inventory_visible_cols');
  ...
  return (
    <div style={{ padding: 16, background: '#0f172a', minHeight: '100vh', color: '#f1f5f9' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', padding: '6px 10px',
        background: '#1e293b', border: '1px solid #334155', borderRadius: 6, marginBottom: 10, fontSize: 11, alignItems: 'center' }}>
        ...
        {COLUMN_DEFS.map(col => (
          <label key={col.key} style={{ display: 'flex', alignItems: 'center', gap: 2, cursor: 'pointer', color: '#64748b' }}>
            <input type="checkbox" checked={!!visibleCols[col.key]}
              onChange={() => setVisibleCols(v => {
                const next = { ...v, [col.key]: !v[col.key] };
                try { localStorage.setItem('sqm_inventory_visible_cols', JSON.stringify(next)); } catch { /* ignore */ }
                return next;
              })}
        ...
        <button onClick={() => {
          const defaults = Object.fromEntries(COLUMN_DEFS.map(c => [c.key, c.defaultVisible]));
          setVisibleCols(defaults);
          try { localStorage.removeItem('sqm_inventory_visible_cols'); } catch { /* ignore */ }
        }} style={{

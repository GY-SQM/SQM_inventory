...
async def picking_list_parse(file: UploadFile = File(...)):
    ...
    while True:
        ...
        if total_size > MAX_SIZE:
            raise HTTPException(413, f"파일 크기 50MB 초과 ({total_size // (1024*1024)}MB).")
        chunks.append(chunk)
...
def monthly_pdf_download(background_tasks: BackgroundTasks, payload: Optional[dict] = None):

...
HELP_TEXT = """📋 <b>SQM Bot v3 Commands</b>

<b>[Query]</b>
/stock         Inventory summary
/outbound      Today's outbound
/outbound 7    Last 7 days outbound
/pending       RESERVED/PICKED status
/lot LOT_NO    LOT detail
/expiry        Con Return within 7 days
/compare       Today's stock changes
/integrity     Integrity check
/backup        Instant backup
/status        Server + DB status

<b>[Execute — 2-step confirm]</b>
/confirm LOT_NO      Confirm outbound
/cancel LOT_NO NO    Cancel outbound
/unreserve LOT_NO    Cancel reservation
/yes                 Execute confirmed
/abort               Abort pending cmd
...
<b>[Korean aliases still work]</b>
/재고 /출고 /대기 /만료 /비교 /상태""".strip()


# ── 선택지 대기 상태 ──────────────────────────────────────────
_choice_pending: dict = {}
CHOICE_TIMEOUT = 60


def cmd_go() -> str:
    """계속 진행 명령 — 현재 대기 중인 작업을 자동 진행"""
    if CHAT_ID in _pending:
        return cmd_확인_2단계()
    return "✅ No pending task. Bot is running normally."


def cmd_select(num: str) -> str:
    """1/2/3 선택지 응답"""
    cp = _choice_pending.get(CHAT_ID)
    if not cp:
        return f"No pending selection. You chose: {num}"
    if time.time() > cp.get("expires", 0):
        _choice_pending.pop(CHAT_ID, None)
        return "⏰ Selection timed out. Please try again."
    ...
    _choice_pending.pop(CHAT_ID, None)
    return f"✅ Selected: <b>{selected}</b>"


def send_choice(question: str, options: list):
    """프로그램에서 사용자에게 선택지를 보낼 때 호출"""
    _choice_pending[CHAT_ID] = {
        "question": question,
        "options": options,
    ...
    lines.append(f"\nReply with 1~{len(options)} (60s timeout)")
    send("\n".join(lines))
...
def route(text: str) -> str:
    ...
    arg2  = parts[2] if len(parts) > 2 else '0'
    ...
    if cmd in ('y', 'yes'):
        if CHAT_ID in _pending:
            return cmd_확인_2단계()
        return "✅ OK (no pending command)"
    if cmd in ('n', 'no'):
        return cmd_취소확인()

    # ── 계속 진행 ─────────────────────────────────────────
    if cmd in ('/go', '/continue', '/proceed', '/진행', '계속'):
        return cmd_go()

    # ── 2단계 인증 (영문 + 한글) ──────────────────────────
    if cmd in ('/yes', '/확인'):
        return cmd_확인_2단계()
    if cmd in ('/abort', '/취소확인'):
        return cmd_취소확인()
    if cmd in ('/confirm', '/확정'):
        return cmd_확정_1단계(arg1)
    if cmd in ('/cancel', '/취소') and arg1:
        return cmd_취소_1단계(arg1, arg2)
    if cmd in ('/unreserve', '/예약취소'):
        return cmd_예약취소_1단계(arg1)

    # ── 조회 (영문 + 한글) ────────────────────────────────
    if cmd in ('/stock', '/재고'):
        return cmd_재고()
    if cmd in ('/outbound', '/출고'):
        return cmd_출고(int(arg1) if arg1.isdigit() else 0)
    if cmd in ('/pending', '/대기'):
        return cmd_대기()
    if cmd in ('/lot',):
        return cmd_lot(arg1)
    if cmd in ('/expiry', '/만료'):
        return cmd_만료()
    if cmd in ('/compare', '/비교'):
        return cmd_비교()
    if cmd in ('/integrity', '/정합성'):
        return cmd_정합성()
    if cmd in ('/backup', '/백업'):
        return cmd_백업()
    if cmd in ('/status', '/상태'):
        return cmd_상태()
    if cmd in ('/help', '/start', '/도움말', 'help'):
        return HELP_TEXT

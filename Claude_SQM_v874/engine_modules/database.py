...
try:
    ...
    norm_bl_no_for_query = None  # DB 모듈은 이 함수 없이도 로드 가능
...
class SQMDatabase(DatabaseSchemaMixin, DatabaseMigrationMixin, DatabaseInterface):
    ...
    def __init__(self, db_path: str = None, use_wal: bool = None, network_mode: bool = None) -> None:
        ...
        if self.network_mode:
            ...
            # 동시접속 최적화 PRAGMA (WAL 모드)
            try:
                self.execute("PRAGMA cache_size=-64000")   # 64MB 캐시
                self.execute("PRAGMA temp_store=MEMORY")    # 임시 테이블 메모리 사용
                self.execute("PRAGMA mmap_size=268435456")  # 256MB 메모리맵
                self.execute("PRAGMA synchronous=NORMAL")   # WAL에서 NORMAL이 안전+빠름
                logger.debug("동시접속 최적화 PRAGMA 적용 완료")
            except Exception as _pe:
                logger.debug("PRAGMA 최적화 일부 실패 (무시): %s", _pe)
    ...
    def transaction(self, mode: str = "IMMEDIATE") -> Any:
        ...
        v8.7.1: P1-3 중첩 트랜잭션 SAVEPOINT 지원 (부분 커밋 방지)
        ...
        # v8.7.1: 중첩 트랜잭션 → SAVEPOINT 사용 (부분 커밋 방지)
        if getattr(self._local, 'in_transaction', False):
            sp_name = f"sp_{id(self)}_{threading.get_ident()}"
            try:
                ...
                logger.error(f"중첩 트랜잭션 SAVEPOINT 롤백: {type(e).__name__}: {e}")
                raise
            finally:
    ...
    def fetchone(self, sql: str, params: tuple = (),
        ...
        if use_cache and sql.strip().upper().startswith('SELECT'):
            try:
                ...
                logger.debug("fetchone 캐시 경로 실패, 일반 경로 fallback: %s", exc)
        cursor = self.conn.cursor()
        cursor.execute(sql, params)
        row = cursor.fetchone()
    ...
    def fetchall(self, sql: str, params: tuple = (),
        ...
        if use_cache and sql.strip().upper().startswith('SELECT'):
            try:
                ...
                logger.debug("fetchall 캐시 경로 실패, 일반 경로 fallback: %s", exc)
        try:

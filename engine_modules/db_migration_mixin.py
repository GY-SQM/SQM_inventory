...
class DatabaseMigrationMixin:
    ...
    def _migrate_v868_packing_type_column(self) -> None:
        """
        v8.7.0: inventory.packing_type 및 inbound_template.packing_type 컬럼 추가.
        ...
        try:
            ...
            if "packing_type" not in cols:
                ...
                logger.info("[v8.7.0] inventory.packing_type 컬럼 추가")
            ...
            if "packing_type" not in tpl_cols:
                ...
                logger.info("[v8.7.0] inbound_template.packing_type 컬럼 추가")
            self.execute(
    ...
    def _migrate_v868_pending_workflow_columns(self) -> None:
        """
        v8.7.0: inventory 테이블에 port_date, inbound_type 컬럼 추가.
        ...
        try:
            ...
            if "port_date" not in cols:
                ...
                logger.info("[v8.7.0] inventory.port_date 컬럼 추가")
            if "inbound_type" not in cols:
                self.execute("ALTER TABLE inventory ADD COLUMN inbound_type TEXT DEFAULT 'DIRECT'")
                logger.info("[v8.7.0] inventory.inbound_type 컬럼 추가")
        except Exception as e:
            logger.warning(f"[v8.7.0] PENDING 워크플로우 컬럼 마이그레이션 실패: {e}")

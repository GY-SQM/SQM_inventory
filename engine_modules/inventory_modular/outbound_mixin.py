...
class OutboundMixin(InventoryBaseMixin):
                ...
                for r in staged_rows:
                    ...
                    self.db.execute(
                        """UPDATE inventory_tonbag
                           SET status=?, picked_to=?, sale_ref=?, updated_at=?
                           WHERE id=?""",
                        (STATUS_RESERVED, customer, sale_ref, now, tb["id"]),
                    )
                    self.db.execute(
                        """UPDATE allocation_plan
                           SET status='RESERVED', tonbag_id=?, sub_lt=?, workflow_status='APPLIED'
                           WHERE id=? AND status='STAGED' AND workflow_status='APPROVED'""",
                        (tb["id"], tb.get("sub_lt"), plan_id),
                    )
                    result["applied"] += 1
                    try:
                        details = {
    ...
    def gate1_verify_picking(
        ...
        try:
            ...
            lines.append('=' * 60)
            result['error_report'] = '\n'.join(lines)
            logger.info(f'[Gate-1] passed=%s, matched=%s, missing=%s, qty_mismatch=%s',

    ...
    def _export_tonbag_list(self, output_path: str) -> str:
        ...
        try:
            ...
            for idx, item in enumerate(tonbags, 1):
                ...
                status = item.get('tonbag_status') or item.get('status') or 'AVAILABLE'
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                self._apply_excel_formatting(
                    ws, df,
                    title=f'SQM 톤백리스트 — {__import__("datetime").datetime.now().strftime("%Y-%m-%d")}',
                    col_widths={h: w for _, h, w, _ in column_map},
                    num_formats={h: fmt for _, h, _, fmt in column_map if fmt},
                )

(function () {
  ...
  /* ═══════════════════════════════════════════════════════════════
     v8.7.0 (2026-05-25): 바코드 스캔 → SOLD 확정 모달
     2단계: dry_run preview → 사용자 확인 → 실제 적용
     ═══════════════════════════════════════════════════════════════ */
  function showBarcodeSoldConfirmModal() {
    // v8.7.0 fix: 메뉴바 자동 닫기 (onclick으로 호출되면 closeAllMenus 미호출되는 문제)
    ...
    function _renderResult(d, isPreview) {
      ...
      // v8.7.0: 이전 상태 분포 표시 (어떤 상태에서 SOLD로 전환되는지)
      if (matched.length > 0) {
        var prevDist = {};
    ...
    btnApply.onclick = function() {
      ...
      _upload(false, function(d) {
        btnApply.style.display = 'none';
        btnPrev.disabled = true;
        // v8.7.0 fix Issue 3: SOLD 확정 후 관련 페이지 자동 새로고침
  ...
  /* ═══════════════════════════════════════════════════════════════
     v8.7.0 (2026-05-25): 피킹리스트 기반 샘플 SOLD 확정 모달
